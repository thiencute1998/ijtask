...
<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <DocCateView v-model="value" v-if="!isForm">
      </DocCateView>

      <DocCateForm v-model="value" v-if="isForm">
      </DocCateForm>
...
<script>
  ...
  import DocCateView from "./DocCateView";
  import DocCateForm from "./DocCateForm";

  export default {
    name: 'DocCateModal',
    mixins: [mixinLists],
    components: {DocCateForm, DocCateView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'doc/api/doc/update-doc-cate';
        const requestData = {
          method: 'post',
          url: UpdateApi,
          data: {
