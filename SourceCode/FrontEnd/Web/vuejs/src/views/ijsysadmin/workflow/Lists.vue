                    ...
                    <b-row class="mb-2">
                        <b-col md="6" class="col-24">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Quy trình công việc</span>
                            </div>
                        </b-col>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.WFName}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    export default {
      ...
      computed: {
        ...
        captions: function() {
          let fieldsTable = [
            ...
            {key: 'WFName', label: 'Tên quy trình công việc', field: 'WFName', searchOnTopRow: {type: 'text'}, sortable: true},
            // {key: 'Inactive', label: 'Trạng thái', field: 'inactive', tdClass: 'pr-3', thClass: 'pr-3', thStyle: 'width: 15%'},
          ];
          return fieldsTable;
        }
      },
      created() {
        // init setting
        this.settings.FieldID = 'WFID';
        this.settings.Table = 'workflow';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'sysadmin/api/workflow';
        this.settings.DeleteApi = 'sysadmin/api/workflow/delete';
        this.settings.CreateRouter = 'sysadmin-workflow-create';
        this.settings.EditRouter = 'sysadmin-workflow-edit';
        this.settings.ViewRouter = 'sysadmin-workflow-view';
      ...
      methods: {
        ...
        fetchData(){
          ...
          if (this.modelSearch.WFName !== '') {
            requestData.data.WFName = this.modelSearch.WFName;
          }
          ...
          if (this.modelSearch.direction) {
            requestData.data.direction = this.modelSearch.direction;
          }
        ...
        sortChanged(ctx) {
          if (this.modelSearch.direction === 'desc') {
            this.modelSearch.direction = 'asc';
          } else {
            this.modelSearch.direction = 'desc';
          }
          this.fetchData();
        },
        handleExportExcel() {
