                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Người dùng</span>
                            </div>
                        </b-col>
                        <b-col class="col-md-12">
...
<!--                            <template v-slot:button-content>-->
<!--&lt;!&ndash;                              <i class="fa fa-random m-0" v-if="stage.filterInactive === 2"></i>&ndash;&gt;-->
...
<!--                            <b-row>-->
<!--                                <b-col md="2">-->
<!--                                    <b-form-group label-for="searchFieldFullName" class="m-0">-->
<!--                                        <b-form-input-->
<!--                                                id="searchFieldFullName" type="text"-->
<!--                                                v-model="modelSearch.fullName"-->
                      ...
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                        <b-table ref="selectableTable"
                              ...
                              <template slot="top-row" slot-scope="data">
                                  <td :class="[(field.key == 'selected') ? 'pl-3' : '', (field.key == 'UserType') ? 'pr-3' : '']" role="cell" v-for="(field, key) in data.fields">
                                      <!-- type input -->
                                      <b-form-input
                              ...
                              <template v-slot:cell(Avata)="data">
                                <div>
                                  <img :src="baseUrl + data.item.Avata" style="max-height: 20px; object-fit: cover"/>
                                </div>
                              </template>
                          </b-table>
                      </div>
                        ...
                        <div class="kanban-items row">
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-lg-flex justify-content-between">
                                <span class="kanban-title">{{item.username}}</span>
                              </div>
...
<script>
    ...
    export default {
      ...
      computed: {
        ...
        captions: function() {
          let fieldsTable = [
            ...
            // {key: 'Avata', label: 'Ảnh đại diện', thStyle: 'width: 10%'},
            {key: 'username', label: 'Tên đăng nhập', field: 'username', thStyle: 'width: 20%; min-width: 200px',searchOnTopRow: {type: 'text'}},
            {key: 'FullName', label: 'Họ và tên', field: 'FullName', thStyle: 'width: 20%; min-width: 300px', searchOnTopRow: {type: 'text'}},
            {key: 'Note', label: 'Ghi chú', field: 'Note', thStyle: 'width: 50%; min-width: 300px'},
          ];
          return fieldsTable;
        }
      },
      created() {
        // init setting
        this.settings.FieldID = 'UserID';
        this.settings.Table = 'sys_user';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'sysadmin/api/users';
        this.settings.DeleteApi = 'sysadmin/api/users/delete';
        this.settings.CreateRouter = 'sysadmin-user-create';
        this.settings.EditRouter = 'sysadmin-user-edit';
        this.settings.ViewRouter = 'sysadmin-user-view';
      ...
      methods: {
          ...
          // },
          // handleEditItem(data) {
          //     let router = this.settings.EditRouter + '/' + data.item.UserID;
          ...
          //     if (item.Inactive !== self.stage.actionInactive.Inactive) {
      ...
      watch: {
        ...
        // 'stage.updatedData'(){
        //   if (this.state.updatedData) this.fetchData();
        // }
      }
    }
</script>
