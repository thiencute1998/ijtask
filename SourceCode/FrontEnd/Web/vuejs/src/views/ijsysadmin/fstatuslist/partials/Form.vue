            ...
            <vue-perfect-scrollbar class="scroll-area" :settings="$store.state.psSettings">
                <div class="container-fluid">
                    <b-card>
                        ...
                        <div class="form-group row align-items-center">
                            <label class="col-md-4 col-sm-4 col-24 mb-2 mb-sm-0 mb-md-0">Chức năng</label>
                            <div class="col-md-20 col-sm-20 col-24">
                              <Select2 v-model="model.featureID" :options="model.featureOption"></Select2>
                            </div>
                        </div>
                        ...
                        <table class="table b-table table-sm table-bordered table-editable">
                            ...
                            <tbody>
                            <tr v-for="(field, key) in model.sysFeatureStatusItem">
                                <td>
                                    <b-form-input
                                            type="text"
                                            v-model="model.sysFeatureStatusItem[key].StatusDescription"
                                            autocomplete="task code list description">
                                    </b-form-input>
                                </td>
                                <td>
                                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)" v-model="model.sysFeatureStatusItem[key].DataType"></b-form-select>
                                </td>
                                <td>
                                    <b-form-input
                                            v-if="field.DataType === 1"
                                            type="number"
                                            v-model="model.sysFeatureStatusItem[key].StatusValue"
                                            autocomplete="task code list description">
                                    </b-form-input>
                                    <b-form-input
                                    ...
                                    <b-form-select v-if="field.DataType === 5" v-model="model.sysFeatureStatusItem[key].StatusValue" :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                                    <b-form-select v-if="field.DataType === 6" v-model="model.sysFeatureStatusItem[key].StatusValue" :options="[{value: 1, text: 'Đúng'},{value: 2, text: 'Sai'}]"></b-form-select>
...
<script>
    ...
    const ListRouter = 'sysadmin-fstatuslist';
    const EditRouter = 'sysadmin-fstatuslist-edit';
    const CreateRouter = 'sysadmin-fstatuslist-create';
    const ViewRouter = 'sysadmin-fstatuslist-view';
    const ViewApi = 'sysadmin/api/fstatuslist/view';
    const EditApi = 'sysadmin/api/fstatuslist/edit';
    const CreateApi = 'sysadmin/api/fstatuslist/create';
    const StoreApi = 'sysadmin/api/fstatuslist/store';
    const UpdateApi = 'sysadmin/api/fstatuslist/update';
    const ListApi = 'sysadmin/api/fstatuslist';
    ...
    export default {
        ...
        methods: {
                    ...
                    if (responsesData.status === 1) {
                        if (self.idParams || !_.isEmpty(self.itemCopy)) {
                            if (_.isObject(responsesData.data.data)) {
                                ...
                                self.model.featureID = responsesData.data.data.FeatureID;
                                self.model.inactive = (responsesData.data.data.Inactive) ? true : false;

                                self.model.sysFeatureStatusItem = responsesData.data.SysFeatureStatusItem;
                                // set max lineID
                                _.forEach(self.model.sysFeatureStatusItem, function (field, key) {
                                    if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                                    // set type of value
                                    if (field.DataType == 1) self.model.sysFeatureStatusItem[key].StatusValue = Number(self.model.sysFeatureStatusItem[key].StatusValue);
                                    if (field.DataType == 2) self.model.sysFeatureStatusItem[key].StatusValue = String(self.model.sysFeatureStatusItem[key].StatusValue);
                                    if (field.DataType == 3) self.model.sysFeatureStatusItem[key].StatusValue = moment(self.model.sysFeatureStatusItem[key].StatusValue).format('DD/MM/YYYY');
                                    if (field.DataType == 4) self.model.sysFeatureStatusItem[key].StatusValue = moment(self.model.sysFeatureStatusItem[key].StatusValue).format('DD/MM/YYYY hh:mm');
                                });
                            ...
                            if (_.isArray(responsesData.data)) {
                                ...
                                _.forEach(responsesData.data, function (value, key) {
                                    let tmpObj = {};
                                    tmpObj.id = value.FeatureID;
                                  tmpObj.text = value.FeatureName;
                                    self.model.featureOption.push(tmpObj);
                                });
                            }
                        }

                        if (_.isArray(responsesData.data.feature)) {

                            self.model.featureOption = [];
                            _.forEach(responsesData.data.feature, function (value, key) {
                                let tmpObj = {};
                                tmpObj.id = value.FeatureID;
                                tmpObj.text = value.FeatureName;
                                self.model.featureOption.push(tmpObj);
            ...
            onAddFieldOnTable(){
                ...
                this.model.sysFeatureStatusItem.push(fieldObj);
                this.$forceUpdate();
            },
            onDeleteFieldOnTable(field){

                // remove field in fieldOnTableReq
                let fieldExist = _.find(this.model.sysFeatureStatusItem, ['LineID', field.LineID]);
                if (_.isObject(fieldExist)) {
                    _.remove(this.model.sysFeatureStatusItem, ['LineID', field.LineID]);
                }
                this.$forceUpdate();
            },
            handleSubmitForm(){
                ...
                const requestData = {
                    ...
                    data: {
                        ...
                        detail: this.model.sysFeatureStatusItem
                    }
                };
