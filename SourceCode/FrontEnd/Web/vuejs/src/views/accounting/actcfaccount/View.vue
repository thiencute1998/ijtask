        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span><i class="icon-eye icon mr-2"></i> Tài khoản kết chuyển</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-21 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                {{AccountingCFAccount.CoaTypeName}}
              </div>
            </div>
            <div class="form-group row">
              <label class="col-md-3" >Số TK từ:</label>
              <div class="col-md-5">
                {{AccountingCFAccount.FromAccountNo}}
              </div>
              <label class="col-md-3" >Số TK đến:</label>
              <div class="col-md-5">
                {{AccountingCFAccount.ToAccountNo}}
              </div>
              <label class="col-md-3" >Loại số dư:</label>
              <div class="col-md-5">
                <span>{{AccountingCFAccount.CFAccountTypeOption[AccountingCFAccount.CFAccountType]}}</span>
              </div>
            </div>
            <div class="form-group row">
              <label class="col-md-3" >Diễn giải:</label>
              <div class="col-md-21">
                <span >{{AccountingCFAccount.Description}}</span>
              </div>
            </div>
          </b-card>
...
const DeleteApi = 'accounting/api/actcfaccount/delete';
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          self.AccountingCFAccount.CFAccountID = responsesData.data.data.CFAccountID;
          self.AccountingCFAccount.CoaTypeID = responsesData.data.data.CoaTypeID;
          self.AccountingCFAccount.CoaTypeNo = responsesData.data.data.CoaTypeNo;
          self.AccountingCFAccount.CoaTypeName = responsesData.data.data.CoaTypeName;
          self.AccountingCFAccount.FromAccountID = responsesData.data.data.FromAccountID;
          self.AccountingCFAccount.FromAccountNo = responsesData.data.data.FromAccountNo;
          self.AccountingCFAccount.ToAccountID = responsesData.data.data.ToAccountID;
          self.AccountingCFAccount.ToAccountNo = responsesData.data.data.ToAccountNo;
          self.AccountingCFAccount.CFAccountType = responsesData.data.data.CFAccountType;
          self.AccountingCFAccount.Norder = responsesData.data.data.Norder;
          self.AccountingCFAccount.Description = responsesData.data.data.Description;
          self.AccountingCFAccount.Inactive = (responsesData.data.data.Inactive) ? true : false;
        }
        else if(responsesData.status === 3){
          self.$router.push({name: ListRouter, params: {message: responsesData.msg}});
        }
        self.AccountingCFAccount.CFAccountTypeOption = responsesData.CFAccountTypeOption;
    ...
    getItemIds(type){
      ...
      if (this.reqParams.search.Description !== '') {
        requestData.data.Description = this.reqParams.search.Description;
      }
