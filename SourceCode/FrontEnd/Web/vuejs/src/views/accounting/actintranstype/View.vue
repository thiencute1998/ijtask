        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span><i class="icon-eye icon mr-2"></i> Loại nghiệp vụ kế toán: {{AccountingInTransType.InTransTypeName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row">
              ...
              <div class="col-md-21 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-2 app-object-name">
                {{AccountingInTransType.InTransTypeName}}
              </div>
            </div>
            <div class="form-group row">
              <label class="col-md-3 m-0">Diễn giải</label>
              <label class="col-md-21 m-0" id="view-Comment">
                {{AccountingInTransType.Comment}}
              </label>
            </div>
          </b-card>
...
const ListRouter = 'accounting-actintranstype';
const EditRouter = 'accounting-actintranstype-edit';
const CreateRouter = 'accounting-actintranstype-create';
const ViewApi = 'accounting/api/actintranstype/view';
const ListApi = 'accounting/api/actintranstype';
const DeleteApi = 'accounting/api/actintranstype/delete';
const UpdateApi = 'accounting/api/actintranstype/update';

export default {
  name: 'accounting-view-item',
  data () {
    return {
      AccountingInTransType: {
        InTransTypeID: null,
        InTransTypeName: '',
        Comment: '',
        Inactive: false,
  ...
  methods: {
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          self.AccountingInTransType.InTransTypeID = responsesData.data.data.InTransTypeID;
          self.AccountingInTransType.InTransTypeName = responsesData.data.data.InTransTypeName;
          self.AccountingInTransType.Comment = responsesData.data.data.Comment;
          self.AccountingInTransType.Inactive = (responsesData.data.data.Inactive) ? true : false;
    ...
    getItemIds(type){
      ...
      if (this.reqParams.search.InTransTypeName !== '') {
        requestData.data.InTransTypeName = this.reqParams.search.InTransTypeName;
      }
      if (this.reqParams.search.Comment !== '') {
        requestData.data.Comment = this.reqParams.search.Comment;
      }
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (dataResponse.status === 1) {
          ...
          _.forEach(dataResponse.data.data, function (value, key) {
            self.reqParams.idsArray.push(value.InTransTypeID);
          });
