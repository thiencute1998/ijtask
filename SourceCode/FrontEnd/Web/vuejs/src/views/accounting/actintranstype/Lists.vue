                  ...
                  <b-row class="mb-2">
                      <b-col md="6" class="col-24">
                          <div class="main-header-item main-header-name">
                              <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Loại nghiệp vụ kế toán</span>
                          </div>
                      </b-col>
                        ...
                        <div class="b-table-sticky-header table-responsive">
                          <table role="table" aria-busy="false" aria-colcount="2" aria-multiselectable="true"
                            ...
                            <tr role="row" class="">
                              ...
                              <th role="columnheader" scope="col" style="width: 20%; min-width: 120px" aria-sort="none" @click="sortTableByField($event, 'TransDate')" v-if="stage.viewType === 'list'">Tên nghiệp vụ</th>
                              <th role="columnheader" scope="col" style="width: 20%; min-width: 120px" v-else>Tên nghiệp vụ</th>
...
<!--                            <transition-group tag="tbody" name="slide-fade">-->
                              <tr role="row" :key="'table-top-row'" class="b-table-top-row">
                                ...
                                <td role="cell" class="">
                                  <b-form-input v-model="modelSearch.InTransTypeName " @change="$_lists_handleSubmitSearch">
                                  </b-form-input>
                                </td>
                              ...
                              <tr role="row" tabindex="0"
                                ...
                                <td @click="onRowClicked(item)" aria-colindex="2" role="cell" class="text-left">{{item.InTransTypeName }}</td>
...
<!--                                <span class="kanban-title">{{item.TaskName}}</span>-->
                              ...
                              <div class="kanban-record">
                                <span>Mã số</span>
                                <span>{{item.InTransTypeName }}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    export default {
      ...
      computed: {
        ...
        captions: function() {
          let fieldsTable = [
            ...
            {key: 'InTransTypeName', label: 'Tên nghiệp vụ', field: 'InTransTypeName',searchOnTopRow: {type: 'text'}, thStyle: 'width: 8%'},
      ...
      created() {
        // init setting
        this.settings.FieldID = 'InTransTypeID';
        this.settings.Table = 'act_intranstype';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'accounting/api/actintranstype';
        this.settings.DeleteApi = 'accounting/api/actintranstype/delete';
        this.settings.CreateRouter = 'accounting-actintranstype-create';
        this.settings.EditRouter = 'accounting-actintranstype-edit';
        this.settings.ViewRouter = 'accounting-actintranstype-view';
      ...
      methods: {
        ...
        fetchData(){
          ...
          if (this.modelSearch.InTransTypeName  !== '') {
            requestData.data.InTransTypeName  = this.modelSearch.InTransTypeName ;
          }
        ...
        handleExportPrint() {
          ...
          let request = {};
          if (this.modelSearch.InTransTypeName  !== '') {
            request.InTransTypeName  = this.modelSearch.InTransTypeName ;
          }
          if (this.modelSearch.Comment !== '') {
