<template>
                ...
                <b-row class="mb-2">
                    <b-col class="col-md-18">
                        <div class="main-header-item main-header-name">
                            <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Ước thực hiện dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                            <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Ước thực hiện dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                        </div>
                    </b-col>
  ...
  <!--                        <IjcoreModalListing-->
  <!--                          v-model="model" :title="'định mức doanh thu'" :api="'/listing/api/common/list'" @changed="selectednormtable"-->
  <!--                          :table="'norm_table'" :FieldID="'NormTableID'" :FieldName="'NormTableName'" :FieldNo="'NormTableID'">-->
  <!--                        </IjcoreModalListing>-->
                        </div>
  ...
  <!--                      </div>-->
                  ...
                  <div class="table-responsive b-table-sticky-header b-table-sticky-custom m-0" style="max-height: 350px;">
                    ...
                    <tr>
                      ...
                      <!-- Số tiền -->
                      <th scope="col" style="min-width: 150px" class="text-center">Thành tiền <span v-if="model.item_tt">NT</span>  </th>
                      <th v-if="model.item_tt" scope="col" style="min-width: 150px" class="text-center">Thành tiền QĐ  </th>
                      <!-- -->
...
<!--                        <IjcoreModalAutoact-->
<!--                          v-model="model.ActgvoucTransItem[key]" :title="'bút toán'" :api="'/listing/api/common/list'" :FieldWhere="{SysAutoactTypeID : 115}"-->
...
<!--                        </IjcoreModalAutoact>-->
                      ...
                      <!-- Số dự toán -->
                      <td>
                        <ijcore-number v-model="model.ActgvoucTransItem[key].FCPlanAmount" @input="addtoFCPlanAmount(model.ActgvoucTransItem[key].FCPlanAmount, key)"></ijcore-number>
                      </td>
                      <td v-if="model.item_tt">
                        <ijcore-number v-model="model.ActgvoucTransItem[key].LCPlanAmount" ></ijcore-number>
                      </td>
                      <!-- Số đã TH -->
                      <td>
                        <ijcore-number v-model="model.ActgvoucTransItem[key].FCExecAmount" @input="addtoFCExecAmount(model.ActgvoucTransItem[key].FCExecAmount, key)"></ijcore-number>
                      </td>
                      <td v-if="model.item_tt">
                        <ijcore-number v-model="model.ActgvoucTransItem[key].LCExecAmount" ></ijcore-number>
                      </td>
                      <!--Tài sản/Vật tư -->
...
<script>
    ...
    const ViewRouter = 'statebudgetplanning-sbpestimateplan-view';
    ...
    const EditApi = 'state-budget-planning/api/sbpestimateplan/edit';
    const CreateApi = 'state-budget-planning/api/sbpestimateplan/create';
    const StoreApi = 'state-budget-planning/api/sbpestimateplan/store';
    const UpdateApi = 'state-budget-planning/api/sbpestimateplan/update';
    ...
    export default {
        ...
        methods: {
          ...
          addtoFCPlanAmount(evt, key){
            let FCPlanAmount = this.model.ActgvoucTransItem[key].FCPlanAmount;
            let ExchangeRate = this.model.ActgvoucTransItem[key].ExchangeRate;
            let LCPlanAmount = Number(FCPlanAmount) * Number(ExchangeRate);
            if(LCPlanAmount=='Infinity'){LCPlanAmount='';}
            if(LCPlanAmount){//alert(LCUnitPrice);
              this.model.ActgvoucTransItem[key].LCPlanAmount = LCPlanAmount;
            }else{
              this.model.ActgvoucTransItem[key].LCPlanAmount ='';
            }
            //this.$forceUpdate()
          },
          addtoFCExecAmount(evt, key){
            let FCExecAmount = this.model.ActgvoucTransItem[key].FCExecAmount;
            let ExchangeRate = this.model.ActgvoucTransItem[key].ExchangeRate;
            let LCExecAmount = Number(FCExecAmount) * Number(ExchangeRate);
            if(LCExecAmount=='Infinity'){LCExecAmount='';}
            if(LCExecAmount){//alert(LCUnitPrice);
              this.model.ActgvoucTransItem[key].LCExecAmount = LCExecAmount;
            }else{
              this.model.ActgvoucTransItem[key].LCExecAmount ='';
            }
            //this.$forceUpdate()
          },
                 ...
                 if (result.value) {
                   ...
                   ApiService.customRequest(requestData).then((response) => {
                     ...
                     if (responseData.status === 1) {
                       ...
                       //self.onBackToList('Đã thêm tỷ lệ phân bổ điều tiết!');
                     } else {
                       Swal.fire(
                         'Có lỗi',
                         '',
          ...
          selectednormtable(){
              ...
              const requestData = {
                method: 'post',
                url: 'state-budget-planning/api/sbpestimateplan/getNormTableItem',
                data: {
                  NormTableID: this.model.NormTableID,
                }
              };
            ...
            if(data.length){
              ...
              data.map(function (val, key) {
                ...
                self.model.ActgvoucTransItem.push({
                  ...
                  AutoactID: 32,
                  AutoactName: 'Ước thực hiện dự toán',
                ...
                LineID++;
                // if(val.NormType) {
                //   let Autoact = _.filter(self.model.Autoact, ['AutoactType', Number(val.NormType)]);
                //   // let Autoact = _.filter(self.model.Autoact, {
                //   //     AutoactType: '1',
                //   //     AutoactType: '2',
                //   //     AutoactType: '3',
                //   // });
                //   //let Autoact = _.filter(self.model.Autoact,{'AutoactType': Number(val.NormType),'AutoactType': 2});
                //   self.model.Autoact = Autoact;
                // }
              });
          ...
          changeTemplate() {
            ...
            requestData.url = '/state-budget-planning/api/sbpestimateplan/load-temp/' + this.cloneTransID;
            this.$store.commit('isLoading', true);
            ApiService.setHeader();
            ApiService.customRequest(requestData).then((responses) => {
              let responsesData = responses.data; //console.log(responses.data);
              if (responsesData.status === 1) {
                ...
                self.model.NormTableNo = responsesData.data.data.NormTableNo;
                self.model.NormTableName = responsesData.data.data.NormTableName;
                self.model.TransTypeID = 8;

                self.model.ActgvoucTransItem = responsesData.data.SbpmakeplanTrans;
                _.map(self.model.ActgvoucTransItem, function (value, key){
                  //value.AutoactName = value.actautoact.AutoactName;
                  value.AutoactID = 32;
                  value.AutoactName = 'Ước thực hiện dự toán';
                  value.FCPlanAmount  = value.FCDebitAmount;
                  value.LCPlanAmount  = value.LCDebitAmount;
                  value.FCNormAmount = value.FCDebitAmount;
                  value.LCNormAmount = value.LCDebitAmount;
                  value.FCApprovedAmount = value.FCDebitAmount;
                  value.LCApprovedAmount = value.LCDebitAmount;
                  self.$set(self.model.ActgvoucTransItem[key], 'Show', true);
                  self.$set(self.model.ActgvoucTransItem[key], 'ShowPagination', false);
                  if (value.LineID > self.RowItem) {
                    self.RowItem = value.LineIDTmp;
