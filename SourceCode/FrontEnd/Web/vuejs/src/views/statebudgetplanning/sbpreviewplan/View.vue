                ...
                <b-row class="mb-2">
                    <b-col class="col-md-18">
                        <div class="main-header-item main-header-name">
                            <span><i class="icon-eye icon mr-2"></i> Xem xét dự toán</span>
                        </div>
                    </b-col>
                          ...
                          <b-dropdown variant="primary" id="dropdown-actions" class="main-header-action mr-2" text="Thao tác">
                                <li class="dropdown b-dropdown dropright">
                                  ...
                                  <ul role="menu" tabindex="-1" class="dropdown-sub-menu dropdown-menu m-0">
                                    <li role="presentation"><a role="menuitem" target="_self" class="dropdown-item" @click="handleExportPrintCV01">Công văn STC gửi các đơn vị xem xét</a></li>
                                    <li role="presentation"><a role="menuitem" target="_self" class="dropdown-item" @click="handleExportPrintCV02">Công văn STC gửi các đơn vị xem xét vòng 2</a></li>
                                    <li role="presentation"><a role="menuitem" target="_self" class="dropdown-item" @click="handleExportPrintToTrinh">tờ trình</a></li>
                                    <li role="presentation"><a role="menuitem" target="_self" class="dropdown-item" @click="handleExportPrintToTrinhUBNDtinh">tờ trình STC_UBND tỉnh</a></li>

                                    <li role="presentation"><a role="menuitem" target="_self" class="dropdown-item" @click="handleExportPrintDTNSSChungTuGiaoDuToanBM48">Quyết định về việc giao dự toán thu chi ngân sách nhà nước BM48</a></li>
                                    <li v-if="model.TransID == 150" role="presentation"><a role="menuitem" target="_self" href="/#/report/UocThucHien_VPSoNN-XemXet" class="dropdown-item" @click="handleExportPrint">Xem xét dự toán ngân sách nhà nước</a></li>
                                    <li v-if="model.TransID == 141" role="presentation"><a role="menuitem" target="_self" href="/#/report/UTH_SoKHDT-XemXet" class="dropdown-item" @click="handleExportPrint">Xem xét dự toán ngân sách nhà nước</a></li>
                                    <li v-if="model.TransID == 133" role="presentation"><a role="menuitem" target="_self" href="/#/report/xemxet" class="dropdown-item" @click="handleExportPrint">Xem xét dự toán thu chi ngân sách nhà nước</a></li>
                                    <li v-if="model.TransID == 156" role="presentation"><a role="menuitem" target="_self" href="/#/report/SBP_UTH_Demo_modul_du_toan_02" class="dropdown-item" @click="handleExportPrint">Xem xét dự toán ngân sách nhà nước</a></li>
                                    <li v-if="model.TransID == 142" role="presentation"><a role="menuitem" target="_self" href="/#/report/SBP_UTH_SOKHDT_04" class="dropdown-item" @click="handleExportPrint">Xem xét dự toán ngân sách nhà nước</a></li>
...
<!--                      </td>-->
                      <!-- Định mức chỉ tiêu -->
                      <td class="text-left">{{field.NormTableItemNo}}</td>
                      <td class="text-left">
                      ...
                      <!-- Định mức -->
                      <td>
                        <span class="text-right">{{field.FCNormAmount | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_tt">
                        <span class="text-right">{{field.LCNormAmount | convertNumberToText}}</span>
                      </td>
                      <!-- Lập dự toán -->
                      <td>
                        <span class="text-right">{{field.FCRequestAmount | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_tt">
                        <span class="text-right">{{field.LCRequestAmount | convertNumberToText}}</span>
                      </td>
                      <!--Tài sản/Vật tư -->
                      ...
                      <td v-if="model.item_3n">
                        <span class="text-right">{{field.FCDebitAmount1 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_3n && model.item_tt">
                        <span class="text-right">{{field.LCDebitAmount1 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_3n">
                        <span class="text-right">{{field.FCDebitAmount2 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_3n && model.item_tt">
                        <span class="text-right">{{field.LCDebitAmount2 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_3n">
                        <span class="text-right">{{field.FCDebitAmount3 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_3n && model.item_tt">
                        <span class="text-right">{{field.LCDebitAmount3 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_5n">
                        <span class="text-right">{{field.FCDebitAmount4 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_5n && model.item_tt">
                        <span class="text-right">{{field.LCDebitAmount4 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_5n">
                        <span class="text-right">{{field.FCDebitAmount5 | convertNumberToText}}</span>
                      </td>
                      <td v-if="model.item_5n && model.item_tt">
                        <span class="text-right">{{field.LCDebitAmount5 | convertNumberToText}}</span>
                      </td>
                      <!-- -->
                      <td>
...
<script>
    ...
    const DeleteApi = 'state-budget-planning/api/sbpreviewplan/delete';
    const CompanyType = {
        1: 'Đơn vị tổng hợp',
        2: 'Đơn vị trực thuộc',
        3: 'Đơn vị liên kết',
    ...
    export default {
        ...
        methods: {
          ...
          handleExportPrintCV01: function(){
              let request = {};
              request.id = this.idParams;
              request.exportData = true;
              this.$router.push({
                name: 'report_tt3422016cv1ttbtc',
                query: request
              });
            },
          handleExportPrintCV02: function(){
              let request = {};
              request.id = this.idParams;
              request.exportData = true;
              this.$router.push({
                name: 'report_tt3422016cv2ttbtc',
                query: request
              });
            },
          handleExportPrintToTrinh: function(){
              let request = {};
              request.id = this.idParams;
              request.exportData = true;
              this.$router.push({
                name: 'PFMIS_DUTOAN_CVTT-ĐVDT_CQTC-TTr',
                query: request
              });
            },
          handleExportPrintToTrinhUBNDtinh: function(){
              let request = {};
              request.id = this.idParams;
              request.exportData = true;
              this.$router.push({
                name: 'PFMIS_DUTOAN_TOTRINH_STC_UBNDtinh',
                query: request
              });
            },

          handleExportPrintDTNSSChungTuGiaoDuToanBM48: function(){
            let request = {};
            request.id = this.idParams;
            request.exportData = true;
            this.$router.push({
              name: 'PFMIS_DTNSNN_CHUNGTU_GIAODT_BM48',
              query: request
            });
          },
            handleExportReport(){
          ...
          actbook(TransID) {
            ...
            const requestData = {
              method: 'post',
              url: 'state-budget-planning/api/sbpreviewplan/actbook',
              data: {
