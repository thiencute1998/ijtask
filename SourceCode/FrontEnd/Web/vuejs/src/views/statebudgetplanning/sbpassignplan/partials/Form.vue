                ...
                <b-row class="mb-2">
                    <b-col class="col-md-18">
                        <div class="main-header-item main-header-name">
                            <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Giao dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                            <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Giao dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                        </div>
                    </b-col>
            ...
            <div class="card">
              <div class="form-group row" style="margin-bottom: 0px !important; margin-top: 10px; margin-right: 0px;">
                <div class="col-md-13 col-lg-13 top-trans">
                  <!--                      <b-card no-body>-->
                  <b-tabs card>
                    <b-tab title="Đơn vị giao" active>
                      <b-card-text>
                        <div class="form-group row align-items-center">
                          <label class="col-md-4 col-xl-3 m-0">Tên</label>
                          <div class="col-md-20 col-xl-21">
                            <IjcoreModalSearchCompany
                              v-model="model.ParentCompanyName" :fieldCateList="'28'" :fieldCateValue="[0,1,2]" :title="'Đơn vị giao'" :table="'company'" :api="'/listing/api/company/get-cate-value'"
                              :placeholderInput="'Đơn vị giao'" @changeCompanyState1="handleGetChangedCompany1">
                            </IjcoreModalSearchCompany>
                          </div>
                        </div>
                        <!--                              <div class="form-group row align-items-center">-->
...
<!--                          <IjcoreModalAutoact-->
<!--                            v-model="model.ActgvoucTransItem[key]" :title="'bút toán'" :api="'/listing/api/common/list'" :FieldWhere="{SysAutoactTypeID : 119}"-->
...
<script>
    ...
    const ListRouter = 'statebudgetplanning-sbpassignplan';
    const EditRouter = 'statebudgetplanning-sbpassignplan-edit';
    const ViewRouter = 'statebudgetplanning-sbpassignplan-view';
    const CreateRouter = 'statebudgetplanning-sbpassignplan-create';
    const ViewApi = 'state-budget-planning/api/sbpassignplan/view';
    const EditApi = 'state-budget-planning/api/sbpassignplan/edit';
    const CreateApi = 'state-budget-planning/api/sbpassignplan/create';
    const StoreApi = 'state-budget-planning/api/sbpassignplan/store';
    const UpdateApi = 'state-budget-planning/api/sbpassignplan/update';
    const ListApi = 'state-budget-planning/api/sbpassignplan';
    ...
    export default {
        ...
        methods: {
          ...
          addtoFCApprovedAmount(evt, key){
            let FCApprovedAmount = this.model.ActgvoucTransItem[key].FCApprovedAmount;
            let ExchangeRate = this.model.ActgvoucTransItem[key].ExchangeRate;
            let LCApprovedAmount = Number(FCApprovedAmount) * Number(ExchangeRate);
            if(LCApprovedAmount=='Infinity'){LCApprovedAmount='';}
            if(LCApprovedAmount){//alert(LCReviewAmount);
              this.model.ActgvoucTransItem[key].LCApprovedAmount = LCApprovedAmount;
            }else{
              this.model.ActgvoucTransItem[key].LCApprovedAmount ='';
            }
            //this.$forceUpdate()
          },
          ...
          selectednormtable(){
              ...
              const requestData = {
                method: 'post',
                url: 'state-budget-planning/api/sbpassignplan/getNormTableItem',
                data: {
                  NormTableID: this.model.NormTableID,
                }
              };
          ...
          changeTemplate() {
            ...
            requestData.url = '/state-budget-planning/api/sbpassignplan/load-temp/' + this.cloneTransID;
            this.$store.commit('isLoading', true);
            ApiService.setHeader();
            ApiService.customRequest(requestData).then((responses) => {
              let responsesData = responses.data; //console.log(responses.data);
              if (responsesData.status === 1) {
                ...
                self.model.NormTableNo = responsesData.data.data.NormTableNo;
                self.model.NormTableName = responsesData.data.data.NormTableName;
                self.model.TransTypeID = 6;

                self.model.ActgvoucTransItem = responsesData.data.SbpmakeplanTrans;
                _.map(self.model.ActgvoucTransItem, function (value, key) {
                  //value.AutoactName = value.actautoact.AutoactName;
                  value.AutoactID = 36;
                  value.AutoactName = 'Giao dự toán';
