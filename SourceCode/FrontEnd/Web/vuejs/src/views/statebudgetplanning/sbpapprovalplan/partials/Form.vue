                ...
                <b-row class="mb-2">
                    <b-col class="col-md-18">
                        <div class="main-header-item main-header-name">
                            <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Phê duyệt dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                            <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Phê duyệt dự toán<span v-if="model.companyName">:</span> {{model.companyName}}</span>
                        </div>
                    </b-col>
              ...
              <div class="form-group row" style="margin-bottom: 0px !important; margin-top: 10px; margin-right: 0px;">
                    ...
                    <b-tab title="Đơn vị xem xét">
                      <b-card-text>
                        <div class="form-group row align-items-center">
                          ...
                          <div class="col-md-20 col-xl-21">
                            <IjcoreModalSearchCompany
                              v-model="model.ParentCompanyName" :fieldCateList="'28'" :fieldCateValue="[0,1,2]" :title="'Đơn vị xem xét'" :table="'company'" :api="'/listing/api/company/get-cate-value'"
                              :placeholderInput="'Đơn vị xem xét'" @changeCompanyState1="handleGetChangedCompany1">
                            </IjcoreModalSearchCompany>
                          </div>
                        </div>
                        <!--                              <div class="form-group row align-items-center">-->
                ...
                <div class="col-md-10 col-lg-10 w-form-control" style="margin-top: 32px;">
                  ...
                  <div class="form-group row align-items-center">
                    ...
                    <div v-if="model.item_599" class="col-md-12 col-lg-12 col-xl-7">
...
<!--                          <IjcoreModalAutoact-->
<!--                            v-model="model.ActgvoucTransItem[key]" :title="'bút toán'" :api="'/listing/api/common/list'" :FieldWhere="{SysAutoactTypeID : 118}"-->
...
<!--                          </IjcoreModalAutoact>-->
                        ...
                        <!-- Phê duyệt dự toán -->
                        <td>
                          <ijcore-number v-model="model.ActgvoucTransItem[key].FCReviewAmount" @input="addtoFCReviewAmount(model.ActgvoucTransItem[key].FCReviewAmount, key)"></ijcore-number>
                        </td>
                        <td v-if="model.item_tt">
                          <ijcore-number v-model="model.ActgvoucTransItem[key].LCReviewAmount" ></ijcore-number>
                        </td>
                        <!--Tài sản/Vật tư -->
...
<script>
    ...
    const ViewRouter = 'statebudgetplanning-sbpapprovalplan-view';
    ...
    const EditApi = 'state-budget-planning/api/sbpapprovalplan/edit';
    const CreateApi = 'state-budget-planning/api/sbpapprovalplan/create';
    const StoreApi = 'state-budget-planning/api/sbpapprovalplan/store';
    const UpdateApi = 'state-budget-planning/api/sbpapprovalplan/update';
    ...
    export default {
        ...
        methods: {
          ...
          addtoFCReviewAmount(evt, key){
            let FCReviewAmount = this.model.ActgvoucTransItem[key].FCReviewAmount;
            let ExchangeRate = this.model.ActgvoucTransItem[key].ExchangeRate;
            let LCReviewAmount = Number(FCReviewAmount) * Number(ExchangeRate);
            if(LCReviewAmount=='Infinity'){LCReviewAmount='';}
            if(LCReviewAmount){//alert(LCUnitPrice);
              this.model.ActgvoucTransItem[key].LCReviewAmount = LCReviewAmount;
            }else{
              this.model.ActgvoucTransItem[key].LCReviewAmount ='';
            }
            //this.$forceUpdate()
          },
          ...
          selectednormtable(){
              ...
              const requestData = {
                method: 'post',
                url: 'state-budget-planning/api/sbpapprovalplan/getNormTableItem',
                data: {
                  NormTableID: this.model.NormTableID,
                }
              };
          ...
          changeTemplate() {
            ...
            requestData.url = '/state-budget-planning/api/sbpapprovalplan/load-temp/' + this.cloneTransID;
            this.$store.commit('isLoading', true);
            ApiService.setHeader();
            ApiService.customRequest(requestData).then((responses) => {
              let responsesData = responses.data;
              if (responsesData.status === 1) {
                ...
                self.model.NormTableNo = responsesData.data.data.NormTableNo;
                self.model.NormTableName = responsesData.data.data.NormTableName;
                self.model.TransTypeID = 4;

                self.model.ActgvoucTransItem = responsesData.data.SbpmakeplanTrans;
                _.map(self.model.ActgvoucTransItem, function (value, key){
                  //value.AutoactName = value.actautoact.AutoactName;
                  value.AutoactID = 35;
                  value.AutoactName = 'Phê duyệt dự toán';
                  value.FCNormAmount = value.FCDebitAmount;
                  value.LCNormAmount = value.LCDebitAmount;
                  value.FCReviewAmount = value.FCDebitAmount;
                  value.LCReviewAmount = value.LCDebitAmount;
                  self.$set(self.model.ActgvoucTransItem[key], 'Show', true);
                  self.$set(self.model.ActgvoucTransItem[key], 'ShowPagination', false);
                  if (value.LineID > self.RowItem) {
                    self.RowItem = value.LineIDTmp;
