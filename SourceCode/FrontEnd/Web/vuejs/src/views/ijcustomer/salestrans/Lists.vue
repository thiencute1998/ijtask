                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Giao dịch bán hàng</span>
                            </div>
                        </b-col>
                    ...
                    <b-row class="mb-2">
                      <b-col class="col-lg-12 col-md-24 col-sm-24 col-24 mb-2 mb-lg-0 d-lg-flex justify-content-start align-items-center">
                        <div class="main-header-item main-header-actions">
                          <CustomerSalesTransForm v-model="model.CustomerSalesTrans" :title="'Giao dịch bán hàng'" :addnew="true" :CheckCustomerID="true" :Customer="Customer"
                                                   :CustomerSalesTransCate="CustomerSalesTransCate"
                                                  :StatusOption="StatusOption"
                                                  :StatusValueOption="StatusValueOption"
                                                  @on:save-data="fetchData">
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                        <b-table ref="selectableTable"
                           ...
                           @row-clicked="$_lists_onRowClicked_view"
                           @click="$_lists_onRowClicked_view"
...
<!--                       :striped="propsTable.striped"-->
                          ...
                          <template slot="top-row" slot-scope="data">
                            <td :class="[(field.key == 'selected') ? 'pl-3' : '', (field.key == 'ItemName') ? 'pr-3' : '']" role="cell" v-for="(field, key) in data.fields">
                              ...
                              <ijcore-date-picker
                                ...
                                ref="datePicker"
                                @clear-date-picker="clearTransDate()"
                                @change="changerTransDate()"
                                style="width: 141px; margin-top: 3px"
                                :plain="true"
                                :name="field.field"
                                v-model="modelSearch[field.field]">
                              </ijcore-date-picker>
                          ...
                          <template v-slot:cell(TransComment)="data">
                            <span :title="data.item.TransComment">{{data.item.TransComment}}</span>
                          </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.TransDate | convertServerDateToClientDate}}</span>
                                <span class="kanban-no">{{item.EmployeeName}}</span>
                                <span class="kanban-no">{{item.CustomerInfo}}</span>
                                <span class="kanban-no">{{item.TransComment}}</span>
                                <span class="kanban-no">{{item.FCAmount|convertNumberToText}}</span>
                                <span class="kanban-no">{{item.PercentSuccess}}</span>
                                <span class="kanban-no">{{item.StatusDescription}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    import CustomerSalesTransForm from "./../customer/partials/CustomerSalesTransForm";
    ...
    export default {
      ...
      computed: {
          ...
          captions: function() {
              let fieldsTable = [
                  ...
                  {key: 'TransDate', label: 'Ngày giao dịch', field: 'TransDate', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'date'}, tdClass: 'pr-3'},
                  {key: 'EmployeeName', label: 'Nhân viên bán hàng', field: 'EmployeeName', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
                  {key: 'CustomerInfo', label: 'Khách hàng', field: 'CustomerInfo', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
                  {key: 'TransComment', label: 'Nội dung', field: 'TransComment', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
                  {key: 'FCAmount', label: 'Giá trị', field: 'FCAmount', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
                  {key: 'PercentSuccess', label: '% Thành công', field: 'PercentSuccess', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
                  {key: 'StatusDescription', label: 'Trạng thái', field: 'StatusDescription', thStyle: 'min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
              ];
      ...
      created() {
        ...
        this.settings.Table = 'customer_sales_trans';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'customer/api/salestrans';
        this.settings.DeleteApi = 'customer/api/salestrans/delete';
        this.settings.CreateRouter = 'customer-salestrans-create';
        this.settings.EditRouter = 'customer-salestrans-edit';
        this.settings.ViewRouter = 'customer-salestrans-view';
      ...
      methods: {
          ...
          fetchData(){
              ...
              if (this.modelSearch.EmployeeName !== '') {
                requestData.data.EmployeeName = this.modelSearch.EmployeeName;
              }
              if (this.modelSearch.CustomerInfo !== '') {
                requestData.data.CustomerInfo = this.modelSearch.CustomerInfo;
              }
              if (this.modelSearch.TransComment !== '') {
                  requestData.data.TransComment = this.modelSearch.TransComment;
              }
              if (this.modelSearch.FCAmount !== '') {
                requestData.data.FCAmount = this.modelSearch.FCAmount;
              }
              if (this.modelSearch.PercentSuccess !== '') {
                requestData.data.PercentSuccess = this.modelSearch.PercentSuccess;
              }
              if (this.modelSearch.StatusDescription !== '') {
                requestData.data.StatusDescription = this.modelSearch.StatusDescription;
              }
              ...
              ApiService.customRequest(requestData).then((response) => {
                  ...
                  if (dataResponse.status === 1) {
                      self.totalRows = dataResponse.data.data.total;
                      self.perPage = String(dataResponse.data.data.per_page);
                      self.currentPage = dataResponse.data.data.current_page;
                      // converse object to array
                      self.itemsArray = _.toArray(dataResponse.data.data.data);

                      // set params request
                      self.paramsReqRouter.lastPage = dataResponse.data.data.last_page;
                      self.paramsReqRouter.from = dataResponse.data.data.from;
                      self.paramsReqRouter.to = dataResponse.data.data.to;
                      self.$_lists_setParamsReqRouter();
                    //
                    self.CustomerSalesTransCate = dataResponse.data.CustomerSalesTransCate;
                    if (_.isArray(dataResponse.data.Sl_Status)) {
                      self.StatusOption = [];
                      _.forEach(dataResponse.data.Sl_Status, function (status, key) {
                        let tmpObj = {};
                    ...
                    if (dataResponse.data.Sl_StatusItem) {
                      self.StatusValueOption = [];
                      _.forEach(dataResponse.data.Sl_StatusItem, function (statusItem, key) {
                        let tmpObj = {};
          ...
          scrollHandle(evt){},
          clearTransDate() {},
          changerTransDate() {this.fetchData();},
          //
          vew_onToggleModal(){
            this.onToggleModal();
          }
