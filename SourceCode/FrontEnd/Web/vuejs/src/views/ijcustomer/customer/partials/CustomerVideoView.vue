  ...
  <div class="table-responsive">
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        <td class="pr-3">{{item.VideoName}}</td>
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <CustomerVideoEdit :Customer="Customer" v-model="value[key]"></CustomerVideoEdit>
    ...
    <b-modal ref="modal-video" id="modal-form-input-view-video" scrollable no-close-on-backdrop :size="sizeModal">
      <template v-slot:modal-header="{ close }">
        <h5 style="margin-bottom: 0;"><i class="icon-eye icon"></i> Phim</h5>
        <div class="right">
...
<script>
  ...
  import CustomerVideoEdit from "./CustomerVideoEdit";

  export default {
    name: 'CustomerVideoView',
    components: {
      CustomerVideoEdit,
      IjcoreUploadVideo,
    ...
    methods: {
      ...
      downloadVideo(FileID, name) {
        let self = this;
        let urlApi = '/customer/api/customer/download-video/' + FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
      ...
      viewVideo(key) {
        let type = this.value[key].VideoType;
        let href = this.value[key].Link;
        let ValidCustomerTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
        let self = this;
      ...
      deleteLine(key) {
        let self = this;
        let urlApi = '/customer/api/customer/delete-video/' + self.value[key].LineID;
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {},
