  ...
  <div class="table-responsive table-responsive-assign" ref="DivContainerTable">
    ...
    white-space: nowrap;">{{item.TransComment}}</div></td>
        ...
        <td style="text-align: center;width: 50px;right: 8px;" class="right-absolute">
          <CustomerSupportTransForm v-model="value" :keyArray="key" :title="'Giao dịch hỗ trợ'" :Customer="Customer" :StatusOption="StatusOption" :StatusValueOption="StatusValueOption" :CustomerSupportTransCate="CustomerSupportTransCate" :per="per" :addnew="false" :StyleIcon="'position: absolute; left: 2px;'">
...
<script>
  ...
  import CustomerSupportTransForm from "./CustomerSupportTransForm";

  export default {
    name: 'CustomerSupportTransContent',
    components: {
      IjcoreNumber,
      CustomerSupportTransForm
    },
    ...
    methods: {
      ...
      onAddFieldCustomerSupportTransCate() {
        ...
        if(this.value.CustomerSupportTransCate[this.TransItemIDCurrent] == undefined){
          this.value.CustomerSupportTransCate[this.TransItemIDCurrent] = [];
        }
        this.value.CustomerSupportTransCate[this.TransItemIDCurrent].push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldCustomerSupportTransCate(key) {
        this.value.CustomerSupportTransCate[this.TransItemIDCurrent].splice(key, 1);
        //this.setStyleAction();
        this.$forceUpdate();
      },
      ...
      deleteLine(key) {
        ...
        const requestData = {
          method: 'post',
          url: 'customer/api/customer/customer-supporttrans-delete',
          data: {
