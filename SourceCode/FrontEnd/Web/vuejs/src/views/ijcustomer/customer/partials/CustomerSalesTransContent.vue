  ...
  <div class="table-responsive table-responsive-assign" ref="DivContainerTable">
    <table class="not-border" v-if="!isForm" :style="[isDetail ? {width: '1960px'} : '']" style="width: 98.333%">
      ...
      <tr class="text-left">
        ...
        <th class="pr-3" style="width: 136px;">Nhân viên</th>
        <th class="pr-3" style="width: 200px;">Khách hàng</th>
        <th class="pr-3" style="width: 200px;">Nội dung</th>
        <th class="pr-3" v-if="isDetail">Hàng hóa dịch vụ</th>
        <th class="pr-3" v-if="isDetail">Số phút</th>
        <th class="pr-3" v-if="isDetail"><i class="fa fa-paperclip"></i> </th>
        <th class="pr-3" style="width: 120px;">Giá trị (vnđ)</th>
        <th class="pr-3" v-if="isDetail">Ngày chốt</th>
        <th class="pr-3">% TC</th>
    ...
    white-space: nowrap;">{{item.TransComment}}</div></td>
        <td class="pr-3" v-if="isDetail">{{item.ItemName}}</td>
        <td class="pr-3" v-if="isDetail">{{item.Time}}</td>
        <td class="pr-3" v-if="isDetail">{{item.FileName}}</td>
        <td class="pr-3" style="text-align: right">{{item.FCAmount|convertNumberToText}}</td>
        <td class="pr-3" v-if="isDetail">{{item.ExpectedEndDate}}</td>
        <td class="pr-3">{{item.PercentSuccess}}</td>
        ...
        <td style="text-align: center;width: 50px;right: 8px;" class="right-absolute">
          <CustomerSalesTransForm v-model="value" :keyArray="key" :title="'Giao dịch bán hàng'" :Customer="Customer" :StatusOption="StatusOption" :StatusValueOption="StatusValueOption" :CustomerSalesTransCate="CustomerSalesTransCate" :per="per" :addnew="false" :StyleIcon="'position: absolute; left: 2px;'">
...
<script>
  ...
  import CustomerSalesTransForm from "./CustomerSalesTransForm";

  export default {
    name: 'CustomerSalesTransContent',
    components: {
      IjcoreNumber,
      CustomerSalesTransForm
    },
    ...
    methods: {
      ...
      onAddFieldCustomerSalesTransCate() {
        ...
        if(this.value.CustomerSalesTransCate[this.TransItemIDCurrent] == undefined){
          this.value.CustomerSalesTransCate[this.TransItemIDCurrent] = [];
        }
        this.value.CustomerSalesTransCate[this.TransItemIDCurrent].push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldCustomerSalesTransCate(key) {
        this.value.CustomerSalesTransCate[this.TransItemIDCurrent].splice(key, 1);
        //this.setStyleAction();
        this.$forceUpdate();
      },
      ...
      deleteLine(key) {
        ...
        const requestData = {
          method: 'post',
          url: 'customer/api/customer/customer-salestrans-delete',
          data: {
