    ...
    <div class="table-responsive">
      <table class="not-border mb-2">
        ...
        <tr v-for="(field, key) in per">
          ...
          <td class="pr-3">
            <CustomerPerFieldForm :Customer="value" :disable="!per[key].Access" :keyPer="key" :title="'Phân quyền'" field-model="AccessField" :table="'doc'" v-model="field.AccessField" @updateFromChild="updateParent"></CustomerPerFieldForm>
          </td>
          <td class="pr-3 text-center">
          ...
          <td class="pr-3">
            <CustomerPerFieldForm :Customer="value" :disable="!per[key].Edit" :keyPer="key" :title="'Phân quyền'" field-model="EditField" :table="'doc'" v-model="field.EditField" @updateFromChild="updateParent"></CustomerPerFieldForm>
          </td>
          <td class="pr-3 text-center">
...
<!--          </td>-->
      ...
      <IjcoreModalMultiListing v-model="value" @changed="addLine" :title="'nhân viên'"
...
<script>
  ...
  import CustomerPerFieldForm from "./CustomerPerFieldForm";
  import Select2 from 'v-select2-component';
  import IjcoreModalMultiListing from "../../../../components/IjcoreModalMultiListing";

  export default {
    name: 'CustomerPerForm',
    components: {IjcoreModalMultiListing, CustomerPerFieldForm, CustomerPerFieldView, Select2},
    computed: {},
    data() {
      return {
        OptionAccessType: {
    ...
    methods: {
      getListEmployee(TypeSearch){
        let self = this;
        let urlApi = '/doc/api/doc/get-list-employee';
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {
        ...
        ApiService.customRequest(requestData).then((response) => {
          ...
          if (dataResponse.status === 1) {
            _.forEach(dataResponse.data, function (val, key) {
              let indexExist = _.findIndex(self.per, ['EmployeeID', val.EmployeeID]);
              if (indexExist < 0) {
                self.per.push({
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'Task', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .mr-bottom-3 {
