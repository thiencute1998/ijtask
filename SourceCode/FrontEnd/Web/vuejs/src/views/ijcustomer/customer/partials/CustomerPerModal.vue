<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <CustomerPerView v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></CustomerPerView>
      <CustomerPerForm v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption"></CustomerPerForm>
      <template v-slot:modal-footer>
        <div class="w-100 left">
          <b-button variant="primary" size="md" class="float-left mr-2" @click="onEdit()" v-if="!isForm && per.length">
            Sửa
          </b-button>
          <b-button variant="primary" size="md" class="float-left mr-2" @click="onEdit()" v-if="!isForm && !per.length">
            Thêm
...
<script>
  ...
  import CustomerPerView from "./CustomerPerView";
  import CustomerPerForm from "./CustomerPerForm";

  export default {
    name: 'CustomerPerModal',
    components: {CustomerPerForm, CustomerPerView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        let self = this;
        let UpdateApi = 'customer/api/customer/update-per';
        _.forEach(self.per, function (per, key) {
          if (per.Edit) {
            self.per[key].EditField = 'all';
          }
          if (per.Access) {
            self.per[key].AccessField = 'all';
          }
        });
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'Customer', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .mr-bottom-3 {
