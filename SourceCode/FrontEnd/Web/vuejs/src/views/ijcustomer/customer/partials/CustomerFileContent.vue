...
<!--            <i class="cui-cloud-download icons ij-icon" style="font-size: 18px; cursor: pointer;"></i>-->
<!--          </a>-->
<!--          <CustomerFileEdit :Customer="Customer" v-model="value[key]" v-if="per['Edit']" :per="per"></CustomerFileEdit>-->
  ...
  export default {
    ...
    methods: {
      ...
      viewFile(key) {
        ...
        let href = this.$store.state.appRootApi + this.value[key].Link;
      ...
      updateHour(key) {
        if (this.value[key].StartDate && this.value[key].DueDate) {
          let self = this;
          let urlApi = '/customer/api/customer/get-hour';
          let requestData = {
            method: 'post',
            url: urlApi,
            data: {
              ...
              CalendarTypeID: self.Customer.CalendarTypeID,
            },
