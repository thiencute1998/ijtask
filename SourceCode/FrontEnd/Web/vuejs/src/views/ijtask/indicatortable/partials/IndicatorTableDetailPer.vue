    ...
    <b-modal ref="modal" id="modal-form-input-per-module-detail" scrollable size="md">
      ...
      <div class="table-responsive">
        <table>
          ...
          <tr class="text-left">
            <th>
              <b-form-checkbox :disabled="!isForm" v-model="checkAll" @change="checkAllItem(checkAll)">
              </b-form-checkbox>
            </th>
            <th>Tên trường</th>
          </tr>
          ...
          <tr v-for="(item, key) in this.listtable">
            <td class="text-center">
              <b-form-checkbox v-model="item.checked" :disabled="!isForm" @change="checkItem(key, item.checked)">
              </b-form-checkbox>
            </td>
            <td>
      ...
      <template v-slot:modal-footer>
        <div class="w-100 left">
          <b-button variant="primary" size="md" class="float-left mr-2" v-if="isForm" @click="onSaveModal">
            Lưu & Đóng
          </b-button>
          <b-button
            variant="primary"
    ...
    methods: {
      ...
      fetchData() {
        ...
        let requestData = {
          ...
          url: urlApi,
          data: {
            TableName: 'task_indicator_table',
          },
      ...
      onSaveModal() {
        if(this.arrListField){
          this.$emit('updateFromChild', this.keyIndex, this.arrListField.join(','), this.fieldModel);
        }else{
          this.$emit('updateFromChild', this.keyIndex, '', this.fieldModel);
        }
