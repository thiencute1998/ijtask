<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" scrollable id="modal-form-input-task-request" size="lg" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{this.title}}
      </template>
      <TaskRequestContent v-model="value" v-if="!isForm" :Task="Task" :per="per">
      </TaskRequestContent>
      <TaskRequestContent v-model="value" :isForm="true" v-if="isForm" :Task="Task" :per="per">
      </TaskRequestContent>
  ...
  export default {
    ...
    methods: {
      ...
      onToggleModal() {
        ...
        this.fetchData();
        if (!this.$parent.showTaskRequest) this.$parent.showTaskRequest = true;
      },
      onHideModalDataflow() {
        if (this.isDataflow) {
      ...
      onUpdate() {
        ...
        const requestData = {
          method: 'post',
          url: 'task/api/task/task-request/' + this.Task.TaskID,
          data: {
            TaskRequest: self.value
          }
        };
        // edit user
