                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Mẫu bảng chỉ tiêu ĐGCV </span>
                            </div>
                        </b-col>
            ...
            <div class="main-body" :class="(stage.viewType === 'list') ? 'main-body-view-list' : 'main-body-view-kanban'">
                <b-card class="m-0 border-0" body-class="py-0 px-0">
                    <div class="content-body">
                      <div class="content-table content-body-list table-temp" v-if="stage.viewType === 'list'">
                        <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(TableType)="data">
                                <b-dropdown variant="primary" id="dropdown-actions1" @toggle="$_lists_onToggleActionMajor" class="main-header-action mr-2" text="Bảng mẫu">
                                  <b-dropdown-item @click="onBackToList(data.item)" >Mẫu bảng chỉ tiêu ĐGCV </b-dropdown-item>
                                  <b-dropdown-item @click="onBackToList1(data.item)">Bảng mẫu chỉ tiêu ĐGCV – Kết quả then chốt</b-dropdown-item>
                                  <b-dropdown-item @click="onBackToList2(data.item)">Bảng mẫu chỉ tiêu ĐGCV - kết quả phản hồi</b-dropdown-item>
                                </b-dropdown>
                              </template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'TemplateNo', label: 'Mã mẫu', field: 'TemplateNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
                    {key: 'TemplateName', label: 'Tên mẫu', field: 'TemplateName',searchOnTopRow: {type: 'text'}, thStyle: 'min-width: 300px'},
                    {key: 'TableType', label: 'Loại bảng mẫu', field: 'TableType', tdClass: 'pr-5', thClass: 'pr-5', thStyle: 'width: 15%; min-width: 150px'},
                ];
        ...
        created() {
          this.settings.FieldID = 'TemplateID';
          this.settings.Table = 'task_indicator_temp';
          this.settings.FieldInactive = 'Inactive';
          this.settings.ListApi = 'task/api/indicator-temp';
          this.settings.DeleteApi = 'task/api/indicator-temp/delete';
          this.settings.CreateRouter = 'task-indicator-temp-create';
          this.settings.EditRouter = 'task-indicator-temp-edit';
          this.settings.ViewRouter = 'task-indicator-temp-view';
        ...
        methods: {
          ...
          handleDebugger(data){
              console.log(data);indicator-temp
          },
          ...
          fetchData(){
              ...
              if (this.modelSearch.TemplateNo.trim() !== '') {
                  requestData.data.TemplateNo = this.modelSearch.TemplateNo;
              }
              if (this.modelSearch.TemplateName !== '') {
                  requestData.data.TemplateName = this.modelSearch.TemplateName;
              }
          ...
          onBackToList(item) {
            if (item.TemplateID) {
              this.$router.push({name: 'task-indicator-tempitem', params: {id: item.TemplateID}});
            } else {
              this.$router.push({name: 'task-indicator-tempitem'});
            }
          },
          onBackToList1(item) {
            if (_.isString(item.TemplateID)) {
              this.$router.push({name: '', params: {id: item.TemplateID}});
            } else {
...
<style lang="css">
    ...
    .table-temp .table.b-table td,.table-temp .table.b-table td div,.table-temp .table.b-table td span {
        overflow: unset;
    }
</style>
