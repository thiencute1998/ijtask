        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại khách hàng<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại khách hàng<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên khách hàng" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  ...
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại khách hàng cha'"
                  :placeholderSearch="'Nhập loại khách hàng'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.customerCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeCustomerTableItem">
                <tr class="draggable" v-for="(field, key) in model.customerCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.customerCateValue[key].Description"
                      autocomplete="customer cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.customerCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.customerCateValue[key].CateValue"
                      autocomplete="customer cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.customerCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.customerCateValue[key].CateValue"
...
const ListRouter = 'listing-customercatelist';
const EditRouter = 'listing-customercatelist-edit';
const CreateRouter = 'listing-customercatelist-edit';
const ViewRouter = 'listing-customercatelist-view';
const ViewApi = 'listing/api/customer-cate-list/view';
const EditApi = 'listing/api/customer-cate-list/edit';
const CreateApi = 'listing/api/customer-cate-list/create';
const StoreApi = 'listing/api/customer-cate-list/store';
const UpdateApi = 'listing/api/customer-cate-list/update';
const ListApi = 'listing/api/customer-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.customerCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.customerCateValue = responsesData.data.customerCateValue;

              // set max lineID
              _.forEach(self.model.customerCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.customerCateValue[key].CateValue = Number(self.model.customerCateValue[key].CateValue);
                if (field.DataType == 2) self.model.customerCateValue[key].CateValue = String(self.model.customerCateValue[key].CateValue);
                if (field.DataType == 3) self.model.customerCateValue[key].CateValue = moment(self.model.customerCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.customerCateValue[key].CateValue = moment(self.model.customerCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.customerCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.customerCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.customerCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.customerCateValue
        }
      };
