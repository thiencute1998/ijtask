          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Khoản  chi</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <expense-modal-search-link
                    v-model="filter.ExpenseLink"
                ...
                <div class="col-6 mb-2">
                  <expense-modal-search-input-catelist
                    v-model="filter.ExpenseCateList"
                    :listApi="'listing/api/expense/get-expense-cate-list'"
          ...
          <div class="content-body">
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              ...
              >
                ...
                <template v-slot:cell(ExpenseName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>

                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-'+data.item.ExpenseNo"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.ExpenseName}}</span>
                    <span
                ...
                <template v-else>
                  ...
                  <span :title="data.item.ExpenseName">{{data.item.ExpenseName}} </span>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.ExpenseName}}</span>
                      <span class="kanban-no">{{item.ExpenseNo}}</span>
                    </div>
...
import ExpenseModalSearchLink from "./partials/ExpenseModalSearchLink";
import ExpenseModalSearchInputCatelist  from "./partials/ExpenseModalSearchInputCatelist";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        ExpenseLink: (this.$route.query && this.$route.query.ExpenseLink) ? this.$route.query.ExpenseLink : [],
      }
    }
  },
  components:{
    ExpenseModalSearchLink,
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'ExpenseNo', label: 'Mã số', field: 'ExpenseNo', thStyle: 'width: 8%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'ExpenseName', label: 'Tên Khoản  chi', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'ExpenseName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'ExpenseID';
    this.settings.Table = 'expense';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/expense';
    this.settings.DeleteApi = 'listing/api/expense/delete';
    this.settings.GetListChildApi = 'listing/api/expense/get-list-child';
    this.settings.CreateRouter = 'listing-expense-create';
    this.settings.EditRouter = 'listing-expense-edit';
    this.settings.ViewRouter = 'listing-expense-view';

    this.modelSearch = {
      ExpenseNo: (this.$route.query && this.$route.query.ExpenseNo) ? this.$route.query.ExpenseNo : '',
      ExpenseName: (this.$route.query && this.$route.query.ExpenseName) ? this.$route.query.ExpenseName : '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.ExpenseNo.trim() !== '') {
        requestData.data.ExpenseNo = this.modelSearch.ExpenseNo;
      }
      if (this.modelSearch.ExpenseName !== '') {
        requestData.data.ExpenseName = this.modelSearch.ExpenseName;
      }
      ...
      if (this.filter.ExpenseLink.length) {
        let haveLink = false;
        _.forEach(this.filter.ExpenseLink, function (expenseLink, key) {
        ...
        if (haveLink) {
          requestData.data.ExpenseLink = this.filter.ExpenseLink;
        }
      }
    ...
    onToggleChildren($event,item){
      ...
      let allChildExpense = this.getAllChildExpense(item, this.itemsArray);
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          self.$store.commit('isLoading', true);
          self.getListChild(item.ExpenseID);
        }
        _.forEach(allChildExpense, function (expense, key) {
          let _index = _.findIndex(self.itemsArray, ['ExpenseNo', expense.ExpenseNo]);
          if (_index >= 0) {
            if(expense.ParentID == item.ExpenseID){
              self.$set(self.itemsArray[_index], 'hidden', false);
              if(expense.Detail == 0){
                expense.Class = 'fa fa-plus-square-o';
              }
            }
          }
        });
    ...
    getAllChildExpense(item, expenseArr){
      let self = this, listChild = [];
      let allChild = _.filter(expenseArr, ['ParentID', item.ExpenseID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['ExpenseNo'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(expenseArr, ['ParentID', value.ExpenseID]).length) {
            let recursiveArr = self.getAllChildExpense(value, expenseArr);
            recursiveArr = _.orderBy(recursiveArr, ['ExpenseNo', 'asc']);
    ...
    getListChild(ExpenseID){
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.itemsArray, ['ExpenseID', ExpenseID]);
    ...
    haveChildren(item){
      let children = _.filter(this.itemsArray, ['ParentID', item.ExpenseID]);
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.ExpenseNo !== '') {
        request.ExpenseNo = this.modelSearch.ExpenseNo;
      }
      if (this.modelSearch.ExpenseName !== '') {
        request.ExpenseName = this.modelSearch.ExpenseName;
      }
      ...
      if (this.filter.ExpenseLink.length) {
        ...
        if (haveLink) {
          request.ExpenseLink = this.filter.ExpenseLink;
        }
      }
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                let index = _.findIndex(self.itemsArray,['ExpenseID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['ExpenseID',self.itemsArray[index].ParentID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('expense');
      let request = {};
      if (this.modelSearch.ExpenseNo !== '') {
        request.ExpenseNo = this.modelSearch.ExpenseNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/expense/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Expense');
    },
  },
  watch: {
    currentPage() {
