<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-expense-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <ExpenseLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></ExpenseLinkContent>
...
<script>
  ...
  import ExpenseLinkContent from "./ExpenseLinkContent";

  export default {
    name: 'ExpenseLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Expense: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showExpenseLink){
          console.log(this.$parent);
          this.$parent.showExpenseLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/expense/expense-data-list',
          data: {
            ExpenseLink : this.value,
