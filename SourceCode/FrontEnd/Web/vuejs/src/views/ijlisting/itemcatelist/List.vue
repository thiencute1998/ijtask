          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Loại vật tư - hàng hóa - dịch vụ</span>
              </div>
            </b-col>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CateName', label: 'Tên loại vật tư - hàng hóa - dịch vụ', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'item_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/item-cate-list';
    this.settings.DeleteApi = 'listing/api/item-cate-list/delete';
    this.settings.CreateRouter = 'listing-itemcatelist-create';
    this.settings.EditRouter = 'listing-itemcatelist-edit';
    this.settings.ViewRouter = 'listing-itemcatelist-view';
  ...
  methods: {
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/item-cate-list/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: CateID,
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('item-cate-list');
      let request = {};
      if (this.modelSearch.CateID !== '') {
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/item-cate-list/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateItem');
    },
  },
  watch: {
    currentPage() {
