        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại vật tư - hàng hóa - dịch vụ<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại vật tư - hàng hóa - dịch vụ<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên vật tư - hàng hóa - dịch vụ" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại vật tư - hàng hóa - dịch vụ'"
                  :api="'/listing/api/common/listCate'"
                  :table="'item'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại vật tư - hàng hóa - dịch vụ cha'"
                  :placeholderSearch="'Nhập loại vật tư - hàng hóa - dịch vụ'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.itemCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeItemTableItem">
                <tr class="draggable" v-for="(field, key) in model.itemCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.itemCateValue[key].Description"
                      autocomplete="item cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.itemCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.itemCateValue[key].CateValue"
                      autocomplete="item cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.itemCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.itemCateValue[key].CateValue"
...
const ListRouter = 'listing-itemcatelist';
const EditRouter = 'listing-itemcatelist-edit';
const CreateRouter = 'listing-itemcatelist-edit';
const ViewRouter = 'listing-itemcatelist-view';
const ViewApi = 'listing/api/item-cate-list/view';
const EditApi = 'listing/api/item-cate-list/edit';
const CreateApi = 'listing/api/item-cate-list/create';
const StoreApi = 'listing/api/item-cate-list/store';
const UpdateApi = 'listing/api/item-cate-list/update';
const ListApi = 'listing/api/item-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.itemCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.itemCateValue = responsesData.data.itemCateValue;

              // set max lineID
              _.forEach(self.model.itemCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.itemCateValue[key].CateValue = Number(self.model.itemCateValue[key].CateValue);
                if (field.DataType == 2) self.model.itemCateValue[key].CateValue = String(self.model.itemCateValue[key].CateValue);
                if (field.DataType == 3) self.model.itemCateValue[key].CateValue = moment(self.model.itemCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.itemCateValue[key].CateValue = moment(self.model.itemCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.itemCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.itemCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.itemCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.itemCateValue
        }
      };
