<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <sbr-sector-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></sbr-sector-per-view>
      <sbr-sector-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></sbr-sector-per-form>
...
import SbrSectorPerView from "./SbrSectorPerView";
import SbrSectorPerForm from "./SbrSectorPerForm";

export default {
  name: 'sbr-sector-per-modal',
  components: {SbrSectorPerForm, SbrSectorPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/sbr-sector/update-per';
      ...
      const requestData = {
        ...
        data: {
          ...
          SbrSectorPer: self.per
        }
      };
      // edit user
      requestData.url = UpdateApi + '/' + self.value.SbrSectorID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'SbrSector', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
