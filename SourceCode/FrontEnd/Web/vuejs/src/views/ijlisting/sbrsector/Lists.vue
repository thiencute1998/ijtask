          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Lĩnh vực thu</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <sbr-sector-modal-search-link
                    v-model="filter.SbrSectorLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(SbrSectorName)="data">
                  <span :title="data.item.SbrSectorName">{{data.item.SbrSectorName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.SbrSectorName}}</span>
                      <span class="kanban-no">{{item.SbrSectorNo}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
import SbrSectorModalSearchLink from "./partials/SbrSectorModalSearchLink";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        SbrSectorLink: (this.$route.query && this.$route.query.SbrSectorLink) ? this.$route.query.SbrSectorLink : [],
      }
    }
  },
  components:{
    SbrSectorModalSearchLink,
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'SbrSectorNo', label: 'Mã số', field: 'SbrSectorNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'SbrSectorName', label: 'Tên lĩnh vực thu', thStyle: 'min-width: 300px', field: 'SbrSectorName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'SbrSectorID';
    this.settings.Table = 'sbr_sector';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/sbr-sector';
    this.settings.DeleteApi = 'listing/api/sbr-sector/delete';
    this.settings.CreateRouter = 'listing-sbr-sector-create';
    this.settings.EditRouter = 'listing-sbr-sector-edit';
    this.settings.ViewRouter = 'listing-sbr-sector-view';

    this.modelSearch = {
      SbrSectorNo : (this.$route.query && this.$route.query.SbrSectorNo) ? this.$route.query.SbrSectorNo : '',
      SbrSectorName: (this.$route.query && this.$route.query.SbrSectorName) ? this.$route.query.SbrSectorName : '',
    };
    this.init();
  },
  updated() {
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.SbrSectorNo.trim() !== '') {
        requestData.data.SbrSectorNo = this.modelSearch.SbrSectorNo;
      }
      if (this.modelSearch.SbrSectorName !== '') {
        requestData.data.SbrSectorName = this.modelSearch.SbrSectorName;
      }
      ...
      if (this.filter.SbrSectorLink.length) {
        let haveLink = false;
        _.forEach(this.filter.SbrSectorLink, function (sbrSectorLink, key) {
        ...
        if (haveLink) {
          requestData.data.SbrSectorLink = this.filter.SbrSectorLink;
        }
      }
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.SbrSectorNo !== '') {
        request.SbrSectorNo = this.modelSearch.SbrSectorNo;
      }
      if (this.modelSearch.SbrSectorName !== '') {
        request.SbrSectorName = this.modelSearch.SbrSectorName;
      }
      ...
      if (this.filter.SbrSectorLink.length) {
        ...
        if (haveLink) {
          request.SbrSectorLink = this.filter.SbrSectorLink;
        }
      }
      ...
      this.$router.push({
        name: 'listing-sbr-sector-report',
        query: request
      });
    },
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('sbr-sector');
      let request = {};
      if (this.modelSearch.SbrSectorNo !== '') {
        request.SbrSectorNo = this.modelSearch.SbrSectorNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/sbr-sector/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'SbrSector');
    },
  },
  watch: {
    currentPage() {
