<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-sbr-sector-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <SbrSectorLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></SbrSectorLinkContent>
...
<script>
  ...
  import SbrSectorLinkContent from "./SbrSectorLinkContent";

  export default {
    name: 'SbrSectorLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      SbrSector: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showSbrSectorLink){
          console.log(this.$parent);
          this.$parent.showSbrSectorLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/sbr-sector/sbr-sector-data-list',
          data: {
            SbrSectorLink : this.value,
            SbrSectorID: this.SbrSector.SbrSectorID
          }
