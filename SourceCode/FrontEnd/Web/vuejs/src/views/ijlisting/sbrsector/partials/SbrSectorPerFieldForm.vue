    ...
    <b-modal ref="modal" id="modal-form-input-per-module-detail" scrollable size="md">
      ...
      <div class="table-responsive">
        <table>
          ...
          <tr class="text-left">
            <th>
              <b-form-checkbox v-model="checkAll" @change="checkAllItem(checkAll)">
              </b-form-checkbox>
            </th>
            <th>Tên trường</th>
          </tr>
          ...
          <tr v-for="(item, key) in this.listtable">
            <td class="text-center">
              <b-form-checkbox v-model="item.checked" @change="checkItem(key, item.checked)">
              </b-form-checkbox>
            </td>
            <td>
      ...
      <template v-slot:modal-footer>
        <div class="w-100 left">
          <b-button variant="primary" size="md" class="float-left mr-2" @click="onSaveModal">
            Lưu & Đóng
          </b-button>
          <b-button
            variant="primary"
...
export default {
  ...
  methods: {
    ...
    checkItem(key, checked){
      if(checked){
        this.checkAll = false;
        let index = this.arrListField.indexOf(this.listtable[key].FieldName);
    ...
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (dataResponse.status === 1) {
          // self.listtable = _.toArray(dataResponse.data);
    ...
    onSaveModal() {
      if(this.arrListField){
        this.$emit('updateFromChild', this.arrListField.join(','), this.fieldModel, this.keyPer);
      }else{
        this.$emit('updateFromChild', '', this.fieldModel, this.keyPer);
      }
