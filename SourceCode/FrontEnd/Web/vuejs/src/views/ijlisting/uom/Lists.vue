                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Đơn vị tính</span>
                            </div>
                        </b-col>
...
<!--                                :striped="propsTable.striped"-->
                          ...
                          <template v-slot:cell(UomName)="data">
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.UomName}}</span>
                                <span class="kanban-no">{{item.UomNo}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    export default {
      ...
      computed: {
          ...
          captions: function() {
              let fieldsTable = [
                  ...
                  {key: 'UomNo', label: 'Mã đơn vị tính', field: 'UomNo', thStyle: 'width: 15%; min-width: 150px', searchOnTopRow: {type: 'text'}},
                  {key: 'UomName', label: 'Tên đơn vị tính', field: 'UomName', thStyle: 'min-width: 300px', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
              ];
      ...
      created() {
        this.settings.FieldID = 'UomID';
        this.settings.Table = 'uom';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'listing/api/uom';
        this.settings.DeleteApi = 'listing/api/uom/delete';
        this.settings.CreateRouter = 'listing-uom-create';
        this.settings.EditRouter = 'listing-uom-edit';
        this.settings.ViewRouter = 'listing-uom-view';
      ...
      methods: {
          ...
          fetchData(){
              ...
              if (this.modelSearch.UomNo.trim() !== '') {
                  requestData.data.UomNo = this.modelSearch.UomNo;
              }
              if (this.modelSearch.UomName !== '') {
                  requestData.data.UomName = this.modelSearch.UomName;
              }
        ...
        handleExportPrint() {
          ...
          let request = {};
          if (this.modelSearch.UomNo !== '') {
            request.UomNo = this.modelSearch.UomNo;
          }
          if (this.modelSearch.UomName !== '') {
