<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-anu-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-anu-per-view>
      <coa-anu-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaAnuOption="CoaAnuOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-anu-per-form>
...
import CoaAnuPerView from "./CoaAnuPerView";
import CoaAnuPerForm from "./CoaAnuPerForm";

export default {
  name: 'coa-anu-per-modal',
  components: {CoaAnuPerForm, CoaAnuPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-anu/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaAnu', 'isDetail', 'per', 'EmployeeOption', 'CoaAnuOption', 'GroupOption'],
}
</script>
<style>
.readonly {
