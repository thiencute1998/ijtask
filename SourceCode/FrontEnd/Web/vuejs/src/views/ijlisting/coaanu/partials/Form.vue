        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Hệ thống tài khoản hành chính sự nghiệp<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Hệ thống tài khoản hành chính sự nghiệp<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.AccountName" type="text" id="AccountName" class="form-control" placeholder="Tên hệ thống tài khoản hành chính sự nghiệp" name="AccountName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParent
                  v-model="model"
                  :title="'Hệ thống tài khoản hành chính sự nghiệp'"
                  :api="'/listing/api/common/get-parent'"
                  :table="'coa_anu'"
                  :fieldName="'AccountName'"
                  :fieldNo="'AccountNo'"
                  :fieldID="'AccountID'"
                  :placeholderInput="'Chọn hệ thống tài khoản  hành chính sự nghiệp cha'"
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-21">
                <CoaAnuModalSearchInputCatelist
                  v-model="model.CoaAnuCate"
                  title-modal="Loại HTTKHN"
                  placeholder="Loại hệ thống tài khoản  hành chính sự nghiệp"
                ></CoaAnuModalSearchInputCatelist>
              </div>
            </div>
            <div class="form-group row align-items-center">
...
import CoaAnuModalSearchInputCatelist from "@/views/ijlisting/coaanu/partials/CoaAnuModalSearchInputCatelist";
import IjcoreModalSystemParent from "../../../../components/IjcoreModalSystemParent";
import CoaAnuModalSearchEmployee from './CoaAnuModalSearchEmployee';

const ListRouter = 'listing-coa-anu';
const EditRouter = 'listing-coa-anu-edit';
const ViewRouter = 'listing-coa-anu-view';
const CreateRouter = 'listing-coa-anu-create';
const ViewApi = 'listing/api/coa-anu/view';
const EditApi = 'listing/api/coa-anu/edit';
const CreateApi = 'listing/api/coa-anu/create';
const StoreApi = 'listing/api/coa-anu/store';
    ...
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          if (_.isArray(responsesData.data.coaAnu)){

            self.model.CoaAnuOption = [];
            _.forEach(responsesData.data.coaAnu, function (value, key) {
              let tmpObj = {};
              tmpObj.id = value.AccountID;
              tmpObj.text = value.AccountName;
              self.model.CoaAnuOption.push(tmpObj);
            });
          }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        data: {
          ...
          CoaAnuCate: this.model.CoaAnuCate,
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          ...
          let item = {
            ...
            CoaAnuCate: self.model.CoaAnuCate,
          }
