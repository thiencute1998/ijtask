<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-anu-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaAnuLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaAnuLinkContent>
...
<script>
  ...
  import CoaAnuLinkContent from "./CoaAnuLinkContent";

  export default {
    name: 'CoaAnuLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaAnu: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaAnuLink){
          console.log(this.$parent);
          this.$parent.showCoaAnuLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-anu/coa-anu-data-list',
          data: {
            CoaAnuLink : this.value,
            AccountID: this.CoaAnu.AccountID
          }
