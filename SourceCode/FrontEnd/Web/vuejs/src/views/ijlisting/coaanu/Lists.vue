          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Hệ thống tài khoản hành chính sự nghiệp</span>
              </div>
            </b-col>
            ...
            <b-col class="col-lg-12 col-md-24 col-sm-24 col-24 mb-2 mb-lg-0 d-lg-flex justify-content-start align-items-center">
              <div class="main-header-item main-header-actions">
                ...
                <b-dropdown variant="primary" id="dropdown-actions" @toggle="$_lists_onToggleActionMajor" class="main-header-action mr-2" text="Thao tác">
                  ...
                  <b-dropdown-item @click="openReport">In</b-dropdown-item>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <coa-anu-modal-search-link
                    v-model="filter.CoaAnuLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              ...
              >
                ...
                <template v-slot:cell(AccountName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>

                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-'+data.item.AccountNo"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.AccountName}}</span>
                    <span
                ...
                <template v-else>
                  ...
                  <span :title="data.item.AccountName">{{data.item.AccountName}} </span>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.AccountName}}</span>
                      <span class="kanban-no">{{item.AccountNo}}</span>
                    </div>
...
import CoaAnuModalSearchLink from "./partials/CoaAnuModalSearchLink";
import CoaAnuModalSearchEmployee from "./partials/CoaAnuModalSearchEmployee";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        CoaAnuLink: (this.$route.query && this.$route.query.CoaAnuLink) ? this.$route.query.CoaAnuLink : [],
      }
    }
  },
  components:{
    CoaAnuModalSearchLink,
    CoaAnuModalSearchEmployee
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'AccountNo', label: 'Mã số', field: 'AccountNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'AccountName', label: 'Tên Hệ thống tài khoản hành chính sự nghiệp', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'AccountName',searchOnTopRow: {type: 'text'}},
      ];
      return fieldsTable;
    }
  },
  created() {
    ...
    this.settings.FieldID = 'AccountID';
    this.settings.Table = 'coa_anu';
    this.settings.FieldInactive = 'Inactive';
    this.settings.ListApi = 'listing/api/coa-anu';
    this.settings.DeleteApi = 'listing/api/coa-anu/delete';
    this.settings.GetListChildApi = 'listing/api/coa-anu/get-list-child';
    this.settings.CreateRouter = 'listing-coa-anu-create';
    this.settings.EditRouter = 'listing-coa-anu-edit';
    this.settings.ViewRouter = 'listing-coa-anu-view';

    this.modelSearch = {
      AccountNo: (this.$route.query && this.$route.query.AccountNo) ? this.$route.query.AccountNo : '',
      AccountName: (this.$route.query && this.$route.query.AccountName) ? this.$route.query.AccountName : '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.AccountNo.trim() !== '') {
        requestData.data.AccountNo = this.modelSearch.AccountNo;
      }
      if (this.modelSearch.AccountName !== '') {
        requestData.data.AccountName = this.modelSearch.AccountName;
      }
      ...
      if (this.filter.CoaAnuLink.length) {
        let haveLink = false;
        _.forEach(this.filter.CoaAnuLink, function (coaAnuLink, key) {
        ...
        if (haveLink) {
          requestData.data.CoaAnuLink = this.filter.CoaAnuLink;
        }
      }
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                ...
                let index = _.findIndex(self.itemsArray,['AccountID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['AccountID',self.itemsArray[index].ParentID]);
    ...
    onToggleChildren($event,item){
      ...
      let allChildCoaAnu= this.getAllChildCoaAnu(item, this.itemsArray);
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          self.$store.commit('isLoading', true);
          self.getListChild(item.AccountID);
        }
        _.forEach(allChildCoaAnu, function (coaAnu, key) {

          let _index = _.findIndex(self.itemsArray, ['AccountNo', coaAnu.AccountNo]);
          if (_index >= 0) {
            if(coaAnu.ParentID == item.AccountID){
              self.$set(self.itemsArray[_index], 'hidden', false);
              if(coaAnu.Detail == 0){
                coaAnu.Class = 'fa fa-plus-square-o';
              }
            }
          }
        });
    ...
    getAllChildCoaAnu(item, coaanuArr){
      let self = this, listChild = [];
      let allChild = _.filter(coaanuArr, ['ParentID', item.AccountID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(coaanuArr, ['ParentID', value.AccountID]).length) {
            let recursiveArr = self.getAllChildCoaAnu(value, coaanuArr);
            recursiveArr = _.orderBy(recursiveArr, ['AccountID', 'asc']);
    ...
    getListChild(AccountID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/coa-anu/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: AccountID,
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.itemsArray, ['AccountID', AccountID]);
    ...
    haveChildren(item){
      let children = _.filter(this.itemsArray, ['ParentID', item.AccountID]);
    ...
    openReport(){
      this.$router.push({
        name: 'listing-coa-anu-report-list'
      });
    },
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('coa-anu');
      let request = {};
      if (this.modelSearch.AccountNo !== '') {
        request.AccountNo = this.modelSearch.AccountNo;
      }
      if (this.modelSearch.AccountName !== '') {
        request.AccountName = this.modelSearch.AccountName;
      }
      ...
      if (this.filter.CoaAnuLink.length) {
        ...
        if (haveLink) {
          request.CoaAnuLink = this.filter.CoaAnuLink;
        }
      }
      request.exportData = true;

      let reportData = await this.$_lists_handleReportResponse('listing/api/coa-anu/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CoaAnu');
    },
  },
  watch: {
    currentPage() {
