<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-sia-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-sia-per-view>
      <coa-sia-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaSiaOption="CoaSiaOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-sia-per-form>
...
import CoaSiaPerView from "./CoaSiaPerView";
import CoaSiaPerForm from "./CoaSiaPerForm";

export default {
  name: 'coa-sia-per-modal',
  components: {CoaSiaPerForm, CoaSiaPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-sia/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaSia', 'isDetail', 'per', 'EmployeeOption', 'CoaSiaOption', 'GroupOption'],
}
</script>
<style>
.readonly {
