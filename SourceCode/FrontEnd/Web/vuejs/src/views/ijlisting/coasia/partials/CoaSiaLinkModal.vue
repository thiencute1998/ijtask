<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-sia-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaSiaLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaSiaLinkContent>
...
<script>
  ...
  import CoaSiaLinkContent from "./CoaSiaLinkContent";

  export default {
    name: 'CoaSiaLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaSia: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaSiaLink){
          console.log(this.$parent);
          this.$parent.showCoaSiaLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-sia/coa-sia-data-list',
          data: {
            CoaSiaLink : this.value,
            AccountID: this.CoaSia.AccountID
          }
