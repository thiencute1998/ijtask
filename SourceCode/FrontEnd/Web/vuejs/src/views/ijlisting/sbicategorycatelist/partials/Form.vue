        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại -loại khoản<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại -loại khoản<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên -loại khoản" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại -loại khoản'"
                  :api="'/listing/api/common/listCate'"
                  :table="'sbi_category'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại -loại khoản cha'"
                  :placeholderSearch="'Nhập loại -loại khoản'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.sbiCategoryCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeSbiCategoryTableItem">
                <tr class="draggable" v-for="(field, key) in model.sbiCategoryCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.sbiCategoryCateValue[key].Description"
                      autocomplete="sbiCategory cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.sbiCategoryCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.sbiCategoryCateValue[key].CateValue"
                      autocomplete="sbiCategory cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.sbiCategoryCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.sbiCategoryCateValue[key].CateValue"
...
const ListRouter = 'listing-sbicategorycatelist';
const EditRouter = 'listing-sbicategorycatelist-edit';
const CreateRouter = 'listing-sbicategorycatelist-edit';
const ViewRouter = 'listing-sbicategorycatelist-view';
const ViewApi = 'listing/api/sbi-category-cate-list/view';
const EditApi = 'listing/api/sbi-category-cate-list/edit';
const CreateApi = 'listing/api/sbi-category-cate-list/create';
const StoreApi = 'listing/api/sbi-category-cate-list/store';
const UpdateApi = 'listing/api/sbi-category-cate-list/update';
const ListApi = 'listing/api/sbi-category-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.sbiCategoryCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.sbiCategoryCateValue = responsesData.data.sbiCategoryCateValue;

              // set max lineID
              _.forEach(self.model.sbiCategoryCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.sbiCategoryCateValue[key].CateValue = Number(self.model.sbiCategoryCateValue[key].CateValue);
                if (field.DataType == 2) self.model.sbiCategoryCateValue[key].CateValue = String(self.model.sbiCategoryCateValue[key].CateValue);
                if (field.DataType == 3) self.model.sbiCategoryCateValue[key].CateValue = moment(self.model.sbiCategoryCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.sbiCategoryCateValue[key].CateValue = moment(self.model.sbiCategoryCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.sbiCategoryCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.sbiCategoryCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.sbiCategoryCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.sbiCategoryCateValue
        }
      };
    ...
    changeSbiCategoryTableItem(moved) {},
  },
  watch: {
    idParams() {
      this.fetchData();
