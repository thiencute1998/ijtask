<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-sbi-category-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <SbiCategoryLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></SbiCategoryLinkContent>
...
<script>
  ...
  import SbiCategoryLinkContent from "./SbiCategoryLinkContent";

  export default {
    name: 'SbiCategoryLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      SbiCategory: {}
    ...
    methods: {
      fetchData(){
        ...
        .then(response=> {
          ...
          _.forEach(responseData.data, (value, key)=> {
            if (value.TableName !== 'sbi_category') {
              let tmpObj = {};
      ...
      onToggleModal(){
        ...
        if(this.$parent.showSbiCategoryLink){
          console.log(this.$parent);
          this.$parent.showSbiCategoryLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/sbi-category/sbi-category-data-list',
          data: {
            SbiCategoryLink : this.value,
            SbiCategoryID: this.SbiCategory.SbiCategoryID
          }
