<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    >
      ...
      <div class="ijcore-modal-sbi-category-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
            <tr>
              <th class="pr-3">Loại - Loại khoản</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại - loại khoản'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'sbi-category-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/sbi-category/get-sbi-category-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.SbiCategoryCateList;
        _.forEach(self.CateList,(value,key)=> {
            let tmpObject = {};
            tmpObject.id = value.CateID;
            tmpObject.text = value.CateName;
            self.CateListOptions.push(tmpObject);
        })

        self.CateValue = responsesData.data.SbiCategoryCateValue;
        _.forEach(self.CateValue, (value,key)=> {
           let tmpObject = {};
           tmpObject.id = value.CateValue;
           tmpObject.text = value.Description;
...
<style lang="css">
  ...
  .ijcore-modal-sbi-category-ccatelist .select2.select2-container{
    width: 100%;
  }
