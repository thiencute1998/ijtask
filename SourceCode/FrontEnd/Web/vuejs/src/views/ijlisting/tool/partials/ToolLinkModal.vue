<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" scrollable id="modal-form-input-tool-link" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{title}}
      </template>
      <tool-link-content v-model="value" v-if="!isForm" :Tool="Tool"></tool-link-content>
      <tool-link-content v-model="value" :isForm="true" v-if="isForm" :Tool="Tool" :SysTable="SysTable"></tool-link-content>
...
<script>
  ...
  import ToolLinkContent from "./ToolLinkContent";

  export default {
    name: 'tool-link-modal',
    components: {
      ToolLinkContent
    },
    computed: {
    ...
    methods: {
      ...
      onToggleModal() {
        ...
        // this.$parent.onToggleToolLink(false);
        this.currentPage = 1;
        this.$refs['modal'].show();
        if (!this.$parent.showToolLink) this.$parent.showToolLink = true;
      },
      onHideModalDataflow() {
        if (this.isDataflow) {
          this.$emit('onHideModalTool');
        }
      },
      onResetModal() {
      },
      ...
      onUpdate() {
        ...
        const requestData = {
          method: 'post',
          url: 'listing/api/tool/tool-data-list',
          data: {
            ToolLink: self.value,
        ...
        // edit user
        requestData.data.ItemID = this.Tool.ToolID;
