  ...
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-task-file-edit" size="lg" no-close-on-backdrop>
      <template slot="modal-title">
        <i class="fa fa-edit"></i> Tệp: {{Tool.ToolName}}
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      onUpdate() {
        ...
        formData.append('ToolID', self.value.ToolID);
        ...
        // send upload request
        axios.post('listing/api/vendor/vendor-upload-video/' + self.Tool.ToolID, formData, config)
