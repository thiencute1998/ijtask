    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <tool-video-edit :Invest="Invest" v-model="value[key]"></tool-video-edit>
...
<script>
  ...
  import ToolVideoEdit from "./ToolVideoEdit";

  export default {
    name: 'tool-video-view',
    components: {
      ToolVideoEdit,
      IjcoreUploadVideo,
    ...
    methods: {
      ...
      downloadVideo(FileID, name) {
        let self = this;
        let urlApi = '/listing/api/tool/download-video/' + FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
      ...
      viewVideo(key) {
        ...
        let self = this;
        let urlApi = '/listing/api/tool/download-video/' + this.value[key].FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
      ...
      deleteLine(key) {
        let self = this;
        let urlApi = '/listing/api/tool/delete-video/' + self.value[key].LineID;
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {},
