<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-tool-general" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{this.title}}
      </template>
      <tool-general-view v-model="value" v-if="!isForm" :isDetail="true"></tool-general-view>
      <tool-general-form v-model="value" v-if="isForm"></tool-general-form>
...
<script>
  ...
  import ToolGeneralView from "./ToolGeneralView";
  import ToolGeneralForm from "./ToolGeneralForm";

  export default {
    name: 'tool-general-modal',
    components: {ToolGeneralForm, ToolGeneralView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/tool/update';
        const requestData = {
          method: 'post',
          url: UpdateApi,
          data: {
            ToolNo: this.value.ToolNo,
            ToolName: this.value.ToolName,
        ...
        // edit user
        requestData.url = UpdateApi + '/' + self.value.ToolID;

        if (this.value.ToolCate && this.value.ToolCate.length) {
          requestData.data.ToolCate = this.value.ToolCate;
        }
