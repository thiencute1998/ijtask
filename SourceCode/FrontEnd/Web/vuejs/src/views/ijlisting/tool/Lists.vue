                  ...
                  <b-row class="mb-2">
                    <b-col class="col-md-12">
                        <div class="main-header-item main-header-name">
                            <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Công cụ dụng cụ</span>
                        </div>
                    </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <tool-modal-search-link
                            v-model="filter.ToolLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(ToolName)="data">
                                <span :title="data.item.ToolName">{{data.item.ToolName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record">
                                <span>Mã số: </span>
                                <span class="kanban-text">{{item.ToolNo}}</span>
                              </div>
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.ToolName}}</span>
                              </div>
                              <div class="kanban-record">
...
<script>
    ...
    import ToolModalSearchLink from "./partials/ToolModalSearchLink";
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
          return {
            filter: {
              ...
              ToolLink: (this.$route.query && this.$route.query.ToolLink) ? this.$route.query.ToolLink : [],
            }
          }
        },
        components:{
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'ToolNo', label: 'Mã số', thStyle: 'width: 6%; min-width: 100px', field: 'ToolNo',searchOnTopRow: {type: 'text'}},
                    {key: 'ToolName', label: 'Tên công cụ dụng cụ', thStyle: 'width: 40%; min-width: 300px', field: 'ToolName',searchOnTopRow: {type: 'text'}},
        ...
        created() {
          this.settings.FieldID = 'ToolID';
          this.settings.Table = 'tool';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/tool';
          this.settings.DeleteApi = 'listing/api/tool/delete' + '';
          this.settings.CreateRouter = 'listing-tool-create';
          this.settings.EditRouter = 'listing-tool-edit';
          this.settings.ViewRouter = 'listing-tool-view';
          this.modelSearch = {
            ToolNo: (this.$route.query && this.$route.query.ToolNo) ? this.$route.query.ToolNo : '',
            ToolName: (this.$route.query && this.$route.query.ToolName) ? this.$route.query.ToolName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.ToolNo !== '') {
                requestData.data.ToolNo = this.modelSearch.ToolNo;
            }
            if (this.modelSearch.ToolName !== '') {
                requestData.data.ToolName = this.modelSearch.ToolName;
            }
            ...
            if (this.filter.ToolLink.length) {
              let haveLink = false;
              _.forEach(this.filter.ToolLink, function (investAssetLink, key) {
              ...
              if (haveLink) {
                requestData.data.ToolLink = this.filter.ToolLink;
              }
            }
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('tool');
            let request = {};
            if (this.modelSearch.ToolNo !== '') {
              request.ToolNo = this.modelSearch.ToolNo;
            }
            if (this.modelSearch.ToolName !== '') {
              request.ToolName = this.modelSearch.ToolName;
            }
            if (this.modelSearch.officePhone !== '') {
            ...
            if (this.filter.ToolLink.length) {
              ...
              if (haveLink) {
                request.ToolLink = this.filter.ToolLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/tool/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'tool');
          },
