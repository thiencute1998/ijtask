        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại lĩnh vực chi<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại lĩnh vực chi<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên lĩnh vực chi" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại lĩnh vực chi'"
                  :api="'/listing/api/common/listCate'"
                  :table="'sector'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại lĩnh vực chi cha'"
                  :placeholderSearch="'Nhập loại lĩnh vực chi'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.sectorCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeSectorTableItem">
                <tr class="draggable" v-for="(field, key) in model.sectorCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.sectorCateValue[key].Description"
                      autocomplete="sector cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.sectorCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.sectorCateValue[key].CateValue"
                      autocomplete="sector cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.sectorCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.sectorCateValue[key].CateValue"
...
const ListRouter = 'listing-sectorcatelist';
const EditRouter = 'listing-sectorcatelist-edit';
const CreateRouter = 'listing-sectorcatelist-edit';
const ViewRouter = 'listing-sectorcatelist-view';
const ViewApi = 'listing/api/sector-cate-list/view';
const EditApi = 'listing/api/sector-cate-list/edit';
const CreateApi = 'listing/api/sector-cate-list/create';
const StoreApi = 'listing/api/sector-cate-list/store';
const UpdateApi = 'listing/api/sector-cate-list/update';
const ListApi = 'listing/api/sector-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.sectorCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.sectorCateValue = responsesData.data.sectorCateValue;

              // set max lineID
              _.forEach(self.model.sectorCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.sectorCateValue[key].CateValue = Number(self.model.sectorCateValue[key].CateValue);
                if (field.DataType == 2) self.model.sectorCateValue[key].CateValue = String(self.model.sectorCateValue[key].CateValue);
                if (field.DataType == 3) self.model.sectorCateValue[key].CateValue = moment(self.model.sectorCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.sectorCateValue[key].CateValue = moment(self.model.sectorCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.sectorCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.sectorCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.sectorCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.sectorCateValue
        }
      };
