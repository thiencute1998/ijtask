                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Đối tượng khác</span>
                            </div>
                        </b-col>
                      ...
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'" >
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(ObjectName)="data">
                                <span :title="data.item.ObjectName">{{data.item.ObjectName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.ObjectName}}</span>
                                <span class="kanban-no">{{item.ObjectNo}}</span>
                              </div>
                              <div class="kanban-record">
                                <span class="kanban-text" v-if="item.Address && item.Address !== ''">Địa chỉ: {{item.Address}}</span>
                              </div>
                              <div class="kanban-record">
...
<script>
    ...
    export default {
      ...
      mixins: [mixinLists],
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'ObjectNo', label: 'Mã đối tượng', thStyle: 'width: 12%; min-width: 100px',field: 'ObjectNo',searchOnTopRow: {type: 'text'}},
                    {key: 'ObjectName', label: 'Tên đối tượng',thStyle: 'width: 30%; min-width: 300px', field: 'ObjectName',searchOnTopRow: {type: 'text'}},
        ...
        created() {
          this.settings.FieldID = 'ObjectID';
          this.settings.Table = 'object';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/object';
          this.settings.DeleteApi = 'listing/api/object/delete';
          this.settings.CreateRouter = 'listing-object-create';
          this.settings.EditRouter = 'listing-object-edit';
          this.settings.ViewRouter = 'listing-object-view';
        ...
        methods: {
            ...
            fetchData(){
                ...
                if (this.modelSearch.ObjectNo !== '') {
                    requestData.data.ObjectNo = this.modelSearch.ObjectNo;
                }
                if (this.modelSearch.ObjectName !== '') {
                    requestData.data.ObjectName = this.modelSearch.ObjectName;
                }
