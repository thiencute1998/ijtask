<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-project-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <ProjectLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></ProjectLinkContent>
  ...
  export default {
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showProjectLink){
          console.log(this.$parent);
          this.$parent.showProjectLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/project/project-data-list',
          data: {
            ProjectLink : this.value,
