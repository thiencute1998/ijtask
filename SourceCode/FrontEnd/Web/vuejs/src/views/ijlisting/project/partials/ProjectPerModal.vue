<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <project-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></project-per-view>
      <project-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></project-per-form>
...
import ProjectPerView from "./ProjectPerView";
import ProjectPerForm from "./ProjectPerForm";

export default {
  name: 'project-per-modal',
  components: {ProjectPerForm, ProjectPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/project/update-per';
      ...
      // edit user
      requestData.url = UpdateApi + '/' + self.value.ProjectID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Project', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
