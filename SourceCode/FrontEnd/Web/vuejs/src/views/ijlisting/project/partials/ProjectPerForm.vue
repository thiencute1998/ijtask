    ...
    <div class="table-responsive">
      <table class="not-border mb-2">
        ...
        <tr v-for="(field, key) in per" v-if="field.EmployeeID != EmployeeLogin.EmployeeID">
          ...
          <td class="pr-3">
            <project-per-field-form :Project="value" :disable="!per[key].Access" :keyPer="key" :title="'Phân quyền'" field-model="AccessField" :table="'doc'" v-model="field.AccessField" @updateFromChild="updateParent"></project-per-field-form>
          </td>
          <td class="pr-3 text-center">
          ...
          <td class="pr-3">
            <project-per-field-form :Project="value" :disable="!per[key].Edit" :keyPer="key" :title="'Phân quyền'" field-model="EditField" :table="'doc'" v-model="field.EditField" @updateFromChild="updateParent"></project-per-field-form>
          </td>
          <td class="pr-3 text-center">
...
import ProjectPerFieldView from "./ProjectPerFieldView";
import ProjectPerFieldForm from "./ProjectPerFieldForm";
import Select2 from 'v-select2-component';
import IjcoreModalMultiListing from "../../../../components/IjcoreModalMultiListing";

export default {
  name: 'ProjectPerForm',
  components: {IjcoreModalMultiListing, ProjectPerFieldForm, ProjectPerFieldView, Select2},
  computed: {
          ...
          _.forEach(dataResponse.data, function (val, key) {
            ...
            if(isExist == 0){
              self.per.push({
                ...
                ProjectID: self.value.ProjectID,
                Access: true,
                AccessField: 'all',
                Edit: false,
                EditField: '',
