        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại tài liệu<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại tài liệu<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
        ...
        <div class="container-fluid">
          <b-card>
            ...
            <div class="form-group row align-items-center">
              <label class="col-md-4 m-0" for="CateName">Tên loại tài liệu</label>
              <div class="col-md-20">
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-20">
                <Select2 v-model="model.parentID" :options="model.docCateListOption"></Select2>
              </div>
            </div>

            <label>Giá trị loại tài liệu:</label>
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <tr>
                ...
                <th scope="col" style="width: 3%" class="text-center"></i>
                </th>
              </tr>
              </thead>
              <tbody>
              <tr v-for="(field, key) in model.docCateValue">
                <td>
                  <b-form-input
                    type="text"
                    v-model="model.docCateValue[key].Description"
                    autocomplete="Doc cate list description">
                  </b-form-input>
                </td>
                <td>
                  <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                 v-model="model.docCateValue[key].DataType"></b-form-select>
                </td>
                <td>
                  <b-form-input
                    v-if="field.DataType === 1"
                    type="number"
                    v-model="model.docCateValue[key].CateValue"
                    autocomplete="Doc cate list description">
                  </b-form-input>
                  <b-form-input
                  ...
                  <b-form-select v-if="field.DataType === 5" v-model="model.docCateValue[key].CateValue"
                                 :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                  <b-form-select v-if="field.DataType === 6" v-model="model.docCateValue[key].CateValue"
...
<script>
  ...
  const ListRouter = 'listing-dcatelist';
  const EditRouter = 'listing-dcatelist-edit';
  const CreateRouter = 'listing-dcatelist-create';
  const ViewRouter = 'listing-dcatelist-view';
  const ViewApi = 'listing/api/doc-cate-list/view';
  const EditApi = 'listing/api/doc-cate-list/edit';
  const CreateApi = 'listing/api/doc-cate-list/create';
  const StoreApi = 'listing/api/doc-cate-list/store';
  const UpdateApi = 'listing/api/doc-cate-list/update';
  const ListApi = 'listing/api/doc-cate-list';
  ...
  export default {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data.data)) {
                ...
                self.model.docCateValue = responsesData.data.docCateValue;
                // set max lineID
                _.forEach(self.model.docCateValue, function (field, key) {
                  if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                  // set type of value
                  if (field.DataType == 1) self.model.docCateValue[key].CateValue = Number(self.model.docCateValue[key].CateValue);
                  if (field.DataType == 2) self.model.docCateValue[key].CateValue = String(self.model.docCateValue[key].CateValue);
                  if (field.DataType == 3) self.model.docCateValue[key].CateValue = moment(self.model.docCateValue[key].CateValue).format('DD/MM/YYYY');
                  if (field.DataType == 4) self.model.docCateValue[key].CateValue = moment(self.model.docCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
                });

              }
            }

            if (_.isArray(responsesData.data.docCateList)) {

              self.model.docCateListOption = [];
              _.forEach(responsesData.data.docCateList, function (value, key) {
                let tmpObj = {};
                tmpObj.id = value.CateID;
                tmpObj.text = value.CateName;
                if (self.model.parentID == value.CateID) self.model.parentID = value.CateID;
                self.model.docCateListOption.push(tmpObj);
              });
            }
      ...
      onAddFieldOnTable() {
        ...
        this.model.docCateValue.push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldOnTable(field) {

        // remove field in fieldOnTableReq
        let fieldExist = _.find(this.model.docCateValue, ['LineID', field.LineID]);
        if (_.isObject(fieldExist)) {
          _.remove(this.model.docCateValue, ['LineID', field.LineID]);
        }
        this.$forceUpdate();
      },
      handleSubmitForm() {
        ...
        const requestData = {
          ...
          data: {
            ...
            detail: this.model.docCateValue
          }
        };
