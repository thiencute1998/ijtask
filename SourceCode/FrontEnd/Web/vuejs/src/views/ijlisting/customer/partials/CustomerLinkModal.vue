<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-customer-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CustomerLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CustomerLinkContent>
  ...
  export default {
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCustomerLink){
          console.log(this.$parent);
          this.$parent.showCustomerLink = true;
        }
      },
      onEdit(){
        this.fetchData();
