              ...
              <div class="kanban-items row">
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    ...
                    <div class="kanban-record" v-if="item.Address && item.Address !== ''">
                      <span>Địa chỉ</span>
                      <span class="kanban-text">{{item.Address}}</span>
                    </div>
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        CustomerLink: (this.$route.query && this.$route.query.CustomerLink) ? this.$route.query.CustomerLink : [],
      }
    }
  },
  components:{
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CustomerNo', label: 'Mã khách hàng', field: 'CustomerNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'CustomerName', label: 'Tên khách hàng', thStyle: 'min-width: 300px', field: 'CustomerName',searchOnTopRow: {type: 'text'}},
        {key: 'Address', label: 'Địa chỉ', thStyle: 'width: 15%; min-width: 150px', field: 'Address',searchOnTopRow: {type: 'text'}},
        {key: 'Fax', label: 'Fax', field: 'Fax', thStyle: 'width: 15%; min-width: 150px', searchOnTopRow: {type: 'text'}, tdClass: 'text-center'},
  ...
  created() {
    ...
    this.settings.ListApi = 'listing/api/customer';
    this.settings.DeleteApi = 'listing/api/customer/delete';
    this.settings.CreateRouter = 'listing-customer-create';
    this.settings.EditRouter = 'listing-customer-edit';
    this.settings.ViewRouter = 'listing-customer-view';

    this.modelSearch = {
      CustomerNo: (this.$route.query && this.$route.query.CustomerNo) ? this.$route.query.CustomerNo : '',
      CustomerName: (this.$route.query && this.$route.query.CustomerName) ? this.$route.query.CustomerName : '',
      Address: (this.$route.query && this.$route.query.Address) ? this.$route.query.Address : '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.CustomerNo.trim() !== '') {
      ...
      if (this.modelSearch.Address !== '') {
        requestData.data.Address = this.modelSearch.Address;
      }
      ...
      if (this.filter.CustomerLink.length) {
        let haveLink = false;
        _.forEach(this.filter.CustomerLink, function (customerLink, key) {
        ...
        if (haveLink) {
          requestData.data.CustomerLink = this.filter.CustomerLink;
        }
      }
    ...
    handleExportPrint() {
      ...
      if (this.modelSearch.CustomerNo !== '') {
        request.CustomerNo = this.modelSearch.CustomerNo;
      }
      if (this.modelSearch.CustomerName !== '') {
        request.CustomerName = this.modelSearch.CustomerName;
      }
      if (this.modelSearch.Fax !== '') {
        request.Fax = this.modelSearch.Fax;
      }
      if (this.modelSearch.Address !== '') {
        request.Address = this.modelSearch.Address;
      }
      if (this.modelSearch.Email !== '') {
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('customer');
      let request = {};
      if (this.modelSearch.CustomerNo !== '') {
        request.CustomerNo = this.modelSearch.CustomerNo;
      }
      ...
      if (this.filter.CustomerLink.length) {
        ...
        if (haveLink) {
          request.CustomerLink = this.filter.CustomerLink;
        }
      }
      request.exportData = true;

      let reportData = await this.$_lists_handleReportResponse('listing/api/customer/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Customer');
    },
  },
  watch: {
    currentPage() {
