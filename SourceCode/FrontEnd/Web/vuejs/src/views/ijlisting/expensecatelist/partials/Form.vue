        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại khoản chi<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại khoản chi<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="ExpenseName" class="form-control" placeholder="Tên khoản chi" name="ExpenseName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  ...
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại khoản chi cha'"
                  :placeholderSearch="'Nhập loại khoản chi'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.expenseCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeExpenseTableItem">
                <tr class="draggable" v-for="(field, key) in model.expenseCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.expenseCateValue[key].Description"
                      autocomplete="expense cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.expenseCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.expenseCateValue[key].CateValue"
                      autocomplete="expense cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.expenseCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.expenseCateValue[key].CateValue"
...
const ListRouter = 'listing-expensecatelist';
const EditRouter = 'listing-expensecatelist-edit';
const CreateRouter = 'listing-expensecatelist-edit';
const ViewRouter = 'listing-expensecatelist-view';
const ViewApi = 'listing/api/expense-cate-list/view';
const EditApi = 'listing/api/expense-cate-list/edit';
const CreateApi = 'listing/api/expense-cate-list/create';
const StoreApi = 'listing/api/expense-cate-list/store';
const UpdateApi = 'listing/api/expense-cate-list/update';
const ListApi = 'listing/api/expense-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                let Parent = _.filter(responsesData.data.expenseCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.expenseCateValue = responsesData.data.expenseCateValue;

              // set max lineID
              _.forEach(self.model.expenseCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.expenseCateValue[key].CateValue = Number(self.model.expenseCateValue[key].CateValue);
                if (field.DataType == 2) self.model.expenseCateValue[key].CateValue = String(self.model.expenseCateValue[key].CateValue);
                if (field.DataType == 3) self.model.expenseCateValue[key].CateValue = moment(self.model.expenseCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.expenseCateValue[key].CateValue = moment(self.model.expenseCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.expenseCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.expenseCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.expenseCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.expenseCateValue
        }
      };
    ...
    changeExpenseTableItem(moved) {},

    getListChild(CateID){
      let self = this;
      let requestData = {
        method: 'post',
        url: 'listing/api/expense-cate-list/get-list-child',
        data: {
