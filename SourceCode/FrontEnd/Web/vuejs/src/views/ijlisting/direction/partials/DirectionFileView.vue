    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <direction-file-edit :Direction="Direction" v-model="value[key]"></direction-file-edit>
...
import DirectionFileEdit from "./DirectionFileEdit";

export default {
  name: 'DirectionFileView',
  components: {
    DirectionFileEdit,
    IjcoreUploadFile,
  ...
  methods: {
    ...
    downloadFile(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/direction/download-file/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewFile(key) {
      ...
      let ValidDirectionTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/direction/download-file/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (ValidDirectionTypes.indexOf(type) >= 0) {
          self.FileType = 1;
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/direction/delete-file/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
