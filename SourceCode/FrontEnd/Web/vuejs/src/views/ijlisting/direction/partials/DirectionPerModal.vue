<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <direction-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></direction-per-view>
      <direction-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></direction-per-form>
...
import DirectionPerView from "./DirectionPerView";
import DirectionPerForm from "./DirectionPerForm";

export default {
  name: 'direction-per-modal',
  components: {DirectionPerForm, DirectionPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/direction/update-per';
      ...
      const requestData = {
        ...
        data: {
          ...
          DirectionPer: self.per
        }
      };
      // edit user
      requestData.url = UpdateApi + '/' + self.value.DirectionID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Direction', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
