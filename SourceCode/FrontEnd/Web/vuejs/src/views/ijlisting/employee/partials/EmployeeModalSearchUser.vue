<template>
  <div class="ijcore-modal-search-input" @mouseenter="onMouseenter">
    <b-input-group>
      <b-form-input placeholder="Chọn người dùng" :readonly="true" v-model="value.UserName" class="input-user-employee readonly form-control"></b-form-input>
      <input type="hidden" :readonly="true" v-model="value.UserID" class="readonly form-control"/>
    ...
    <b-modal ref="modal" id="modal">
      ...
      <b-input-group>
        <b-form-input v-model="search" placeholder="Nhập tên người dùng" class="readonly form-control" @change="fetchData"></b-form-input>
      ...
      <div class="table-responsive">
        <table class="table b-table table-hover table-sm b-table-selectable b-table-select-multi">
          <tbody>
          <tr v-for="item in listtable" @click="updateForParent(item['UserID'], item['FullName'])">
            <td class="pr-3 tr-no" style="width: 80px;">{{item['UserID']}}</td>
            <td class="pr-3"><span :title="item['FullName']">{{item['FullName']}}</span></td>
<!--            <td class="pr-3"><span :title="item.UserName">{{item.UserName}}</span></td>-->
          </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  mounted() {
    let self = this;
    this.$el.querySelector('.input-user-employee').addEventListener('click', function () {
      self.onToggleModal();
    });
  },
  methods: {
    ...
    updateForParent: function (UserID, UserName) {
