<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-employee-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <EmployeeLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></EmployeeLinkContent>
...
<script>
  ...
  import EmployeeLinkContent from "./EmployeeLinkContent";

  export default {
    name: 'EmployeeLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Employee: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showEmployeeLink){
          console.log(this.$parent);
          this.$parent.showEmployeeLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/employee/employee-data-list',
          data: {
            EmployeeLink : this.value,
