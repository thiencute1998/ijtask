                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Nhân viên</span>
                            </div>
                        </b-col>
...
<!--                              <i class="fa fa-random m-0"></i>-->
            ...
            <div class="main-body" :class="(stage.viewType === 'list') ? 'main-body-view-list' : 'main-body-view-kanban'">
                <b-card class="m-0 border-0" body-class="py-0 px-0">
                  <div class="content-body content-body-list" v-if="stage.viewType === 'list'">
                      <div class="content-table">
                          <b-table ref="selectableTable"
                            ...
                            <template v-slot:cell(PositionName)="data">
                              <span :title="data.item.PositionName">{{data.item.PositionName}}</span>
                            </template>
                          </b-table>
                      </div>
                  </div>
                      ...
                      <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                        <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                          <div class="kanban-record d-flex justify-content-between">
                            <span class="kanban-title">{{item.EmployeeName}}</span>
                            <span class="kanban-no">{{item.EmployeeNo}}</span>
                          </div>
...
<script>
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
            return {
              filter: {
                ...
                EmployeeLink: (this.$route.query && this.$route.query.EmployeeLink) ? this.$route.query.EmployeeLink : [],
              }
            }
        },
        components:{
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'EmployeeNo', label: 'Mã nhân viên', thStyle: 'width: 10%; min-width: 100px', field: 'EmployeeNo', searchOnTopRow: {type: 'text'}},
                    {key: 'EmployeeName', label: 'Tên nhân viên', thStyle: 'width: 20%;min-width: 150px',field: 'EmployeeName',searchOnTopRow: {type: 'text'}},
                    {key: 'PositionName', label: 'Chức vụ',thStyle: 'width: 15%; min-width: 150px', field: 'PositionName',searchOnTopRow: {type: 'text'}},
                    {key: 'CompanyName', label: 'Đơn vị',thStyle: 'width: 35%; min-width: 300px', field: 'CompanyName',searchOnTopRow: {type: 'text'}},
        ...
        created() {
          this.settings.FieldID = 'EmployeeID';
          this.settings.Table = 'employee';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/employee';
          this.settings.DeleteApi = 'listing/api/employee/delete';
          this.settings.CreateRouter = 'listing-employee-create';
          this.settings.EditRouter = 'listing-employee-edit';
          this.settings.ViewRouter = 'listing-employee-view';

          this.modelSearch = {
            EmployeeNo: (this.$route.query && this.$route.query.EmployeeNo) ? this.$route.query.EmployeeNo : '',
            EmployeeName: (this.$route.query && this.$route.query.EmployeeName) ? this.$route.query.EmployeeName : '',
            PositionName: (this.$route.query && this.$route.query.PositionName) ? this.$route.query.PositionName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.EmployeeNo.trim() !== '') {
              requestData.data.EmployeeNo = this.modelSearch.EmployeeNo;
            }
            ...
            if (this.modelSearch.PositionName !== '') {
              requestData.data.PositionName = this.modelSearch.PositionName;
            }
            ...
            if (this.filter.EmployeeLink.length) {
              let haveLink = false;
              _.forEach(this.filter.EmployeeLink, function (employeeLink, key) {
                if (employeeLink.LinkID) {
                  haveLink = true;
                  return false;
                }
              });
              if (haveLink) {
                requestData.data.EmployeeLink = this.filter.EmployeeLink;
              }
            }
            ...
            ApiService.customRequest(requestData).then((response) => {
              ...
              if (dataResponse.status === 1) {
                ...
                _.forEach(dataResponse.data.data, function (employee, key) {
                  let Prefix = (employee.Prefix) ? employee.Prefix : '';
                  let Suffix = (employee.Suffix) ? employee.Suffix : '';
                  dataResponse.data.data[key].EmployeeNo = Prefix + employee.EmployeeNo + Suffix;
                });
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('employee');
            let request = {};
            if (this.modelSearch.EmployeeNo !== '') {
              request.EmployeeNo = this.modelSearch.EmployeeNo;
            }
            if (this.modelSearch.EmployeeName !== '') {
              request.EmployeeName = this.modelSearch.EmployeeName;
            }
            if (this.modelSearch.PositionName !== '') {
              request.PositionName = this.modelSearch.PositionName;
            }
            if (this.modelSearch.CompanyName !== '') {
            ...
            if (this.filter.EmployeeLink.length) {
              ...
              if (haveLink) {
                request.EmployeeLink = this.filter.EmployeeLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/employee/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'Employee');
          },
          handleExportPrint() {
            ...
            if (this.modelSearch.CompanyID !== '') {
              request.CompanyID = this.modelSearch.CompanyID;
            }
            if (this.modelSearch.Email !== '') {
              request.Email = this.modelSearch.Email;
            }
