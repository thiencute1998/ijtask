                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Nhà tài trợ</span>
                            </div>
                        </b-col>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.SponsorName}}</span>
                                <span class="kanban-no">{{item.SponsorNo}}</span>
                              </div>
                              <div class="kanban-record">
                                <span class="kanban-text" v-if="item.SponsorAddress && item.SponsorAddress !== ''">{{item.SponsorAddress}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'SponsorNo', label: 'Mã số', field: 'SponsorNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
                    {key: 'SponsorName', label: 'Tên nhà tài trợ', thStyle: 'min-width: 300px', field: 'SponsorName',searchOnTopRow: {type: 'text'}},
                    {key: 'SponsorAddress', label: 'Địa chỉ', thStyle: 'width: 15%; min-width: 150px', field: 'SponsorAddress',searchOnTopRow: {type: 'text'}},
                ];
        ...
        created() {
          ...
          this.settings.FieldID = 'SponsorID';
          this.settings.Table = 'sponsor';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/sponsor';
          this.settings.DeleteApi = 'listing/api/sponsor/delete';
          this.settings.CreateRouter = 'listing-sponsor-create';
          this.settings.EditRouter = 'listing-sponsor-edit';
          this.settings.ViewRouter = 'listing-sponsor-view';
        ...
        methods: {
            ...
            fetchData(){
              ...
              if (this.modelSearch.SponsorNo.trim() !== '') {
                  requestData.data.SponsorNo = this.modelSearch.SponsorNo;
              }
              if (this.modelSearch.SponsorName !== '') {
                  requestData.data.SponsorName = this.modelSearch.SponsorName;
              }
              if (this.modelSearch.SponsorAddress !== '') {
                  requestData.data.SponsorAddress = this.modelSearch.SponsorAddress;
              }
          ...
          handleExportPrint() {
            ...
            let request = {};
            if (this.modelSearch.SponsorNo !== '') {
              request.SponsorNo = this.modelSearch.SponsorNo;
            }
            if (this.modelSearch.SponsorName !== '') {
              request.SponsorName = this.modelSearch.SponsorName;
            }
            if (this.modelSearch.SponsorAddress !== '') {
              request.SponsorAddress = this.modelSearch.SponsorAddress;
            }
