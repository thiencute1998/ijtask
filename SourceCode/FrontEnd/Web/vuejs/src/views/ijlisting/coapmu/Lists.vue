                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Hệ thống tài khoản ban quản lý dự án</span>
                            </div>
                        </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <coa-pmu-modal-search-link
                            v-model="filter.CoaPmuLink"
...
<script>
    ...
    import CoaPmuModalSearchLink from "./partials/CoaPmuModalSearchLink";
    import CoaPmuModalSearchEmployee from "./partials/CoaPmuModalSearchEmployee";
    ...
    export default {
        ...
        data() {
            return {
              filter: {
                ...
                CoaPmuLink: (this.$route.query && this.$route.query.CoaPmuLink) ? this.$route.query.CoaPmuLink : [],
              }
            }
        },
        components:{
          CoaPmuModalSearchLink,
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'AccountName', label: 'Tên Hệ thống tài khoản ban quản lý dự án', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'AccountName',searchOnTopRow: {type: 'text'}},
                ];
                return fieldsTable;
            }
        },
        created() {
          ...
          this.settings.Table = 'coa_pmu';
          this.settings.FieldInactive = 'Inactive';
          this.settings.ListApi = 'listing/api/coa-pmu';
          this.settings.DeleteApi = 'listing/api/coa-pmu/delete';
          this.settings.GetListChildApi = 'listing/api/coa-pmu/get-list-child';
          this.settings.CreateRouter = 'listing-coa-pmu-create';
          this.settings.EditRouter = 'listing-coa-pmu-edit';
          this.settings.ViewRouter = 'listing-coa-pmu-view';
        ...
        methods: {
          ...
          handle (data){
            ...
            fetchData(){
              ...
              if (this.filter.CoaPmuLink.length) {
                let haveLink = false;
                _.forEach(this.filter.CoaPmuLink, function (coaPmuLink, key) {
                ...
                if (haveLink) {
                  requestData.data.CoaPmuLink = this.filter.CoaPmuLink;
                }
              }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCoaPmu= this.getAllChildCoaPmu(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.AccountID);
              }
              _.forEach(allChildCoaPmu, function (coaPmu, key) {

                let _index = _.findIndex(self.itemsArray, ['AccountNo', coaPmu.AccountNo]);
                if (_index >= 0) {
                  if(coaPmu.ParentID == item.AccountID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(coaPmu.Detail == 0){
                      coaPmu.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
            ...
            getAllChildCoaPmu(item, coapmuArr){
              let self = this, listChild = [];
              let allChild = _.filter(coapmuArr, ['ParentID', item.AccountID]);
              if (allChild.length) {
                allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
                _.forEach(allChild, function (value, key) {
                  listChild.push(value);
                  if (_.filter(coapmuArr, ['ParentID', value.AccountID]).length) {
                    let recursiveArr = self.getAllChildCoaPmu(value, coapmuArr);
          ...
          getListChild(AccountID){
            ...
            let requestData = {
              method: 'post',
              url: 'listing/api/coa-pmu/get-list-child',
              data: {
                per_page: this.perPage,
                page: this.currentPage,
                ParentID: AccountID,
            ...
            openReport(){
              this.$router.push({
                name: 'listing-coa-pmu-report-list'
              });
            },
            ...
            let reportJson = await this.$_lists_handleReportTemplate('coa-pmu');
            let request = {};
            ...
            if (this.filter.CoaPmuLink.length) {
              ...
              if (haveLink) {
                request.CoaPmuLink = this.filter.CoaPmuLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/coa-pmu/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'CoaPmu');
          },
        },
      watch: {
        currentPage() {
