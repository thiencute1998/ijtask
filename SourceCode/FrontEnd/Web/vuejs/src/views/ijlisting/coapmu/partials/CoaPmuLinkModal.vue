<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-pmu-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaPmuLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaPmuLinkContent>
...
<script>
  ...
  import CoaPmuLinkContent from "./CoaPmuLinkContent";

  export default {
    name: 'CoaPmuLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaPmu: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaPmuLink){
          console.log(this.$parent);
          this.$parent.showCoaPmuLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-pmu/coa-pmu-data-list',
          data: {
            CoaPmuLink : this.value,
            AccountID: this.CoaPmu.AccountID
          }
