<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-pmu-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-pmu-per-view>
      <coa-pmu-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaPmuOption="CoaPmuOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-pmu-per-form>
...
import CoaPmuPerView from "./CoaPmuPerView";
import CoaPmuPerForm from "./CoaPmuPerForm";

export default {
  name: 'coa-pmu-per-modal',
  components: {CoaPmuPerForm, CoaPmuPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-pmu/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaPmu', 'isDetail', 'per', 'EmployeeOption', 'CoaPmuOption', 'GroupOption'],
}
</script>
<style>
.readonly {
