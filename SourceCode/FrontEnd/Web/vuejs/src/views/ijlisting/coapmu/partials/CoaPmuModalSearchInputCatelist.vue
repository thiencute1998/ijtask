<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    >
      ...
      <div class="ijcore-modal-coa-pmu-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
            <tr>
              <th class="pr-3">Loại HTTK BQLDA</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại HTTK BQLDA'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'coa-pmu-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/coa-pmu/get-coa-pmu-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.CoaPmuCateList;
        ...
        self.CateValue = responsesData.data.CoaPmuCateValue;
...
<style lang="css">
  ...
  .ijcore-modal-coa-pmu-ccatelist .select2.select2-container{
    width: 100%;
  }
