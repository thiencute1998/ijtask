                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Hệ thống tài khoản kho bạc nhà nước</span>
                            </div>
                        </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <coa-tab-modal-search-link
                            v-model="filter.CoaTabLink"
...
<script>
    ...
    import CoaTabModalSearchLink from "./partials/CoaTabModalSearchLink";
    import CoaTabModalSearchEmployee from "./partials/CoaTabModalSearchEmployee";
    ...
    export default {
        ...
        data() {
            return {
              filter: {
                ...
                CoaTabLink: (this.$route.query && this.$route.query.CoaTabLink) ? this.$route.query.CoaTabLink : [],
              }
            }
        },
        components:{
          CoaTabModalSearchLink,
          CoaTabModalSearchEmployee
        },
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'AccountName', label: 'Tên Hệ thống tài khoản kho bạc nhà nước', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'AccountName',searchOnTopRow: {type: 'text'}},
                ];
                return fieldsTable;
            }
        },
        created() {
          ...
          this.settings.Table = 'coa_tab';
          this.settings.FieldInactive = 'Inactive';
          this.settings.ListApi = 'listing/api/coa-tab';
          this.settings.DeleteApi = 'listing/api/coa-tab/delete';
          this.settings.GetListChildApi = 'listing/api/coa-tab/get-list-child';
          this.settings.CreateRouter = 'listing-coa-tab-create';
          this.settings.EditRouter = 'listing-coa-tab-edit';
          this.settings.ViewRouter = 'listing-coa-tab-view';
        ...
        methods: {
          ...
          handle (data){
            ...
            fetchData(){
              ...
              if (this.filter.CoaTabLink.length) {
                let haveLink = false;
                _.forEach(this.filter.CoaTabLink, function (coaTabLink, key) {
                ...
                if (haveLink) {
                  requestData.data.CoaTabLink = this.filter.CoaTabLink;
                }
              }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCoaTab= this.getAllChildCoaTab(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.AccountID);
              }
              _.forEach(allChildCoaTab, function (coaTab, key) {

                let _index = _.findIndex(self.itemsArray, ['AccountNo', coaTab.AccountNo]);
                if (_index >= 0) {
                  if(coaTab.ParentID == item.AccountID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(coaTab.Detail == 0){
                      coaTab.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
            ...
            getAllChildCoaTab(item, coatabArr){
              let self = this, listChild = [];
              let allChild = _.filter(coatabArr, ['ParentID', item.AccountID]);
              if (allChild.length) {
                allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
                _.forEach(allChild, function (value, key) {
                  listChild.push(value);
                  if (_.filter(coatabArr, ['ParentID', value.AccountID]).length) {
                    let recursiveArr = self.getAllChildCoaTab(value, coatabArr);
          ...
          getListChild(AccountID){
            ...
            let requestData = {
              method: 'post',
              url: 'listing/api/coa-tab/get-list-child',
              data: {
                per_page: this.perPage,
                page: this.currentPage,
                ParentID: AccountID,
            ...
            openReport(){
              this.$router.push({
                name: 'listing-coa-tab-report-list'
              });
            },
            ...
            let reportJson = await this.$_lists_handleReportTemplate('coa-tab');
            let request = {};
            ...
            if (this.filter.CoaTabLink.length) {
              ...
              if (haveLink) {
                request.CoaTabLink = this.filter.CoaTabLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/coa-tab/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'CoaTab');
          },
        },
      watch: {
        currentPage() {
