<template>
  <div>
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-coa-tab-general" size="xl">
      <template v-slot:modal-title>
        <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i> {{title}}
      </template>
      <coa-tab-general-form v-model="value" :coaTabOptions="coaTabOptions" :employeeOptions="employeeOptions" v-if="isForm"></coa-tab-general-form>
      <coa-tab-general-view :isDetail="true" v-model="value" v-else></coa-tab-general-view>
...
import CoaTabGeneralView from "./CoaTabGeneralView";
import CoaTabGeneralForm from "./CoaTabGeneralForm";

export default {
  name: 'CoaTabGeneralModal',
  props: ['value','title'],
  components: {
    CoaTabGeneralView,
    CoaTabGeneralForm
  ...
  methods: {
    ...
    onUpdate(){
      ...
      let updateAPI = 'listing/api/coa-tab/update/';
      let requestData = {
        method: 'post',
        ...
        data: {
          ...
          CoaTabCate: this.value.CoaTabCate,
      ...
      .then(response=>{
        ...
        if(responsesData.status === 1){
          ...
          let item = {
            ...
            CoaTabCate: self.value.CoaTabCate,
