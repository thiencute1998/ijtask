<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-tab-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaTabLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaTabLinkContent>
...
<script>
  ...
  import CoaTabLinkContent from "./CoaTabLinkContent";

  export default {
    name: 'CoaTabLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaTab: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaTabLink){
          console.log(this.$parent);
          this.$parent.showCoaTabLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-tab/coa-tab-data-list',
          data: {
            CoaTabLink : this.value,
            AccountID: this.CoaTab.AccountID
          }
