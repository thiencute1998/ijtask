<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-tab-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-tab-per-view>
      <coa-tab-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaTabOption="CoaTabOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-tab-per-form>
...
import CoaTabPerView from "./CoaTabPerView";
import CoaTabPerForm from "./CoaTabPerForm";

export default {
  name: 'coa-tab-per-modal',
  components: {CoaTabPerForm, CoaTabPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-tab/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaTab', 'isDetail', 'per', 'EmployeeOption', 'CoaTabOption', 'GroupOption'],
}
</script>
<style>
.readonly {
