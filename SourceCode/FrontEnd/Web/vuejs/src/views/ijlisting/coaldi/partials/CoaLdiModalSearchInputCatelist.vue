<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    >
      ...
      <div class="ijcore-modal-coa-ldi-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
            <tr>
              <th class="pr-3">Loại HTTK QĐTPTĐP</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại HTTK QĐTPTĐP'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'coa-ldi-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/coa-ldi/get-coa-ldi-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.CoaLdiCateList;
        ...
        self.CateValue = responsesData.data.CoaLdiCateValue;
...
<style lang="css">
  ...
  .ijcore-modal-coa-ldi-ccatelist .select2.select2-container{
    width: 100%;
  }
