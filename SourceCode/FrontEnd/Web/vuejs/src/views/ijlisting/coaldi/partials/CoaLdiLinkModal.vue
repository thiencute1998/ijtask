<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-ldi-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaLdiLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaLdiLinkContent>
...
<script>
  ...
  import CoaLdiLinkContent from "./CoaLdiLinkContent";

  export default {
    name: 'CoaLdiLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaLdi: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaLdiLink){
          console.log(this.$parent);
          this.$parent.showCoaLdiLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-ldi/coa-ldi-data-list',
          data: {
            CoaLdiLink : this.value,
            AccountID: this.CoaLdi.AccountID
          }
