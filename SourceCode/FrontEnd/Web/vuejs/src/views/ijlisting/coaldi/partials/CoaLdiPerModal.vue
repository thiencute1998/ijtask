<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-ldi-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-ldi-per-view>
      <coa-ldi-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaLdiOption="CoaLdiOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-ldi-per-form>
...
import CoaLdiPerView from "./CoaLdiPerView";
import CoaLdiPerForm from "./CoaLdiPerForm";

export default {
  name: 'coa-ldi-per-modal',
  components: {CoaLdiPerForm, CoaLdiPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-ldi/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaLdi', 'isDetail', 'per', 'EmployeeOption', 'CoaLdiOption', 'GroupOption'],
}
</script>
<style>
.readonly {
