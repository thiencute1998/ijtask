<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <sbi-item-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></sbi-item-per-view>
      <sbi-item-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></sbi-item-per-form>
...
import SbiItemPerView from "./SbiItemPerView";
import SbiItemPerForm from "./SbiItemPerForm";

export default {
  name: 'sbi-item-per-modal',
  components: {SbiItemPerForm, SbiItemPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/sbi-item/update-per';
      ...
      // edit user
      requestData.url = UpdateApi + '/' + self.value.SbiItemID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'SbiItem', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
