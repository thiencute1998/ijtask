          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Loại nhà cung cấp</span>
              </div>
            </b-col>
            <b-col class="col-md-12"></b-col>
          </b-row>
          <b-row class="mb-2">partials
            Create.vue
            Edit.vue
            Index.vue
            List.vue
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CateName', label: 'Tên loại nhà cung cấp', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'vendor_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/vendor-cate-list';
    this.settings.DeleteApi = 'listing/api/vendor-cate-list/delete';
    this.settings.CreateRouter = 'listing-vendorcatelist-create';
    this.settings.EditRouter = 'listing-vendorcatelist-edit';
    this.settings.ViewRouter = 'listing-vendorcatelist-view';
  ...
  methods: {
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                debugger
                let index = _.findIndex(self.itemsArray,['CateID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['CateID',self.itemsArray[index].ParentID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('vendor-cate-list');
      let request = {};
      if (this.modelSearch.CateID !== '') {
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/vendor-cate-list/get-report-data', request);
