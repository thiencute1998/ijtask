        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại nhà cung cấp<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại nhà cung cấp<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên nhà cung cấp" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  ...
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại nhà cung cấp cha'"
                  :placeholderSearch="'Nhập loại nhà cung cấp'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.vendorCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeVendorTableItem">
                <tr class="draggable" v-for="(field, key) in model.vendorCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.vendorCateValue[key].Description"
                      autocomplete="vendor cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.vendorCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.vendorCateValue[key].CateValue"
                      autocomplete="vendor cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.vendorCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.vendorCateValue[key].CateValue"
...
const ListRouter = 'listing-vendorcatelist';
const EditRouter = 'listing-vendorcatelist-edit';
const CreateRouter = 'listing-vendorcatelist-edit';
const ViewRouter = 'listing-vendorcatelist-view';
const ViewApi = 'listing/api/vendor-cate-list/view';
const EditApi = 'listing/api/vendor-cate-list/edit';
const CreateApi = 'listing/api/vendor-cate-list/create';
const StoreApi = 'listing/api/vendor-cate-list/store';
const UpdateApi = 'listing/api/vendor-cate-list/update';
const ListApi = 'listing/api/vendor-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                let Parent = _.filter(responsesData.data.vendorCateList, ["CateID", responsesData.data.data.ParentID])
    ...
    onAddFieldOnTable() {
      ...
      this.model.vendorCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.vendorCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.vendorCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.vendorCateValue
        }
      };
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/vendor-cate-list/get-list-child',
        data: {
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          _.orderBy(self.$route.params.req.itemsArray,'VendorNo','asc');
        };
        self.$store.commit('isLoading', false);
      }, (error) => {
        console.log(error);
