        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại giao dịch hỗ trợ<span
                v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại giao dịch hỗ trợ<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
        ...
        <div class="container-fluid">
          <b-card>
            ...
            <div class="form-group row align-items-center">
              <label class="col-md-4 m-0" >Tên loại GDHT</label>
              <div class="col-md-20">
                <input v-model="model.CateName" type="text" id="CateName" class="form-control"
                       placeholder="Tên loại giao dịch hỗ trợ" name="CateName"/>
              </div>
            </div>
...
<!--                <Select2 v-model="model.ParentID" :options="model.customerCateListOption"></Select2>-->
                <IjcoreModalParentListing v-model="model" :title="'loại giá trị hỗ trợ'" :api="'/listing/api/common/list'"
                                    :table="'customer_support_trans_cate_list'"
            ...
            <label>Giá trị loại giao dịch hỗ trợ:</label>
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <tbody>
              <tr v-for="(field, key) in model.customerSupportTransCateValue">
                <td>
                  <b-form-input
                    type="text"
                    v-model="model.customerSupportTransCateValue[key].CateValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                </td>
                <td>
                  <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                 v-model="model.customerSupportTransCateValue[key].DataType"></b-form-select>
                </td>
                <td>
                  <b-form-input
                    v-if="field.DataType == 1"
                    type="number"
                    v-model="model.customerSupportTransCateValue[key].ConvertedValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                  <b-form-input
                  ...
                  <b-form-select v-if="field.DataType == 5" v-model="model.customerSupportTransCateValue[key].ConvertedValue"
                                 :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                  <b-form-select v-if="field.DataType == 6" v-model="model.customerSupportTransCateValue[key].ConvertedValue"
...
<script>
  ...
  const ListRouter = 'listing-csupporttranscatelist';
  const EditRouter = 'listing-csupporttranscatelist-edit';
  const CreateRouter = 'listing-csupporttranscatelist-create';
  const ViewRouter = 'listing-csupporttranscatelist-view';
  const ViewApi = 'listing/api/customer-support-trans-cate-list/view';
  const EditApi = 'listing/api/customer-support-trans-cate-list/edit';
  const CreateApi = 'listing/api/customer-support-trans-cate-list/create';
  const StoreApi = 'listing/api/customer-support-trans-cate-list/store';
  const UpdateApi = 'listing/api/customer-support-trans-cate-list/update';
  const ListApi = 'listing/api/customer-support-trans-cate-list';
  ...
  export default {
    name: 'listing-csupporttranscatelist-view',
    data() {
      return {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data.data)) {
                ...
                self.model.customerSupportTransCateValue = responsesData.data.customerSupportTransCateValue;
                // set max ValueID
                _.forEach(self.model.customerSupportTransCateValue, function (field, key) {
                  if (Number(field.ValueID) > self.model.maxValueID) self.model.maxValueID = Number(field.ValueID);
                  // set type of value
                  if (field.DataType == 1) self.model.customerSupportTransCateValue[key].ConvertedValue = Number(self.model.customerSupportTransCateValue[key].ConvertedValue);
                  if (field.DataType == 2) self.model.customerSupportTransCateValue[key].ConvertedValue = String(self.model.customerSupportTransCateValue[key].ConvertedValue);
                  if (field.DataType == 3) self.model.customerSupportTransCateValue[key].ConvertedValue = moment(self.model.customerSupportTransCateValue[key].ConvertedValue).format('DD/MM/YYYY');
                  if (field.DataType == 4) self.model.customerSupportTransCateValue[key].ConvertedValue = moment(self.model.customerSupportTransCateValue[key].ConvertedValue).format('DD/MM/YYYY hh:mm');
                });
            ...
            if (_.isArray(responsesData.data.customerSupportTransCateList)) {

              self.model.customerCateListOption = [];
              _.forEach(responsesData.data.customerSupportTransCateList, function (value, key) {
                let tmpObj = {};
                tmpObj.id = value.CateID;
                tmpObj.text = value.CateName;
                if (value.CateID == self.model.ParentID) self.model.ParentID = value.CateID;
      ...
      onAddFieldOnTable() {
        ...
        this.model.customerSupportTransCateValue.push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldOnTable(field) {

        // remove field in fieldOnTableReq
        let fieldExist = _.find(this.model.customerSupportTransCateValue, ['ValueID', field.ValueID]);
        if (_.isObject(fieldExist)) {
          _.remove(this.model.customerSupportTransCateValue, ['ValueID', field.ValueID]);
        }
        this.$forceUpdate();
      },
      handleSubmitForm() {
        ...
        const requestData = {
          ...
          data: {
            ...
            detail: this.model.customerSupportTransCateValue
          }
        };
