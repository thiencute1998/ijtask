<template>
  <div class="component-norm-allot-level">
          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Định mức phân bổ dự toán</span>
              </div>
            </b-col>
            ...
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              <b-table ref="selectableTable"
                ...
                <template slot="top-row" slot-scope="data">
                  <td class="no-overflow" :class="[(field.key == 'selected') ? 'pl-3' : '', (field.key == 'ItemName') ? 'pr-3' : '']" role="cell" v-for="(field, key) in data.fields" style="z-index: 3" >
                    <!-- type input-->
                    <b-form-input
                    ...
                    <ijcore-compare-time
                      v-model="modelSearch[field.field]"
                      @input-date-picker="$_lists_handleSubmitSearch"
                      @clear-date-picker="fetchData"
                      v-if=" field.searchOnTopRow && field.searchOnTopRow.type === 'compareTime' && key !== 0">
                    </ijcore-compare-time>

                    <ijcore-modal-listing
                      v-if=" field.searchOnTopRow && field.searchOnTopRow.type === 'modal-listing' && key !== 0"
                      @changed="$_lists_handleSubmitSearch"
                      v-model="modelSearch" :title="'Đơn vị tính'" :table="'uom'" :api="'/listing/api/common/list'"
                      :field-i-d="'UomID'" :field-no="'UomNo'" :field-name="'UomName'"
                    ></ijcore-modal-listing>
                    <ijcore-compare-number
                      v-if=" field.searchOnTopRow && field.searchOnTopRow.type === 'compare-number' && key !== 0"
                      style="width: 100%"
                      title="Phần trăm hoàn thành"
                      @keyup-compare-input="fetchData"
                      v-model="modelSearch.LCUnitPrice"></ijcore-compare-number>
                ...
                <template v-slot:cell(NormAllotLevelName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-' + data.item.NormAllotLevelID"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.NormAllotLevelName}}</span>
                    <span style="position: absolute;display: block;  width: 35px; height: 30px; z-index: 4;  left: 0; opacity: 0"
                      @click="onToggleChildren($event, data.item)"
                      v-if="data.item.Detail == 0"></span>
                    <i :class="data.item.Class"
                ...
                <template v-else>
                  <span :title="data.item.NormAllotLevelName">{{data.item.NormAllotLevelName}} </span>
                </template>
                <template v-slot:cell(LCUnitPrice)="data">
                  <span>{{data.item.LCUnitPrice | formatCurrency}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.NormAllotLevelName}}</span>
                      <span class="kanban-no">{{item.NormAllotLevelID}}</span>
                    </div>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'NormAllotLevelNo', label: 'Mã số', field: 'NormAllotLevelNo', thStyle: 'width: 5%; min-width: 60px', searchOnTopRow: {type: 'text'}},
        {key: 'NormAllotLevelName', label: 'Tên ĐMPBDT', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'NormAllotLevelName',searchOnTopRow: {type: 'text'}},
        {key: 'EffectiveDate', label: 'Ngày hiệu lực', thStyle: 'z-index:7; width: 13%', tdClass: 'text-center', field: 'EffectiveDate',searchOnTopRow: {type: 'compareTime'}},
        {key: 'ExpirationDate', label: 'Ngày hết hiệu lực', thStyle: 'z-index:7; width: 13%', tdClass: 'text-center', field: 'ExpirationDate',searchOnTopRow: {type: 'compareTime'}},
        {key: 'UomName', label: 'Đơn vị tính', thStyle: 'z-index:7', tdClass: '', field: 'UomName',searchOnTopRow: {type: 'modal-listing'}},
        {key: 'LCUnitPrice', label: 'Định mức', thStyle: 'z-index:7', tdClass: 'text-right', field: 'LCUnitPrice',searchOnTopRow: {type: 'compare-number'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'NormAllotLevelID';
    this.settings.Table = 'norm_allot_level';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/norm-allot-level';
    this.settings.DeleteApi = 'listing/api/norm-allot-level/delete';
    this.settings.CreateRouter = 'listing-normallotlevel-create';
    this.settings.EditRouter = 'listing-normallotlevel-edit';
    this.settings.ViewRouter = 'listing-normallotlevel-view';

    this.modelSearch = {
      NormAllotLevelNo: (this.$route.query && this.$route.query.NormAllotLevelNo) ? this.$route.query.NormAllotLevelNo : '',
      NormAllotLevelName: (this.$route.query && this.$route.query.NormAllotLevelName) ? this.$route.query.NormAllotLevelName : '',
      EffectiveDate: (this.$route.query && this.$route.query.EffectiveDate) ? this.$route.query.EffectiveDate : null,
      ExpirationDate: (this.$route.query && this.$route.query.ExpirationDate) ? this.$route.query.ExpirationDate : null,
      UomName: (this.$route.query && this.$route.query.UomName) ? this.$route.query.UomName : '',
      LCUnitPrice: (this.$route.query && this.$route.query.LCUnitPrice) ? this.$route.query.LCUnitPrice : null,
    };
  ...
  methods: {
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                ...
                let child = _.filter(self.itemsArray, ['NormAllotLevelID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['NormAllotLevelID', child[0].ParentID]);
    ...
    fetchData(){
      ...
      if (this.modelSearch.NormAllotLevelName !== '') {
        requestData.data.NormAllotLevelName = this.modelSearch.NormAllotLevelName;
      }
      if (this.modelSearch.NormAllotLevelNo !== '') {
        requestData.data.NormAllotLevelNo = this.modelSearch.NormAllotLevelNo;
      }
      if (this.stage.filterInactive !== 2) {
        requestData.data.Inactive = this.stage.filterInactive;
      }
      if(this.modelSearch.EffectiveDate && !_.isEmpty(this.modelSearch.EffectiveDate.dateTime) && this.modelSearch.EffectiveDate.dateTime !== '__/__/____'){
        requestData.data.EffectiveDate = this.modelSearch.EffectiveDate;
      }
      if(this.modelSearch.ExpirationDate && !_.isEmpty(this.modelSearch.ExpirationDate.dateTime) && this.modelSearch.ExpirationDate.dateTime !== '__/__/____'){
        requestData.data.ExpirationDate = this.modelSearch.ExpirationDate;
      }
      if(this.modelSearch.UomID ){
        requestData.data.UomID = this.modelSearch.UomID;
      }
      if(this.modelSearch.LCUnitPrice ){
        requestData.data.LCUnitPrice = this.modelSearch.LCUnitPrice;
      }
    ...
    onToggleChildren($event,item){
      ...
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          ...
          self.getListChild(item.NormAllotLevelID);
        }
        _.forEach(allChildCate, function (cate, key) {
          let _index = _.findIndex(self.itemsArray, ['NormAllotLevelNo', cate.NormAllotLevelNo]);
          if (_index >= 0) {
            if(cate.ParentID == item.NormAllotLevelID){
    ...
    getAllChildCate(item, cateArr){
      ...
      let allChild = _.filter(cateArr, ['ParentID', item.NormAllotLevelID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['NormAllotLevelID'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(cateArr, ['ParentID', value.NormAllotLevelID]).length) {
            let recursiveArr = self.getAllChildCate(value, cateArr);
            recursiveArr = _.orderBy(recursiveArr, ['NormAllotLevelID', 'asc']);
    ...
    getListChild(NormAllotLevelID){

      let self = this;
      let requestData = {
        method: 'post',
        url: 'listing/api/norm-allot-level/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: NormAllotLevelID,
        },
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.itemsArray, ['NormAllotLevelID', NormAllotLevelID]);
    ...
    haveChildren(item){
      let children = _.filter(this.itemsArray, ['ParentID', item.NormAllotLevelID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('norm-allot-level');
      let request = {};
      if (this.modelSearch.NormAllotLevelID !== '') {
        request.NormAllotLevelID = this.modelSearch.NormAllotLevelID;
      }
      if (this.modelSearch.NormAllotLevelName !== '') {
        request.NormAllotLevelName = this.modelSearch.NormAllotLevelName;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/norm-allot-level/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateVendor');
    },
  },
  watch: {
    currentPage() {
...
.component-norm-allot-level .component-compare-time .form-group {
  margin-bottom: 0;
}
.component-norm-allot-level .component-compare-number fieldset {
  margin: 0;
}
</style>
