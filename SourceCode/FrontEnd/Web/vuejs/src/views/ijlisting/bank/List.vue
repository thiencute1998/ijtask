          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Ngân hàng </span>
              </div>
            </b-col>
            ...
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(BankName)="data">
                  <span :title="data.item.BankName">{{data.item.BankName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.BankName}}</span>
                      <span class="kanban-no">{{item.BankNo}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'BankNo', label: 'Mã số ', field: 'BankNo', tdClass: 'text-center', thStyle: 'width: 6%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'BankName', label: 'Tên ngân hàng', field: 'BankName',searchOnTopRow: {type: 'text'}, thStyle: 'width: 40%; min-width: 300px'},
        {key: 'BankAddress', label: 'Địa chỉ', field: 'BankAddress', thStyle: 'width: 30%; min-width: 100px', searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    this.settings.FieldID = 'BankID';
    this.settings.Table = 'bank';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/bank';
    this.settings.DeleteApi = 'listing/api/bank/delete';
    this.settings.CreateRouter = 'listing-bank-create';
    this.settings.EditRouter = 'listing-vendorcatelist-edit';
    this.settings.ViewRouter = 'listing-bank-view';
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.BankName !== '') {
        requestData.data.BankName = this.modelSearch.BankName;
      }
      if (this.modelSearch.BankNo !== '') {
        requestData.data.BankNo = this.modelSearch.BankNo;
      }
      if (this.modelSearch.BankAddress !== '') {
        requestData.data.BankAddress = this.modelSearch.BankAddress;
      }
      if (this.modelSearch.Note !== '') {
        requestData.data.Note = this.modelSearch.Note;
      }
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.BankNo !== '') {
        request.BankNo = this.modelSearch.BankNo;
      }
      if (this.modelSearch.BankName !== '') {
        request.BankName = this.modelSearch.BankName;
      }
      if (this.modelSearch.BankAddress !== '') {
        request.BankAddress = this.modelSearch.BankAddress;
      }
