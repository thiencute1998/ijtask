        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại dự án<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại dự án<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="ProjectName" class="form-control" placeholder="Tên dự án" name="ProjectName"/>
              </div>
              <div class="col-md-4 col-sm-8 col-24 d-flex align-items-center app-object-code">
                <span>Mã số</span>
                <input type="text" v-model="model.CateNo" class="form-control" placeholder="Mã số" :disabled="model.Detail == 0"/>
              </div>
            </div>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  ...
                  v-model="model"
                  :title="'loại dự án'"
                  :api="'/listing/api/common/listCate?XDEBUG_SESSION_START=PHPSTORM'"
                  :table="'project'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại dự án cha'"
                  :placeholderSearch="'Nhập loại dự án'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
                </IjcoreModalSystemParentCate>
                <input type="text" v-model="model.ParentName" class="form-control" placeholder="" disabled v-if="model.Detail == 0"/>
              </div>
              <div v-if="model.ParentID" class="col-lg-4 col-md-6 col-sm-8 col-24 d-flex align-items-center app-object-code">
                <span>Mã số: </span>
                <span v-if="model.Detail == 1">{{model.ParentNo}}</span>
                <input type="text" v-model="model.ParentNo" class="form-control" placeholder="" disabled v-if="model.Detail == 0"/>
              </div>
            </div>
            <label>Giá trị :</label>
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.projectCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeProjectTableItem">
                <tr class="draggable" v-for="(field, key) in model.projectCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.projectCateValue[key].Description"
                      autocomplete="project cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.projectCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.projectCateValue[key].CateValue"
                      autocomplete="project cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.projectCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.projectCateValue[key].CateValue"
...
const ListRouter = 'listing-projectcatelist';
const EditRouter = 'listing-projectcatelist-edit';
const CreateRouter = 'listing-projectcatelist-edit';
const ViewRouter = 'listing-projectcatelist-view';
const ViewApi = 'listing/api/project-cate-list/view';
const EditApi = 'listing/api/project-cate-list/edit';
const CreateApi = 'listing/api/project-cate-list/create';
const StoreApi = 'listing/api/project-cate-list/store';
const UpdateApi = 'listing/api/project-cate-list/update';
const ListApi = 'listing/api/project-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                let Parent = _.filter(responsesData.data.projectCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.projectCateValue = responsesData.data.projectCateValue;

              // set max lineID
              _.forEach(self.model.projectCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.projectCateValue[key].CateValue = Number(self.model.projectCateValue[key].CateValue);
                if (field.DataType == 2) self.model.projectCateValue[key].CateValue = String(self.model.projectCateValue[key].CateValue);
                if (field.DataType == 3) self.model.projectCateValue[key].CateValue = moment(self.model.projectCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.projectCateValue[key].CateValue = moment(self.model.projectCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.projectCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.projectCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.projectCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.projectCateValue
        }
      };
            ...
            if(item.ParentID) {
              ...
              if(indexParent >= 0 ){
                if(ClassParentBeforUpdate == 'fa fa-plus-square-o'){
                  ...
                  self.getListChild(self.$route.params.req.itemsArray[indexParent].CateID);
                } else {
    ...
    changeProjectTableItem(moved) {},

    getListChild(CateID){
      let self = this;
      let requestData = {
        method: 'post',
        url: 'listing/api/project-cate-list/get-list-child',
        data: {
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.$route.params.req.itemsArray, ['CateID', CateID]);
          ...
          _.orderBy(self.$route.params.req.itemsArray,'ProjectNo','asc');
        };
        self.$store.commit('isLoading', false);
      }, (error) => {
        console.log(error);
