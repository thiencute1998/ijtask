        ...
        <div class="main-header-padding">
          <b-row class="mb-2">
            <b-col md="12">
              ...
              <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Quốc gia</span>
              </div>
            </b-col>
            <b-col md="12"></b-col>
          </b-row>
            ...
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(CountryName)="data">
                  <span :title="data.item.CountryName">{{data.item.CountryName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.CountryName}}</span>
                      <span class="kanban-no">{{item.CountryNo}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
<script>
  ...
  import mixinLists from '@/mixins/lists';
    ...
    export default {
      ...
      computed: {
        ...
        captions: function() {
          let fieldsTable = [
            ...
            {key: 'CountryName', label: 'Tên quốc gia', field: 'CountryName',thStyle: 'min-width: 300px', searchOnTopRow: {type: 'text'} },
            {key: 'CountryNo', label: 'Mã số', thStyle: 'width: 12%; min-width: 100px', field: 'CountryNo',searchOnTopRow: {type: 'text'}, tdClass:'pr-3'},
      ...
      created() {
        this.settings.FieldID = 'CountryID';
        this.settings.Table = 'country';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'listing/api/country';
        this.settings.ViewRouter = 'listing-country-view';
        this.settings.CreateRouter = 'listing-country-create';
        this.settings.DeleteApi = 'listing/api/country/delete';
      ...
      methods: {
        ...
        fetchData(){
          ...
          if (this.modelSearch.AreaID.trim() !== '') {
            requestData.data.AreaID = this.modelSearch.AreaID;
          }
          if (this.modelSearch.CountryName !== '') {
            requestData.data.CountryName = this.modelSearch.CountryName;
          }
