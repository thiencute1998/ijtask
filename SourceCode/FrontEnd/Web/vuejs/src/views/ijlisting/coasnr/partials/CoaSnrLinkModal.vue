<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-snr-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaSnrLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaSnrLinkContent>
...
<script>
  ...
  import CoaSnrLinkContent from "./CoaSnrLinkContent";

  export default {
    name: 'CoaSnrLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaSnr: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaSnrLink){
          console.log(this.$parent);
          this.$parent.showCoaSnrLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-snr/coa-snr-data-list',
          data: {
            CoaSnrLink : this.value,
            AccountID: this.CoaSnr.AccountID
          }
