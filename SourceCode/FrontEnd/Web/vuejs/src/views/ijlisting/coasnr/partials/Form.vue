        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Hệ thống tài khoản  dự trữ Quốc gia<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Hệ thống tài khoản  dự trữ Quốc gia<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.AccountName" type="text" id="AccountName" class="form-control" placeholder="Tên hệ thống tài khoản dự trữ Quốc gia" name="AccountName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParent v-model="model" :title="'Hệ thống tài khoản  dự trữ Quốc gia'" :api="'/listing/api/common/list'" :table="'coa_snr'" :fieldName="'AccountName'" :fieldNo="'AccountNo'" :fieldID="'AccountID'"  :placeholderInput="'Chọn hệ thống tài khoản  dự trữ Quốc gia cha'" :placeholderSearch="'Nhập tên HTTK QG'" :columnID="'AccountID'" :columnNo="'AccountNo'" :columnName="'AccountName'">
                </IjcoreModalSystemParent>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-9">
                <CoaSnrModalSearchInputCatelist
                  v-model="model.CoaSnrCate"
                  title-modal="Loại HTTK QG"
                  placeholder="Loại hệ thống tài khoản  dự trữ Quốc gia"
                ></CoaSnrModalSearchInputCatelist>
              </div>
...
import CoaSnrModalSearchInputCatelist from "@/views/ijlisting/coasnr/partials/CoaSnrModalSearchInputCatelist";
import IjcoreModalSystemParent from "../../../../components/IjcoreModalSystemParent";
import CoaSnrModalSearchEmployee from './CoaSnrModalSearchEmployee';

const ListRouter = 'listing-coa-snr';
const EditRouter = 'listing-coa-snr-edit';
const ViewRouter = 'listing-coa-snr-view';
const CreateRouter = 'listing-coa-snr-create';
const ViewApi = 'listing/api/coa-snr/view';
const EditApi = 'listing/api/coa-snr/edit';
const CreateApi = 'listing/api/coa-snr/create';
const StoreApi = 'listing/api/coa-snr/store';
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          if (_.isArray(responsesData.data.coaSnr)) {

            self.model.CoaSnrOption = [];
            _.forEach(responsesData.data.coaSnr, function (value, key) {
              let tmpObj = {};
              tmpObj.id = value.AccountID;
              tmpObj.text = value.AccountName;
              self.model.CoaSnrOption.push(tmpObj);
            });
          }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        data: {
          ...
          CoaSnrCate: this.model.CoaSnrCate,
    ...
    changeUserContact() {
      ...
      if (employee) {
        this.model.SnrtactName = employee.text;
      }
    }
