<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-sna-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaSnaLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaSnaLinkContent>
...
<script>
  ...
  import CoaSnaLinkContent from "./CoaSnaLinkContent";

  export default {
    name: 'CoaSnaLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaSna: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaSnaLink){
          console.log(this.$parent);
          this.$parent.showCoaSnaLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-sna/coa-sna-data-list',
          data: {
            CoaSnaLink : this.value,
            AccountID: this.CoaSna.AccountID
          }
