<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-sna-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-sna-per-view>
      <coa-sna-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaSnaOption="CoaSnaOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-sna-per-form>
...
import CoaSnaPerView from "./CoaSnaPerView";
import CoaSnaPerForm from "./CoaSnaPerForm";

export default {
  name: 'coa-sna-per-modal',
  components: {CoaSnaPerForm, CoaSnaPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-sna/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaSna', 'isDetail', 'per', 'EmployeeOption', 'CoaSnaOption', 'GroupOption'],
}
</script>
<style>
.readonly {
