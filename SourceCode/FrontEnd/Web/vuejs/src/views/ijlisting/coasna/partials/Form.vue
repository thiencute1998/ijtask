        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Hệ thống tài khoản  Quốc gia<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Hệ thống tài khoản  Quốc gia<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.AccountName" type="text" id="AccountName" class="form-control" placeholder="Tên hệ thống tài khoản Quốc gia" name="AccountName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParent v-model="model" :title="'Hệ thống tài khoản  Quốc gia'" :api="'/listing/api/common/list'" :table="'coa_sna'" :fieldName="'AccountName'" :fieldNo="'AccountNo'" :fieldID="'AccountID'"  :placeholderInput="'Chọn hệ thống tài khoản  Quốc gia cha'" :placeholderSearch="'Nhập tên HTTK QG'" :columnID="'AccountID'" :columnNo="'AccountNo'" :columnName="'AccountName'">
                </IjcoreModalSystemParent>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-9">
                <CoaSnaModalSearchInputCatelist
                  v-model="model.CoaSnaCate"
                  title-modal="Loại HTTK QG"
                  placeholder="Loại hệ thống tài khoản  Quốc gia"
                ></CoaSnaModalSearchInputCatelist>
              </div>
...
import CoaSnaModalSearchInputCatelist from "@/views/ijlisting/coasna/partials/CoaSnaModalSearchInputCatelist";
import IjcoreModalSystemParent from "../../../../components/IjcoreModalSystemParent";
import CoaSnaModalSearchEmployee from './CoaSnaModalSearchEmployee';

const ListRouter = 'listing-coa-sna';
const EditRouter = 'listing-coa-sna-edit';
const ViewRouter = 'listing-coa-sna-view';
const CreateRouter = 'listing-coa-sna-create';
const ViewApi = 'listing/api/coa-sna/view';
const EditApi = 'listing/api/coa-sna/edit';
const CreateApi = 'listing/api/coa-sna/create';
const StoreApi = 'listing/api/coa-sna/store';
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          if (_.isArray(responsesData.data.coaSna)) {

            self.model.CoaSnaOption = [];
            _.forEach(responsesData.data.coaSna, function (value, key) {
              let tmpObj = {};
              tmpObj.id = value.AccountID;
              tmpObj.text = value.AccountName;
              self.model.CoaSnaOption.push(tmpObj);
            });
          }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        data: {
          ...
          CoaSnaCate: this.model.CoaSnaCate,
    ...
    changeUserContact() {
      ...
      if (employee) {
        this.model.SnatactName = employee.text;
      }
    }
