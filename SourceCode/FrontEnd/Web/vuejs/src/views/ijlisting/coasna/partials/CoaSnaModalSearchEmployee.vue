<template>
  <div class="ijcore-modal-search-input" @mouseenter="onMouseenter">
    <b-input-group>
      <b-form-input placeholder="Chọn người liên hệ" :readonly="true" v-model="value.ContactName" class="input-company-employee readonly form-control"></b-form-input>
      <input type="hidden" :readonly="true" v-model="value.EmployeeID" class="readonly form-control"/>
    ...
    <b-modal ref="modal" id="modal">
      ...
      <b-input-group>
        <b-form-input v-model="search" placeholder="Nhập tên người liên hệ" class="readonly form-control" @change="fetchData"></b-form-input>
      ...
      <div class="table-responsive">
        <table class="table b-table table-hover table-sm b-table-selectable b-table-select-multi">
          ...
          <tr v-for="item in listtable" @click="updateForParent(item[tableName+'ID'], item[tableName+'Name'])">
            <td class="pr-3 tr-no" style="width: 80px;">{{item[tableName + 'No']}}</td>
            <!--            <td class="pr-3"><span :title="item[tableName + 'Name']">{{item[tableName + 'Name']}}</span></td>-->
            <td class="pr-3"><span :title="item.EmployeeName">{{item.EmployeeName}}</span></td>
          </tr>
          </tbody>
        </table>
      </div>
...
export default {
  name: 'CompanyModalSearchEmployee',
  mixins: [mixinLists],
  components: {
  },
  computed: {
  ...
  mounted() {
    let self = this;
    this.$el.querySelector('.input-company-employee').addEventListener('click', function () {
      self.onToggleModal();
    });
  },
  methods: {
    ...
    updateForParent: function (EmployeeID, EmployeeName) {
      this.value.EmployeeID = EmployeeID;
      this.value.ContactName = EmployeeName;
