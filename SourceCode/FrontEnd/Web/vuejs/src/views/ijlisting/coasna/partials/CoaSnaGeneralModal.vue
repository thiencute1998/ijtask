<template>
  <div>
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-coa-sna-general" size="xl">
      <template v-slot:modal-title>
        <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i> {{title}}
      </template>
      <coa-sna-general-form v-model="value" :coaSnaOptions="coaSnaOptions" :employeeOptions="employeeOptions" v-if="isForm"></coa-sna-general-form>
      <coa-sna-general-view :isDetail="true" v-model="value" v-else></coa-sna-general-view>
...
import CoaSnaGeneralView from "./CoaSnaGeneralView";
import CoaSnaGeneralForm from "./CoaSnaGeneralForm";

export default {
  name: 'CoaSnaGeneralModal',
  props: ['value','title'],
  components: {
    CoaSnaGeneralView,
    CoaSnaGeneralForm
  ...
  methods: {
    ...
    onUpdate(){
      let updateAPI = 'listing/api/coa-sna/update/';
      let requestData = {
        method: 'post',
        ...
        data: {
          ...
          CoaSnaCate: this.value.CoaSnaCate,
        }
      }
