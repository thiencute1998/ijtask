          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Loại hệ thống tài khoản bảo hiểm xã hội</span>
              </div>
            </b-col>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CateName', label: 'Tên loại hệ thống tài khoản bảo hiểm xã hội', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'coa_sia_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/coa-sia-cate-list';
    this.settings.DeleteApi = 'listing/api/coa-sia-cate-list/delete';
    this.settings.CreateRouter = 'listing-coasiacatelist-create';
    this.settings.EditRouter = 'listing-coasiacatelist-edit';
    this.settings.ViewRouter = 'listing-coasiacatelist-view';
  ...
  methods: {
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/coa-sia-cate-list/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: CateID,
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('coa-sia-cate-list');
      let request = {};
      if (this.modelSearch.CateID !== '') {
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/coa-sia-cate-list/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateCoaSia');
    },
  },
  watch: {
    currentPage() {
