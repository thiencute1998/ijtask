<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" scrollable id="modal-form-input-vendor-link" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{title}}
      </template>
      <vendor-link-content v-model="value" v-if="!isForm" :Vendor="Vendor"></vendor-link-content>
      <vendor-link-content v-model="value" :isForm="true" v-if="isForm" :Vendor="Vendor" :SysTable="SysTable"></vendor-link-content>
...
<script>
  ...
  import VendorLinkContent from "./VendorLinkContent";

  export default {
    name: 'vendor-link-modal',
    components: {
      VendorLinkContent
    },
    computed: {
    ...
    methods: {
      ...
      onToggleModal() {
        ...
        // this.$parent.onToggleVendorLink(false);
        this.currentPage = 1;
        this.$refs['modal'].show();
        if (!this.$parent.showVendorLink) this.$parent.showVendorLink = true;
      },
      onHideModalDataflow() {
        if (this.isDataflow) {
          this.$emit('onHideModalVendor');
        }
      },
      onResetModal() {
      },
      ...
      onUpdate() {
        ...
        const requestData = {
          method: 'post',
          url: 'listing/api/vendor/vendor-data-list',
          data: {
            VendorLink: self.value,
            VendorID: this.Vendor.VendorID
          }
        };
        // edit user
        requestData.data.ItemID = this.Vendor.VendorID;
