<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-vendor-general" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{this.title}}
      </template>
      <vendor-general-view v-model="value" v-if="!isForm" :isDetail="true"></vendor-general-view>
      <vendor-general-form v-model="value" v-if="isForm"></vendor-general-form>
...
<script>
  ...
  import VendorGeneralView from "./VendorGeneralView";
  import VendorGeneralForm from "./VendorGeneralForm";

  export default {
    name: 'vendor-general-modal',
    components: {VendorGeneralForm, VendorGeneralView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/vendor/update';
        const requestData = {
          method: 'post',
          url: UpdateApi,
          data: {
            VendorNo: this.value.VendorNo,
            VendorName: this.value.VendorName,
        ...
        if (this.value.VendorCate && this.value.VendorCate.length) {
          requestData.data.VendorCate = this.value.VendorCate;
        }
