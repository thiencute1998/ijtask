<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <vendor-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></vendor-per-view>
      <vendor-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :UserIdOption="UserIdOption" :EmployeeLogin="EmployeeLogin"></vendor-per-form>
...
<script>
  ...
  import VendorPerView from "./VendorPerView";
  import VendorPerForm from "./VendorPerForm";

  export default {
    name: 'vendor-per-modal',
    components: {VendorPerForm, VendorPerView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/vendor/update-per';
        ...
        // edit user
        requestData.url = UpdateApi + '/' + self.value.VendorID;
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'Vendor', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .readonly {
