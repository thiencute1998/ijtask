<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    <div style="position: relative;">
      <b-form-input v-model="selectedItem.name" @click="init" :readonly="true" :placeholder="placeholder" :disabled="disabled" :style="!disabled ? styleBackground : ''">
      </b-form-input>
      <i v-if="showIconClose" @click="onClearValue" class="cui-circle-x icons icon-clear"></i>
      <i class="fa fa-external-link icon-popup" @click="init" v-if="!disabled"></i>
    </div>
    <b-modal :id="idModal" :title="titleModal"
         :content-class="'sb-modal-content'"
         :size="sizeModal"
         @show="onShowModal" @hide="onHideModal($event)"
    >
      ...
      <div class="ijcore-modal-coa-con-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
            <tr>
              <th class="pr-3">Loại HTTKHN</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại HTTKHN'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'coa-con-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/coa-con/get-coa-con-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.CoaConCateList;
        ...
        self.CateValue = responsesData.data.CoaConCateValue;
...
<style lang="css">
  ...
  .ijcore-modal-coa-con-ccatelist .select2.select2-container{
    width: 100%;
  }
