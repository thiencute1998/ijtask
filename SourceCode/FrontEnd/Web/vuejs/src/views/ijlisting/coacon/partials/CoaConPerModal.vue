<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-con-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-con-per-view>
      <coa-con-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaConOption="CoaConOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-con-per-form>
...
import CoaConPerView from "./CoaConPerView";
import CoaConPerForm from "./CoaConPerForm";

export default {
  name: 'coa-con-per-modal',
  components: {CoaConPerForm, CoaConPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-con/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaCon', 'isDetail', 'per', 'EmployeeOption', 'CoaConOption', 'GroupOption'],
}
</script>
<style>
.readonly {
