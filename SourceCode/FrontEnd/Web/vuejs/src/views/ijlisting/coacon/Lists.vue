...
<script>
    ...
    import CoaConModalSearchLink from "./partials/CoaConModalSearchLink";
    import CoaConModalSearchEmployee from "./partials/CoaConModalSearchEmployee";
        ...
        data() {
            return {
              filter: {
                ...
                CoaConLink: (this.$route.query && this.$route.query.CoaConLink) ? this.$route.query.CoaConLink : [],
              }
            }
        },
        components:{
          CoaConModalSearchLink,
        ...
        methods: {
          ...
          handle (data){
            ...
            fetchData(){
              ...
              if (this.filter.CoaConLink.length) {
                let haveLink = false;
                _.forEach(this.filter.CoaConLink, function (coaConLink, key) {
                ...
                if (haveLink) {
                  requestData.data.CoaConLink = this.filter.CoaConLink;
                }
              }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCoaCon= this.getAllChildCoaCon(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.AccountID);
              }
              _.forEach(allChildCoaCon, function (coaCon, key) {

                let _index = _.findIndex(self.itemsArray, ['AccountNo', coaCon.AccountNo]);
                if (_index >= 0) {
                  if(coaCon.ParentID == item.AccountID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(coaCon.Detail == 0){
                      coaCon.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
            ...
            getAllChildCoaCon(item, coaconArr){
              let self = this, listChild = [];
              let allChild = _.filter(coaconArr, ['ParentID', item.AccountID]);
              if (allChild.length) {
                allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
                _.forEach(allChild, function (value, key) {
                  listChild.push(value);
                  if (_.filter(coaconArr, ['ParentID', value.AccountID]).length) {
                    let recursiveArr = self.getAllChildCoaCon(value, coaconArr);
