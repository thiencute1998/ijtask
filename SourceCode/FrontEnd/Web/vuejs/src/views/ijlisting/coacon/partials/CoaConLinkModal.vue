<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-con-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaConLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaConLinkContent>
...
<script>
  ...
  import CoaConLinkContent from "./CoaConLinkContent";

  export default {
    name: 'CoaConLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaCon: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaConLink){
          console.log(this.$parent);
          this.$parent.showCoaConLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-con/coa-con-data-list',
          data: {
            CoaConLink : this.value,
            AccountID: this.CoaCon.AccountID
          }
