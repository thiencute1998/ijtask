                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Chỉ tiêu dự toán</span>
                            </div>
                        </b-col>
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list' || stage.viewType === 'tree'">
                        <b-table ref="selectableTable"
                          ...
                          <template v-slot:cell(NormName)="data" v-if="stage.viewType === 'tree'">
                            <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                            <div class="bg-tree-content bg-tree-td"
                                 :id="'table-item-'+data.item.NormID"
                                 :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                              <span style="padding-left: 20px" :title="data.item.Name">{{data.item.NormName}}</span>
                              <span
                          ...
                          <template v-else>
                            <span :title="data.item.NormName">{{data.item.NormName}} </span>
                          </template>
                          <template v-slot:cell(NormType)="data">
                            <span :title="NormTypeOption[data.item.NormType]">{{NormTypeOption[data.item.NormType]}}</span>
                          </template>
                          <template v-slot:cell(UomID)="data">
                            <span :title="data.item.UomName">{{data.item.UomName}}</span>
                          </template>
                          <template v-slot:cell(NormLevelMap)="data">
<!--                            <span @click="setNormLevelMap($event, data.item)"><i class="fa fa-sitemap" aria-hidden="true"></i></span>-->
                            <norm-allot-map :norm="data.item"></norm-allot-map>
                          </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.NormName}}</span>
                              </div>
                              <div class="kanban-record">
                                <span>Mã số </span>
                                <span class="kanban-no">{{item.NormNo}}</span>
                              </div>
                              <div class="kanban-record">
                                <span>Loại </span>
                                <span class="kanban-no">{{NormTypeOption[item.NormType]}}</span>
                              </div>
                              <div class="kanban-record" v-if="item.UomName">
                                <span>Đơn vị tính </span>
                                <span class="kanban-no">{{item.UomName}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
    ...
    export default {
        ...
        data() {
            return {
              ...
              NormTypeOption: {1: 'Thu',2: 'Chi', 3: 'Thu & Chi'},
              NormTypeOptionSearch : [{value: null, text: '-- Tất cả --'}, {value: 1, text: 'Thu'}, {value: 2, text: 'Chi'}, {value: 3, text: 'Thu & Chi'}],
              UomOption: [],
            }
        },
        components:{
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                  ...
                  {key: 'NormNo', label: 'Mã chỉ tiêu', field: 'NormNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
                  {key: 'NormName', label: 'Tên chỉ tiêu', thStyle: 'min-width: 300px; z-index:7', field: 'NormName',tdClass: 'bg-tree-tr', searchOnTopRow: {type: 'text'}},
                  {key: 'NormType', label: 'Loại', field: 'NormType', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'select',option: this.NormTypeOptionSearch}},
                  {key: 'UomID', label: 'Đơn vị tính', field: 'UomID', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'modal-listing'}},
                  {key: 'NormLevelMap', label: '', thStyle: 'width: 5%'},
                ];
        ...
        created() {
          ...
          this.settings.FieldID = 'NormID';
          this.settings.Table = 'norm';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/norm';
          this.settings.DeleteApi = 'listing/api/norm/delete';
          this.settings.CreateRouter = 'listing-norm-create';
          this.settings.EditRouter = 'listing-norm-edit';
          this.settings.ViewRouter = 'listing-norm-view';

          this.modelSearch = {
            NormNo: (this.$route.query && this.$route.query.NormNo) ? this.$route.query.NormNo : '',
            NormName: (this.$route.query && this.$route.query.NormName) ? this.$route.query.NormName : '',
            NormType: (this.$route.query && this.$route.query.NormType) ? this.$route.query.NormType : null,
        ...
        methods: {
          ...
          fetchData(){
              ...
              if (this.modelSearch.NormNo.trim() !== '') {
                  requestData.data.NormNo = this.modelSearch.NormNo;
              }
              if (this.modelSearch.NormName !== '') {
                  requestData.data.NormName = this.modelSearch.NormName;
              }
              if (this.modelSearch.NormType !== null) {
                requestData.data.NormType = this.modelSearch.NormType;
              }
              if (this.modelSearch.UomID !== null) {
          ...
          onToggleChildren($event,item){
            ...
            let allChildNorm = this.getAllChildNorm(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.NormID);
              }
              _.forEach(allChildNorm, function (norm, key) {
                let _index = _.findIndex(self.itemsArray, ['NormNo', norm.NormNo]);
                if (_index >= 0) {
                  if(norm.ParentID == item.NormID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(norm.Detail == 0){
                      norm.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
          ...
          getAllChildNorm(item, normArr){
            let self = this, listChild = [];
            let allChild = _.filter(normArr, ['ParentID', item.NormID]);
            if (allChild.length) {
              allChild = _.orderBy(allChild, ['NormID'], ['asc']);
              _.forEach(allChild, function (value, key) {
                listChild.push(value);
                if (_.filter(normArr, ['ParentID', value.NormID]).length) {
                  let recursiveArr = self.getAllChildNorm(value, normArr);
                  recursiveArr = _.orderBy(recursiveArr, ['NormID', 'asc']);
          ...
          haveChildren(item){
            let children = _.filter(this.itemsArray, ['ParentID', item.NormID]);
          ...
          getListChild(NormID){
            ...
            let requestData = {
              method: 'post',
              url: 'listing/api/norm/get-list-child',
              data: {
                per_page: this.perPage,
                page: this.currentPage,
                ParentID: NormID,
            ...
            ApiService.customRequest(requestData).then((response) => {
              ...
              if (responseData.status === 1) {
                ...
                let keyParent = _.findIndex(self.itemsArray, ['NormID', NormID]);
                ...
                ApiService.customRequest(requestData).then((response) => {
                  ...
                  if (dataResponse.status === 1) {
                    ...
                    _.forEach(self.idsSelected, function (id, key) {
                      let index = _.findIndex(self.itemsArray,['NormID', id]);
                      let keyParent = _.findIndex(self.itemsArray, ['NormID',self.itemsArray[index].ParentID]);
