<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    <b-modal :id="idModal" :title="titleModal"
      ...
      <div class="ijcore-modal-normallot-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
          <tr>
            <th class="pr-3">Loại tiêu chí phân bổ dự toán</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại tiêu chí phân bổ dự toán'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'norm-allot-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/norm-allot/get-norm-allot-cate-list';
      ...
      ApiService.customRequest(requestData)
        .then(response =>{
          ...
          self.CateList = responsesData.data.NormAllotCateList;
          ...
          self.CateValue = responsesData.data.NormAllotCateValue;
...
.ijcore-modal-normallot-ccatelist .select2.select2-container{
  width: 100% !important;
}
