<template>
  <div class="component-norm-allot">
          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Tiêu chí phân bổ dự toán</span>
              </div>
            </b-col>
            ...
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              ...
              >
                ...
                <template slot="top-row" slot-scope="data">
                  <td :class="[(field.key == 'selected' ) ? 'pl-3' : '', (field.key == 'ItemName') ? 'pr-3' : ''+'no-overflow '] " role="cell" v-for="(field, key) in data.fields" style="z-index: 3" >
                    <!-- type input-->
                    <b-form-input
                      type="text" :placeholder="field.searchOnTopRow.placeholder"
                      v-if=" field.searchOnTopRow && field.searchOnTopRow.type == 'text' && key !== 0 && key != 3 && key != 4 & key!=5"
                      :name="field.field"
                ...
                <template v-slot:cell(NormAllotName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-'+data.item.NormAllotID"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.NormAllotName}}</span>
                    <span
                ...
                <template v-else>
                  <span :title="data.item.NormAllotName">{{data.item.NormAllotName}} </span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.NormAllotName}}</span>
                      <span class="kanban-no">{{item.NormAllotNo}}</span>
                    </div>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'NormAllotNo', label: 'Mã số', field: 'NormAllotNo', thStyle: 'width: 10%; min-width: 60px', searchOnTopRow: {type: 'text'}},
        {key: 'NormAllotName', label: 'Tên Định mức phân bổ dự toán', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'NormAllotName',searchOnTopRow: {type: 'text'}},
        {key: 'EffectiveDate', label: 'Ngày hiệu lực', headerTitle: 'Ngày hiệu lực', field: 'EffectiveDate',searchOnTopRow: {type: 'text'}, thStyle: 'min-width: 100px; text-align: center', tdClass: 'text-center'},
        {key: 'ExpirationDate', label: 'Ngày HHL', headerTitle: 'Ngày hết hiệu lực', field: 'ExpirationDate',searchOnTopRow: {type: 'text'}, thStyle: 'min-width: 100px; text-align: center', tdClass: 'text-center'},
        {key: 'UomName', label: 'Đơn vị tính', thStyle: ' min-width: 100px', field: 'UomName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'NormAllotID';
    this.settings.Table = 'norm_allot';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/norm-allot';
    this.settings.DeleteApi = 'listing/api/norm-allot/delete';
    this.settings.GetListChildApi = 'listing/api/norm-allot/get-list-child';
    this.settings.CreateRouter = 'listing-normallot-create';
    this.settings.EditRouter = 'listing-normallot-edit';
    this.settings.ViewRouter = 'listing-normallot-view';

    this.modelSearch = {
      NormAllotNo: (this.$route.query && this.$route.query.NormAllotNo) ? this.$route.query.NormAllotNo : '',
      NormAllotName: (this.$route.query && this.$route.query.NormAllotName) ? this.$route.query.NormAllotName : '',
      EffectiveDate: (this.$route.query && this.$route.query.EffectiveDate) ? this.$route.query.EffectiveDate : null,
      ExpirationDate:(this.$route.query && this.$route.query.ExpirationDate) ? this.$route.query.ExpirationDate: null,
      UomID : (this.$route.query && this.$route.query.UomID) ? this.$route.query.UomID: null,
      UomNo : (this.$route.query && this.$route.query.UomNo) ? this.$route.query.UomNo: '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.NormAllotNo.trim() !== '') {
        requestData.data.NormAllotNo = this.modelSearch.NormAllotNo;
      }
      if (this.modelSearch.NormAllotName !== '') {
        requestData.data.NormAllotName = this.modelSearch.NormAllotName;
      }
    ...
    onToggleChildren($event,item){
      ...
      let allChildNormAllot = this.getAllChildNormAllot(item, this.itemsArray);
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          self.$store.commit('isLoading', true);
          self.getListChild(item.NormAllotID);
        }
        _.forEach(allChildNormAllot, function (normallot, key) {
          let _index = _.findIndex(self.itemsArray, ['NormAllotNo', normallot.NormAllotNo]);
          if (_index >= 0) {
            if(normallot.ParentID == item.NormAllotID){
              self.$set(self.itemsArray[_index], 'hidden', false);
              if(normallot.Detail == 0){
                normallot.Class = 'fa fa-plus-square-o';
              }
            }
          }
        });
    ...
    getAllChildNormAllot(item, normallotArr){
      let self = this, listChild = [];
      let allChild = _.filter(normallotArr, ['ParentID', item.NormAllotID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['NormAllotID'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(normallotArr, ['ParentID', value.NormAllotID]).length) {
            let recursiveArr = self.getAllChildNormAllot(value, normallotArr);
            recursiveArr = _.orderBy(recursiveArr, ['NormAllotID', 'asc']);
    ...
    haveChildren(item){
      let children = _.filter(this.itemsArray, ['ParentID', item.NormAllotID]);
      if (children.length > 0) {
        return true;
      }
      return false;
    },
    getListChild(NormAllotID){
      let self = this;
      let requestData = {
        method: 'post',
        url: 'listing/api/norm-allot/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: NormAllotID,
        },
      };
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.itemsArray, ['NormAllotID', NormAllotID]);
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.NormAllotNo !== '') {
        request.NormAllotNo = this.modelSearch.NormAllotNo;
      }
      if (this.modelSearch.NormAllotName !== '') {
        request.NormAllotName = this.modelSearch.NormAllotName;
      }
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                let index = _.findIndex(self.itemsArray,['NormAllotID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['NormAllotID',self.itemsArray[index].ParentID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('normallot');
      let request = {};
      if (this.modelSearch.NormAllotNo !== '') {
        request.NormAllotNo = this.modelSearch.NormAllotNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/norm-allot/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'NormAllot');
    },
...
.component-norm-allot .component-compare-time .form-group .input-group .input-group-prepend{
  padding-top: 1px;
  height: 31px;
}
</style>
