<template>
  <div>
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-norm-allot-general" size="xl" @shown="onShownModal">
      <template v-slot:modal-title>
        <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i> {{title}}
      </template>
      <norm-allot-general-form v-model="value" :norm-allotOptions="normAllotOptions"  v-if="isForm"></norm-allot-general-form>
      <norm-allot-general-view :isDetail="true" v-model="value" v-else></norm-allot-general-view>
...
import NormAllotGeneralView from "./NormAllotGeneralView";
import NormAllotGeneralForm from "./NormAllotGeneralForm";

export default {
  name: 'NormAllotGeneralModal',
  props: ['value','title'],
  components: {
    NormAllotGeneralView,
    NormAllotGeneralForm
  },
  data() {
    return{
      isForm: false,
  ...
  methods: {
    ...
    onUpdate(){
      let updateAPI = 'listing/api/norm-allot/update/';
      let requestData = {
        method: 'post',
        url: updateAPI + this.value.NormAllotID,
        data: {
          NormAllotID: this.value.NormAllotID,
          NormAllotNo: this.value.NormAllotNo,
          NormAllotName: this.value.NormAllotName,
          ...
          EffectiveDate: this.value.EffectiveDate,
          ExpirationDate : this.value.ExpirationDate,
          Inactive: (this.value.Inactive) ? 1 : 0,
          NormAllotCate: this.value.NormAllotCate,
        ...
        .then(response=>{
          ...
          if(responsesData.status === 1){
            ...
            let item = {
              ...
              NormAllotNo: self.value.NormAllotNo,
              NormAllotName: self.value.NormAllotName,
              ...
              EffectiveDate : self.value.EffectiveDate,
              ExpirationDate: self.value.ExpirationDate,
              NormAllotCate: self.value.NormAllotCate
            }
            ...
            if(indexold >= 0 && self.$route.params.req.itemsArray){
              ...
              if(item.ParentID) {
                indexParent = _.findIndex(self.$route.params.req.itemsArray, {'NormAllotID': self.value.ParentID});
                if(indexParent >= 0 ){
              ...
              // set for ParentOld
              let indexParentOld = _.findIndex(self.$route.params.req.itemsArray, {'NormAllotID':  ParentOldID});
              if(indexParentOld >= 0){
                let ParentOld = self.$route.params.req.itemsArray[indexParentOld];
                let child = _.filter(self.$route.params.req.itemsArray, ['ParentID', ParentOld.NormAllotID]);
                if(child.length > 0){
