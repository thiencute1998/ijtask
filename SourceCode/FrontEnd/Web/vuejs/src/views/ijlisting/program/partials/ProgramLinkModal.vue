<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-program-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <ProgramLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></ProgramLinkContent>
...
<script>
  ...
  import ProgramLinkContent from "./ProgramLinkContent";

  export default {
    name: 'ProgramLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Program: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showProgramLink){
          console.log(this.$parent);
          this.$parent.showProgramLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/program/program-data-list',
          data: {
            ProgramLink : this.value,
