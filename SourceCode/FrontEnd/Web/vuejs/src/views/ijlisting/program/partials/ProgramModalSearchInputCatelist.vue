<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    >
      ...
      <div class="ijcore-modal-program-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          <thead>
            <tr>
              <th class="pr-3">Loại chương trình mục tiêu</th>
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại chương trình mục tiêu'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'program-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/program/get-program-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.ProgramCateList;
        ...
        self.CateValue = responsesData.data.ProgramCateValue;
...
<style lang="css">
  ...
  .ijcore-modal-program-ccatelist .select2.select2-container{
    width: 100%;
  }
