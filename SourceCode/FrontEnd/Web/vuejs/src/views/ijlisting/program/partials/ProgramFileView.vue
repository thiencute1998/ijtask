    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <program-file-edit :Program="Program" v-model="value[key]"></program-file-edit>
...
import ProgramFileEdit from "./ProgramFileEdit";

export default {
  name: 'ProgramFileView',
  components: {
    ProgramFileEdit,
    IjcoreUploadFile,
  ...
  methods: {
    ...
    downloadFile(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/program/download-file/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewFile(key) {
      ...
      let ValidProgramTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/program/download-file/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (ValidProgramTypes.indexOf(type) >= 0) {
          self.FileType = 1;
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/program/delete-file/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
