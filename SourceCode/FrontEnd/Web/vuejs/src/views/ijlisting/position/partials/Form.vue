        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Chức vụ<span v-if="model.PositionName">:</span> {{model.PositionName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Chức vụ<span v-if="model.PositionName">:</span> {{model.PositionName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-lg-16 col-md-14 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-2 app-object-name">
                <input v-model="model.PositionName" type="text" id="PositionName" class="form-control" placeholder="Tên chức vụ" name="PositionName" />
              </div>
              <div class="col-lg-4 col-md-6 col-sm-8 col-24 d-flex align-items-center app-object-code">
                <span>Mã số</span>
                <input type="text" v-model="model.PositionNo" class="form-control" placeholder="Mã số"/>
              </div>
            </div>
...
const ListRouter = 'listing-position';
const EditRouter = 'listing-position-edit';
const CreateRouter = 'listing-position-create';
const ViewRouter = 'listing-position-view';
const ViewApi = 'listing/api/position/view';
const EditApi = 'listing/api/position/edit';
const CreateApi = 'listing/api/position/create';
const StoreApi = 'listing/api/position/store';
const UpdateApi = 'listing/api/position/update';
const ListApi = 'listing/api/position';
...
export default {
  ...
  methods: {
        ...
        if (responsesData.status === 1 || !_.isEmpty(self.itemCopy)) {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              self.model.PositionNo = responsesData.data.data.PositionNo;
              self.model.PositionName = responsesData.data.data.PositionName;
            ...
            if (!_.isEmpty(self.itemCopy)) {
              self.model.PositionNo = responsesData.data.auto;
            }
          } else {
            self.model.PositionNo = responsesData.data.auto;
          }
    ...
    getItemIds(type){
      ...
      if (this.reqParams.search.PositionNo !== '') {
        requestData.data.PositionNo = this.reqParams.search.PositionNo;
      }
      if (this.reqParams.search.PositionName !== '') {
        requestData.data.PositionName = this.reqParams.search.PositionName;
      }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        url: StoreApi,
        data: {
          PositionNo: this.model.PositionNo,
