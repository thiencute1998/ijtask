          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Chức vụ</span>
              </div>
            </b-col>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.PositionNo}}</span>
                      <span class="kanban-no">{{item.PositionName}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'PositionNo', label: 'Mã chức vụ', field: 'PositionNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'PositionName', label: 'Tên chức vụ', thStyle: 'min-width: 300px', field: 'PositionName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'PositionID';
    this.settings.Table = 'position';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/position';
    this.settings.DeleteApi = 'listing/api/position/delete';
    this.settings.CreateRouter = 'listing-position-create';
    this.settings.EditRouter = 'listing-sector-edit';
    this.settings.ViewRouter = 'listing-position-view';
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.PositionNo.trim() !== '') {
        requestData.data.PositionNo = this.modelSearch.PositionNo;
      }
    ...
    async handleExportExcel() {
      ...
      let request = {};
      if (this.modelSearch.PositionNo !== '') {
        request.PositionNo = this.modelSearch.PositionNo;
      }
      if (this.modelSearch.PositionName !== '') {
