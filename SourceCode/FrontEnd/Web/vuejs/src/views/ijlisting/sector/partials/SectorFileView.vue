    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <sector-file-edit :Sector="Sector" v-model="value[key]"></sector-file-edit>
...
import SectorFileEdit from "./SectorFileEdit";
...
export default {
  ...
  methods: {
    ...
    downloadFile(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/sector/download-file/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewFile(key) {
      ...
      let ValidSectorTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/sector/download-file/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (ValidSectorTypes.indexOf(type) >= 0) {
          self.FileType = 1;
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/sector/delete-file/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
