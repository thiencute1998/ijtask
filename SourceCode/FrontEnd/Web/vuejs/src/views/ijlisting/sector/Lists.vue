          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Lĩnh vực chi</span>
              </div>
            </b-col>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.SectorName}}</span>
                      <span class="kanban-no">{{item.SectorNo}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
import SectorModalSearchLink from "./partials/SectorModalSearchLink";
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'SectorNo', label: 'Mã số', field: 'SectorNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'SectorName', label: 'Tên lĩnh vực chi', thStyle: 'min-width: 300px', field: 'SectorName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'SectorID';
    this.settings.Table = 'sector';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/sector';
    this.settings.DeleteApi = 'listing/api/sector/delete';
    this.settings.CreateRouter = 'listing-sector-create';
    this.settings.EditRouter = 'listing-sector-edit';
    this.settings.ViewRouter = 'listing-sector-view';

    this.modelSearch = {
      SectorNo : (this.$route.query && this.$route.query.SectorNo) ? this.$route.query.SectorNo : '',
      SectorName: (this.$route.query && this.$route.query.SectorName) ? this.$route.query.SectorName : '',
    };
    this.init();
  },
  updated() {
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.SectorNo.trim() !== '') {
        requestData.data.SectorNo = this.modelSearch.SectorNo;
      }
      if (this.modelSearch.SectorName !== '') {
        requestData.data.SectorName = this.modelSearch.SectorName;
      }
      ...
      if (this.filter.SectorLink.length) {
        let haveLink = false;
        _.forEach(this.filter.SectorLink, function (sectorLink, key) {
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.SectorNo !== '') {
        request.SectorNo = this.modelSearch.SectorNo;
      }
      if (this.modelSearch.SectorName !== '') {
        request.SectorName = this.modelSearch.SectorName;
      }
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('sector');
      let request = {};
      if (this.modelSearch.SectorNo !== '') {
        request.SectorNo = this.modelSearch.SectorNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/sector/get-report-data', request);
