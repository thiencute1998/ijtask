<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <sector-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></sector-per-view>
      <sector-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></sector-per-form>
...
import SectorPerView from "./SectorPerView";
import SectorPerForm from "./SectorPerForm";

export default {
  name: 'sector-per-modal',
  components: {SectorPerForm, SectorPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/sector/update-per';
      ...
      // edit user
      requestData.url = UpdateApi + '/' + self.value.SectorID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Sector', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
