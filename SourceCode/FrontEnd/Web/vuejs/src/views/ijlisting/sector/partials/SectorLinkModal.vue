<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-sector-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <SectorLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></SectorLinkContent>
...
<script>
  ...
  import SectorLinkContent from "./SectorLinkContent";

  export default {
    name: 'SectorLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Sector: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showSectorLink){
          console.log(this.$parent);
          this.$parent.showSectorLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/sector/sector-data-list',
          data: {
            SectorLink : this.value,
            SectorID: this.Sector.SectorID
          }
