          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Loại tiêu chí phân bổ dự toán</span>
              </div>
            </b-col>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        {key: 'selected', label: '', thStyle: 'width: 3%',tdClass: 'pl-3', thClass: 'pl-3'},
        {key: 'CateNo', label: 'Mã số', field: 'CateNo', thStyle: 'width: 10%; min-width: 60px', searchOnTopRow: {type: 'text'}},
        {key: 'CateName', label: 'Tên loại phân bổ định mức dư toán', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'norm_allot_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/norm-allot-cate-list';
    this.settings.DeleteApi = 'listing/api/norm-allot-cate-list/delete';
    this.settings.CreateRouter = 'listing-normallotcatelist-create';
    this.settings.EditRouter = 'listing-normallotcatelist-edit';
    this.settings.ViewRouter = 'listing-normallotcatelist-view';
  ...
  methods: {
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/norm-allot-cate-list/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: CateID,
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('norm-allot-cate-list');
      let request = {};
      if (this.modelSearch.CateID !== '') {
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/norm-allot-cate-list/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateNormAllot');
    },
  },
  watch: {
    currentPage() {
