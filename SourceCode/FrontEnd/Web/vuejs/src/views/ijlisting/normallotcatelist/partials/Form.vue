        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i>Loại tiêu chí phân bổ dự toán<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i>Loại tiêu chí phân bổ dự toán<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên tiêu chí phân bổ định mức dư toán" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'Loại tiêu chí phân bổ định mức dư toán'"
                  :api="'/listing/api/common/listCate'"
                  :table="'norm_allot'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại tiêu chí phân bổ định mức dự toán cha'"
                  :placeholderSearch="'Nhập loại tiêu chí phân bổ định mức dự toán'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.normAllotCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeNormAllotTableItem">
                <tr class="draggable" v-for="(field, key) in model.normAllotCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.normAllotCateValue[key].Description"
                      autocomplete="normAllot cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.normAllotCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.normAllotCateValue[key].CateValue"
                      autocomplete="normAllot cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.normAllotCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.normAllotCateValue[key].CateValue"
...
const ListRouter = 'listing-normallotcatelist';
const EditRouter = 'listing-normallotcatelist-edit';
const CreateRouter = 'listing-normallotcatelist-edit';
const ViewRouter = 'listing-normallotcatelist-view';
const ViewApi = 'listing/api/norm-allot-cate-list/view';
const EditApi = 'listing/api/norm-allot-cate-list/edit';
const CreateApi = 'listing/api/norm-allot-cate-list/create';
const StoreApi = 'listing/api/norm-allot-cate-list/store';
const UpdateApi = 'listing/api/norm-allot-cate-list/update';
const ListApi = 'listing/api/norm-allot-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.normAllotCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.normAllotCateValue = responsesData.data.normAllotCateValue;

              // set max lineID
              _.forEach(self.model.normAllotCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.normAllotCateValue[key].CateValue = Number(self.model.normAllotCateValue[key].CateValue);
                if (field.DataType == 2) self.model.normAllotCateValue[key].CateValue = String(self.model.normAllotCateValue[key].CateValue);
                if (field.DataType == 3) self.model.normAllotCateValue[key].CateValue = moment(self.model.normAllotCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.normAllotCateValue[key].CateValue = moment(self.model.normAllotCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.normAllotCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.normAllotCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.normAllotCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.normAllotCateValue
        }
      };
    ...
    changeNormAllotTableItem(moved) {},
  },
  watch: {
    idParams() {
      this.fetchData();
