                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Phòng ban</span>
                            </div>
                        </b-col>
                      ...
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                        <b-table ref="selectableTable"
                          ...
                          <template v-slot:cell(DepartmentName)="data">
                            <span :title="data.item.DepartmentName">{{data.item.DepartmentName}}</span>
                          </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.DepartmentName}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    export default {
      ...
      computed: {
          ...
          captions: function() {
              let fieldsTable = [
                  ...
                  {key: 'DepartmentName', label: 'Tên phòng ban',thStyle: 'min-width: 300px', field: 'DepartmentName',searchOnTopRow: {type: 'text'}},
              ];
      ...
      created() {
        this.settings.FieldID = 'DepartmentID';
        this.settings.Table = 'department';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'listing/api/department';
        this.settings.DeleteApi = 'listing/api/department/delete';
        this.settings.CreateRouter = 'listing-department-create';
        this.settings.EditRouter = 'listing-department-edit';
        this.settings.ViewRouter = 'listing-department-view';
      ...
      methods: {
        ...
        fetchData(){
            ...
            if (this.modelSearch.DepartmentName.trim() !== '') {
                requestData.data.DepartmentName = this.modelSearch.DepartmentName;
            }
        ...
        handleExportPrint() {
          ...
          if (this.modelSearch.DepartmentName !== '') {
            request.DepartmentName = this.modelSearch.DepartmentName;
          }
          ...
          this.$router.push({
            name: 'listing-department-report',
            query: request
          });
        },
