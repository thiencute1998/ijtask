        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại lĩnh vực thu<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại lĩnh vực thu<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên lĩnh vực thu" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại lĩnh vực thu'"
                  :api="'/listing/api/common/listCate'"
                  :table="'sbr_sector'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại lĩnh vực thu cha'"
                  :placeholderSearch="'Nhập loại lĩnh vực thu'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.sbrSectorCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeSbrSectorTableItem">
                <tr class="draggable" v-for="(field, key) in model.sbrSectorCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.sbrSectorCateValue[key].Description"
                      autocomplete="sbr sector cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.sbrSectorCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.sbrSectorCateValue[key].CateValue"
                      autocomplete="sbrSector cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.sbrSectorCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.sbrSectorCateValue[key].CateValue"
...
const ListRouter = 'listing-sbrsectorcatelist';
const EditRouter = 'listing-sbrsectorcatelist-edit';
const CreateRouter = 'listing-sbrsectorcatelist-edit';
const ViewRouter = 'listing-sbrsectorcatelist-view';
const ViewApi = 'listing/api/sbr-sector-cate-list/view';
const EditApi = 'listing/api/sbr-sector-cate-list/edit';
const CreateApi = 'listing/api/sbr-sector-cate-list/create';
const StoreApi = 'listing/api/sbr-sector-cate-list/store';
const UpdateApi = 'listing/api/sbr-sector-cate-list/update';
const ListApi = 'listing/api/sbr-sector-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.sbrSectorCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.sbrSectorCateValue = responsesData.data.sbrSectorCateValue;

              // set max lineID
              _.forEach(self.model.sbrSectorCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.sbrSectorCateValue[key].CateValue = Number(self.model.sbrSectorCateValue[key].CateValue);
                if (field.DataType == 2) self.model.sbrSectorCateValue[key].CateValue = String(self.model.sbrSectorCateValue[key].CateValue);
                if (field.DataType == 3) self.model.sbrSectorCateValue[key].CateValue = moment(self.model.sbrSectorCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.sbrSectorCateValue[key].CateValue = moment(self.model.sbrSectorCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.sbrSectorCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.sbrSectorCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.sbrSectorCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.sbrSectorCateValue
        }
      };
    ...
    changeSbrSectorTableItem(moved) {},
  },
  watch: {
    idParams() {
      this.fetchData();
