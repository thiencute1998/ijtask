        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại giao dịch bán hàng<span
                v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại giao dịch bán hàng<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
        ...
        <div class="container-fluid">
          <b-card>
            ...
            <div class="form-group row align-items-center">
              <label class="col-md-4 m-0" >Tên loại GDBH</label>
              <div class="col-md-20">
                <input v-model="model.CateName" type="text" id="CateName" class="form-control"
                       placeholder="Tên loại giao dịch bán hàng" name="CateName"/>
              </div>
            </div>
...
<!--                <Select2 v-model="model.ParentID" :options="model.customerCateListOption"></Select2>-->
                <IjcoreModalParentListing v-model="model" :title="'loại giá trị bán hàng'" :api="'/listing/api/common/list'"
                                    :table="'customer_sales_trans_cate_list'"
            ...
            <label>Giá trị loại giao dịch bán hàng:</label>
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <tbody>
              <tr v-for="(field, key) in model.customerSalesTransCateValue">
                <td>
                  <b-form-input
                    type="text"
                    v-model="model.customerSalesTransCateValue[key].CateValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                </td>
                <td>
                  <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                 v-model="model.customerSalesTransCateValue[key].DataType"></b-form-select>
                </td>
                <td>
                  <b-form-input
                    v-if="field.DataType == 1"
                    type="number"
                    v-model="model.customerSalesTransCateValue[key].ConvertedValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                  <b-form-input
                  ...
                  <b-form-select v-if="field.DataType == 5" v-model="model.customerSalesTransCateValue[key].ConvertedValue"
                                 :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                  <b-form-select v-if="field.DataType == 6" v-model="model.customerSalesTransCateValue[key].ConvertedValue"
...
<script>
  ...
  const ListRouter = 'listing-csalestranscatelist';
  const EditRouter = 'listing-csalestranscatelist-edit';
  const CreateRouter = 'listing-csalestranscatelist-create';
  const ViewRouter = 'listing-csalestranscatelist-view';
  const ViewApi = 'listing/api/customer-sales-trans-cate-list/view';
  const EditApi = 'listing/api/customer-sales-trans-cate-list/edit';
  const CreateApi = 'listing/api/customer-sales-trans-cate-list/create';
  const StoreApi = 'listing/api/customer-sales-trans-cate-list/store';
  const UpdateApi = 'listing/api/customer-sales-trans-cate-list/update';
  const ListApi = 'listing/api/customer-sales-trans-cate-list';
  ...
  export default {
    name: 'listing-csalestranscatelist-view',
    data() {
      return {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data.data)) {
                ...
                self.model.customerSalesTransCateValue = responsesData.data.customerSalesTransCateValue;
                // set max ValueID
                _.forEach(self.model.customerSalesTransCateValue, function (field, key) {
                  if (Number(field.ValueID) > self.model.maxValueID) self.model.maxValueID = Number(field.ValueID);
                  // set type of value
                  if (field.DataType == 1) self.model.customerSalesTransCateValue[key].ConvertedValue = Number(self.model.customerSalesTransCateValue[key].ConvertedValue);
                  if (field.DataType == 2) self.model.customerSalesTransCateValue[key].ConvertedValue = String(self.model.customerSalesTransCateValue[key].ConvertedValue);
                  if (field.DataType == 3) self.model.customerSalesTransCateValue[key].ConvertedValue = moment(self.model.customerSalesTransCateValue[key].ConvertedValue).format('DD/MM/YYYY');
                  if (field.DataType == 4) self.model.customerSalesTransCateValue[key].ConvertedValue = moment(self.model.customerSalesTransCateValue[key].ConvertedValue).format('DD/MM/YYYY hh:mm');
                });
            ...
            if (_.isArray(responsesData.data.customerSalesTransCateList)) {

              self.model.customerCateListOption = [];
              _.forEach(responsesData.data.customerSalesTransCateList, function (value, key) {
                let tmpObj = {};
                tmpObj.id = value.CateID;
                tmpObj.text = value.CateName;
                if (value.CateID == self.model.ParentID) self.model.ParentID = value.CateID;
      ...
      onAddFieldOnTable() {
        ...
        this.model.customerSalesTransCateValue.push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldOnTable(field) {

        // remove field in fieldOnTableReq
        let fieldExist = _.find(this.model.customerSalesTransCateValue, ['ValueID', field.ValueID]);
        if (_.isObject(fieldExist)) {
          _.remove(this.model.customerSalesTransCateValue, ['ValueID', field.ValueID]);
        }
        this.$forceUpdate();
      },
      handleSubmitForm() {
        ...
        const requestData = {
          ...
          data: {
            ...
            detail: this.model.customerSalesTransCateValue
          }
        };
