<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <fund-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></fund-per-view>
      <fund-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :FundOption="FundOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></fund-per-form>
...
import FundPerView from "./FundPerView";
import FundPerForm from "./FundPerForm";

export default {
  name: 'fund-per-modal',
  components: {FundPerForm, FundPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/fund/update-per';
      ...
      // edit user
      requestData.url = UpdateApi + '/' + self.value.FundID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Fund', 'isDetail', 'per', 'EmployeeOption', 'FundOption', 'GroupOption'],
}
</script>
<style>
.readonly {
