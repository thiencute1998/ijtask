<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-fund-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <FundLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></FundLinkContent>
...
<script>
  ...
  import FundLinkContent from "./FundLinkContent";
  ...
  export default {
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showFundLink){
          console.log(this.$parent);
          this.$parent.showFundLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/fund/fund-data-list',
          data: {
            FundLink : this.value,
