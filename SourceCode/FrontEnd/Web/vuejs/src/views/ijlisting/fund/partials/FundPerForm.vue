  ...
  <div>
    <div class="form-group row mt-2">
      ...
      <div class="col-lg-20 col-md-20 col-sm-20 mb-sm-2 mb-md-3 mb-lg-0 mb-3">
        <Select2 v-model="FundID" @change="getListEmployee('Fund')" :settings="{multiple: true, allowClear: true, placeholder: {id: null, text: 'Chọn đơn vị'}}" :options="FundOption"></Select2>
      </div>
    </div>
    ...
    <div class="table-responsive">
      <table class="not-border mb-2">
        ...
        <tr v-for="(field, key) in per" v-if="EmployeeLogin.EmployeeID != field.EmployeeID">
          ...
          <td class="pr-3">
            <fund-per-field-form :Fund="value" :disable="!per[key].Access" :keyPer="key" :title="'Phân quyền'" field-model="AccessField" :table="'doc'" v-model="field.AccessField" @updateFromChild="updateParent"></fund-per-field-form>
          </td>
          <td class="pr-3 text-center">
          ...
          <td class="pr-3">
            <fund-per-field-form :Fund="value" :disable="!per[key].Edit" :keyPer="key" :title="'Phân quyền'" field-model="EditField" :table="'doc'" v-model="field.EditField" @updateFromChild="updateParent"></fund-per-field-form>
          </td>
          <td class="pr-3 text-center">
...
import FundPerFieldView from "./FundPerFieldView";
import FundPerFieldForm from "./FundPerFieldForm";
import Select2 from 'v-select2-component';
import IjcoreModalMultiListing from "../../../../components/IjcoreModalMultiListing";

export default {
  name: 'FundPerForm',
  components: {IjcoreModalMultiListing, FundPerFieldForm, FundPerFieldView, Select2},
  computed: {
  ...
  methods: {
    getListEmployee(TypeSearch){
      ...
      if(TypeSearch == 'Fund'){
        requestData.data.FundID = self.FundID.join()
      }
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Task', 'isDetail', 'per', 'EmployeeOption', 'FundOption', 'GroupOption','EmployeeLogin'],
}
</script>
<style>
.mr-bottom-3 {
