                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Danh mục quỹ</span>
                            </div>
                        </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <fund-modal-search-link
                            v-model="filter.FundLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                        <b-table ref="selectableTable"
                          ...
                          <template v-slot:cell(FundName)="data">
                            <span :title="data.item.FundName">{{data.item.FundName}}</span>
                          </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.FundName}}</span>
                                <span class="kanban-no">{{item.FundNo}}</span>
                              </div>
...
<script>
    ...
    import FundModalSearchLink from "./partials/FundModalSearchLink";
    import FundModalSearchEmployee from "./partials/FundModalSearchEmployee";
    ...
    export default {
        ...
        data() {
            return {
              filter: {
                ...
                FundLink: (this.$route.query && this.$route.query.FundLink) ? this.$route.query.FundLink : [],
              }
            }
        },
        components:{
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'FundNo', label: 'Mã số', field: 'FundNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
                    {key: 'FundName', label: 'Tên danh mục quỹ', thStyle: 'min-width: 300px', field: 'FundName',searchOnTopRow: {type: 'text'}},
                ];
        ...
        created() {
          ...
          this.settings.FieldID = 'FundID';
          this.settings.Table = 'fund';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/fund';
          this.settings.DeleteApi = 'listing/api/fund/delete';
          this.settings.CreateRouter = 'listing-fund-create';
          this.settings.EditRouter = 'listing-fund-edit';
          this.settings.ViewRouter = 'listing-fund-view';

          this.modelSearch = {
            FundNo: (this.$route.query && this.$route.query.FundNo) ? this.$route.query.FundNo : '',
            FundName: (this.$route.query && this.$route.query.FundName) ? this.$route.query.FundName : '',
        ...
        methods: {
            ...
            fetchData(){
              ...
              if (this.modelSearch.FundNo.trim() !== '') {
                  requestData.data.FundNo = this.modelSearch.FundNo;
              }
              if (this.modelSearch.FundName !== '') {
                  requestData.data.FundName = this.modelSearch.FundName;
              }
              ...
              if (this.filter.FundLink.length) {
                let haveLink = false;
                _.forEach(this.filter.FundLink, function (fundLink, key) {
                  if (fundLink.LinkID) {
                    haveLink = true;
                    return false;
                  }
                ...
                if (haveLink) {
                  requestData.data.FundLink = this.filter.FundLink;
                }
              }
          ...
          handleExportPrint() {
            ...
            let request = {};
            if (this.modelSearch.FundNo !== '') {
              request.FundNo = this.modelSearch.FundNo;
            }
            if (this.modelSearch.FundName !== '') {
              request.FundName = this.modelSearch.FundName;
            }
            ...
            if (this.filter.FundLink.length) {
              ...
              if (haveLink) {
                request.FundLink = this.filter.FundLink;
              }
            }
            ...
            let reportJson = await this.$_lists_handleReportTemplate('fund');
            let request = {};
            if (this.modelSearch.FundNo !== '') {
              request.FundNo = this.modelSearch.FundNo;
            }
            ...
            let reportData = await this.$_lists_handleReportResponse('listing/api/fund/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'Fund');
          },
        },
        watch: {
            currentPage() {
