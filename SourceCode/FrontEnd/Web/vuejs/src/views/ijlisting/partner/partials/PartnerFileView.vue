    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <partner-file-edit :Partner="Partner" v-model="value[key]"></partner-file-edit>
...
<script>
  ...
  import PartnerFileEdit from "./PartnerFileEdit";
  ...
  export default {
    ...
    methods: {
      ...
      downloadFile(FileID, name) {
        let self = this;
        let urlApi = '/listing/api/partner/download-file/' + FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
      ...
      viewFile(key) {
        ...
        let ValidPartnerTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
        let self = this;
        let urlApi = '/listing/api/partner/download-file/' + this.value[key].FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
        ...
        ApiService.customRequest(requestData).then((response) => {
          ...
          if (ValidPartnerTypes.indexOf(type) >= 0) {
            self.FileType = 1;
      ...
      deleteLine(key) {
        let self = this;
        let urlApi = '/listing/api/partner/delete-file/' + self.value[key].LineID;
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {},
