          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Đối Tác</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <partner-modal-search-link
                    v-model="filter.PartnerLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(PartnerName)="data">
                  <span :title="data.item.PartnerName">{{data.item.PartnerName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.PartnerName}}</span>
                      <span class="kanban-no">{{item.PartnerNo}}</span>
                    </div>
                    <div class="kanban-record">
                      <span class="kanban-text" v-if="item.FullName && item.FullName !== ''">{{item.FullName}}</span>
                    </div>
                    <div class="kanban-record">
...
import PartnerModalSearchLink from "./partials/PartnerModalSearchLink";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        PartnerLink: (this.$route.query && this.$route.query.PartnerLink) ? this.$route.query.PartnerLink : [],
      }
    }
  },
  components:{
    PartnerModalSearchLink
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'PartnerNo', label: 'Mã  Đối Tác', thStyle: 'width: 12%; min-width: 100px', field: 'PartnerNo',searchOnTopRow: {type: 'text'}},
        {key: 'PartnerName', label: 'Tên Đối Tác', thStyle: 'width: 30%; min-width: 300px', field: 'PartnerName',searchOnTopRow: {type: 'text'}},
        {key: 'FullName', label: 'Tên Nhân Viên', thStyle: 'width: 23%; min-width: 150px', field: 'FullName',searchOnTopRow: {type: 'text'}},
        {key: 'Tel', label: 'Số Điện Thoại', field: 'Tel', thStyle: 'width: 15%; min-width: 150px', searchOnTopRow: {type: 'text'}},
  ...
  created() {
    this.settings.FieldID = 'PartnerID';
    this.settings.Table = 'partner';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/partner';
    this.settings.DeleteApi = 'listing/api/partner/delete';
    this.settings.CreateRouter = 'listing-partner-create';
    this.settings.EditRouter = 'listing-partner-edit';
    this.settings.ViewRouter = 'listing-partner-view';
    this.modelSearch = {
      PartnerNo: (this.$route.query && this.$route.query.PartnerNo) ? this.$route.query.PartnerNo : '',
      PartnerName: (this.$route.query && this.$route.query.PartnerName) ? this.$route.query.PartnerName : '',
      FullName: (this.$route.query && this.$route.query.FullName) ? this.$route.query.FullName : '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.PartnerNo !== '') {
        requestData.data.PartnerNo = this.modelSearch.PartnerNo;
      }
      if (this.modelSearch.PartnerName !== '') {
        requestData.data.PartnerName = this.modelSearch.PartnerName;
      }
      if (this.modelSearch.fullName !== '') {
      ...
      if (this.filter.PartnerLink.length) {
        let haveLink = false;
        _.forEach(this.filter.PartnerLink, function (partnerLink, key) {
        ...
        if (haveLink) {
          requestData.data.PartnerLink = this.filter.PartnerLink;
        }
      }
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('partner');
      let request = {};
      if (this.modelSearch.PartnerNo !== '') {
        request.PartnerNo = this.modelSearch.PartnerNo;
      }
      if (this.modelSearch.PartnerName !== '') {
        request.PartnerName = this.modelSearch.PartnerName;
      }
      if (this.modelSearch.Tel !== '') {
        request.Tel = this.modelSearch.Tel;
      }
      if (this.modelSearch.FullName !== '') {
        request.FullName = this.modelSearch.FullName;
      }
      if (this.modelSearch.Email !== '') {
      ...
      if (this.filter.PartnerLink.length) {
        ...
        if (haveLink) {
          request.PartnerLink = this.filter.PartnerLink;
        }
      }
      request.exportData = true;

      let reportData = await this.$_lists_handleReportResponse('listing/api/partner/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Partner');
    },
    handleExportPrint() {
