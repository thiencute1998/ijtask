    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <contract-video-edit :Contract="Contract" v-model="value[key]"></contract-video-edit>
...
import ContractVideoEdit from "./ContractVideoEdit";

export default {
  name: 'contract-video-view',
  components: {
    ContractVideoEdit,
    IjcoreUploadVideo,
  ...
  methods: {
    ...
    downloadVideo(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/contract/download-video/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewVideo(key) {
      ...
      var ValidContractTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/contract/download-video/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/contract/delete-video/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
