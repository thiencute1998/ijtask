    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <contract-file-edit :Contract="Contract" v-model="value[key]"></contract-file-edit>
...
import ContractFileEdit from "./ContractFileEdit";
...
export default {
  ...
  methods: {
    ...
    downloadFile(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/contract/download-file/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewFile(key) {
      ...
      let ValidContractTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/contract/download-file/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (ValidContractTypes.indexOf(type) >= 0) {
          self.FileType = 1;
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/contract/delete-file/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
