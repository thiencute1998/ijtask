<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-contract-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <ContractLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></ContractLinkContent>
...
<script>
  ...
  import ContractLinkContent from "./ContractLinkContent";

  export default {
    name: 'ContractLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Contract: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showContractLink){
          console.log(this.$parent);
          this.$parent.showContractLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/contract/contract-data-list',
          data: {
            ContractLink : this.value,
