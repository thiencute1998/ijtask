          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Hợp đồng</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <contract-modal-search-link
                    v-model="filter.ContractLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(ContractName)="data">
                  <span :title="data.item.ContractName">{{data.item.ContractName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record">
                      <span>Mã dự án</span>
                      <span class="kanban-no">{{item.ProjectNo}}</span>
                    </div>
                    <div class="kanban-record">
                      <span>Mã hợp đồng</span>
                      <span class="kanban-no">{{item.ContractNo}}</span>
                    </div>
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.ContractName}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
import ContractModalSearchLink from "./partials/ContractModalSearchLink";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        ContractLink: (this.$route.query && this.$route.query.ContractLink) ? this.$route.query.ContractLink : [],
        Employee: "",
      }
    }
  },
  components:{
    ContractModalSearchLink,
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'ProjectNo', label: 'Mã dự án', field: 'ProjectNo', thStyle: 'width: 15%; min-width: 150px',searchOnTopRow: {type: 'text'}},
        {key: 'ContractNo', label: 'Mã hợp đồng', field: 'ContractNo', thStyle: 'width: 15%; min-width: 150px', searchOnTopRow: {type: 'text'}},
        {key: 'ContractName', label: 'Tên hợp đồng', thStyle: 'min-width: 300px', field: 'ContractName',searchOnTopRow: {type: 'text', tdClass: 'pr-3'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'ContractID';
    this.settings.Table = 'contract';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/contract';
    this.settings.DeleteApi = 'listing/api/contract/delete';
    this.settings.CreateRouter = 'listing-contract-create';
    this.settings.EditRouter = 'listing-contract-edit';
    this.settings.ViewRouter = 'listing-contract-view';

    this.modelSearch = {
      ProjectNo: (this.$route.query && this.$route.query.ProjectNo) ? this.$route.query.ProjectNo : '',
      ContractNo: (this.$route.query && this.$route.query.ContractNo) ? this.$route.query.ContractNo : '',
      ContractName: (this.$route.query && this.$route.query.ContractName) ? this.$route.query.ContractName : '',
    };
    this.init();
  },
  updated() {
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.ContractNo.trim() !== '') {
        requestData.data.ContractNo = this.modelSearch.ContractNo;
      }
      if (this.modelSearch.ContractName !== '') {
        requestData.data.ContractName = this.modelSearch.ContractName;
      }
      ...
      if (this.filter.ContractLink.length) {
        let haveLink = false;
        _.forEach(this.filter.ContractLink, function (contractLink, key) {
        ...
        if (haveLink) {
          requestData.data.ContractLink = this.filter.ContractLink;
        }
      }
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.ContractNo !== '') {
        request.ContractNo = this.modelSearch.ContractNo;
      }
      if (this.modelSearch.ContractName !== '') {
        request.ContractName = this.modelSearch.ContractName;
      }
      if (this.modelSearch.ProjectNo !== '') {
      ...
      if (this.filter.ContractLink.length) {
        ...
        if (haveLink) {
          request.ContractLink = this.filter.ContractLink;
        }
      }
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('contract');
      let request = {};
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/contract/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Contract');
    },
  },
  watch: {
    currentPage() {
