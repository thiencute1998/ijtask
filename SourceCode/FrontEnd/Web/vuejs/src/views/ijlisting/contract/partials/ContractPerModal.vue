<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <contract-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></contract-per-view>
      <contract-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :ContractOption="ContractOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></contract-per-form>
...
import ContractPerView from "./ContractPerView";
import ContractPerForm from "./ContractPerForm";

export default {
  name: 'contract-per-modal',
  components: {ContractPerForm, ContractPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/contract/update-per';
      ...
      const requestData = {
        ...
        data: {
          ...
          ContractPer: self.per
        }
      };
      // edit user
      requestData.url = UpdateApi + '/' + self.value.ContractID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Contract', 'isDetail', 'per', 'EmployeeOption', 'ContractOption', 'GroupOption'],
}
</script>
<style>
.readonly {
