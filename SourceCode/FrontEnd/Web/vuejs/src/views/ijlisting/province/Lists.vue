          ...
          <b-row class="mb-2">
            <b-col md="12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Tỉnh - Thành Phố</span>
              </div>
            </b-col>
            <b-col md="12"></b-col>
          </b-row>
                ...
                <div class="col-md-4 col-sm-6 col-12 mb-2">
                  <ijcore-modal-search-input
                    ...
                    :select-fields-api="[
                                ...
                                {field: 'ProvinceName', fieldForSelected: 'name', showInTable: true, label: 'Tên vùng', key: 'ProvinceName', sortable: true, thClass: 'd-none'}
                              ]"
                    ...
                    :item-per-page="8"
                    placeholder="Vùng"
                    :url-api="$store.state.appRootApi + '/listing/api/province/get-province'"
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(ProvinceName)="data">
                  <span :title="data.item.ProvinceName">{{data.item.ProvinceName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.ProvinceName}}</span>
                      <span class="kanban-no">{{item.ProvinceID}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
<script>
  ...
  import IjcoreModalSearchInput from "@/components/IjcoreModalSearchInput";
    ...
    export default {
      ...
      computed: {
        ...
        captions: function() {
          let fieldsTable = [
            ...
            {key: 'ProvinceName', label: 'Tên tỉnh', field: 'ProvinceName',thStyle: 'min-width: 300px', searchOnTopRow: {type: 'text'} },
            {key: 'ProvinceNo', label: 'Mã bưu điện', thStyle: 'width: 12%; min-width: 100px', field: 'ProvinceID',searchOnTopRow: {type: 'text'}, tdClass:'pr-3'},
      ...
      created() {
        this.settings.FieldID = 'ProvinceID';
        this.settings.Table = 'province';
        this.settings.FieldInactive = 'Inactive';

        this.settings.ListApi = 'listing/api/province';
        this.settings.ViewRouter = 'listing-province-view';
        this.settings.CreateRouter = 'listing-province-create';
        this.settings.DeleteApi = 'listing/api/province/delete';
      ...
      methods: {
        ...
        fetchData(){
          ...
          if (this.modelSearch.Province.ProvinceID !== '') {
            requestData.data.ProvinceID = this.modelSearch.Province.ProvinceID;
          }

          if (this.modelSearch.ProvinceID.trim() !== '') {
            requestData.data.ProvinceID = this.modelSearch.ProvinceID;
          }
        ...
        handleExportPrint() {
          ...
          if (this.modelSearch.ProvinceNo !== '') {
            request.ProvinceNo = this.modelSearch.ProvinceNo;
          }
