<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-company-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CompanyLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CompanyLinkContent>
  ...
  export default {
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCompanyLink){
          console.log(this.$parent);
          this.$parent.showCompanyLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/company/company-data-list',
          data: {
            CompanyLink : this.value,
