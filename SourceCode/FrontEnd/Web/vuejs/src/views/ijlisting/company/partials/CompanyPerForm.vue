...
<!--          <th class="pr-3">Thêm</th>-->
        ...
        <tr v-for="(field, key) in per" v-if="field.EmployeeID != EmployeeLogin.EmployeeID">
          ...
          <td class="pr-3">
            <company-per-field-form :Company="value" :disable="!per[key].Access" :keyPer="key" :title="'Phân quyền'" field-model="AccessField" :table="'doc'" v-model="field.AccessField" @updateFromChild="updateParent"></company-per-field-form>
          </td>
          <td class="pr-3 text-center">
          ...
          <td class="pr-3">
            <company-per-field-form :Company="value" :disable="!per[key].Edit" :keyPer="key" :title="'Phân quyền'" field-model="EditField" :table="'doc'" v-model="field.EditField" @updateFromChild="updateParent"></company-per-field-form>
          </td>
          <td class="pr-3 text-center">
...
import CompanyPerFieldView from "./CompanyPerFieldView";
import CompanyPerFieldForm from "./CompanyPerFieldForm";
import Select2 from 'v-select2-component';
import IjcoreModalMultiListing from "../../../../components/IjcoreModalMultiListing";

export default {
  name: 'CompanyPerForm',
  components: {IjcoreModalMultiListing, CompanyPerFieldForm, CompanyPerFieldView, Select2},
  computed: {
          ...
          _.forEach(dataResponse.data, function (val, key) {
            ...
            if(isExist == 0){
              self.per.push({
                ...
                CompanyID: self.value.CompanyID,
                Access: true,
                AccessField: 'all',
                Edit: false,
                EditField: '',
