<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <company-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></company-per-view>
      <company-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :EmployeeLogin="EmployeeLogin"></company-per-form>
...
import CompanyPerView from "./CompanyPerView";
import CompanyPerForm from "./CompanyPerForm";

export default {
  name: 'company-per-modal',
  components: {CompanyPerForm, CompanyPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/company/update-per';
      ...
      // edit user
      requestData.url = UpdateApi + '/' + self.value.CompanyID;
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'Company', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
}
</script>
<style>
.readonly {
