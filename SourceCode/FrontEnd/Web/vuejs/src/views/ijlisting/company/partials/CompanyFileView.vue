    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <company-file-edit :Company="Company" v-model="value[key]"></company-file-edit>
...
export default {
  ...
  methods: {
    ...
    downloadFile(FileID, name) {
      let self = this;
      let urlApi = '/listing/api/company/download-file/' + FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
    ...
    viewFile(key) {
      ...
      let ValidCompanyTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
      let self = this;
      let urlApi = '/listing/api/company/download-file/' + this.value[key].FileID;
      let requestData = {
        method: 'get',
        url: urlApi,
        data: {},
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (ValidCompanyTypes.indexOf(type) >= 0) {
          self.FileType = 1;
    ...
    deleteLine(key) {
      let self = this;
      let urlApi = '/listing/api/company/delete-file/' + self.value[key].LineID;
      let requestData = {
        method: 'post',
        url: urlApi,
        data: {},
