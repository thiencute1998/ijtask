                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Đơn vị</span>
                            </div>
                        </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <company-modal-search-link
                            v-model="filter.CompanyLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list' || stage.viewType === 'tree'">
                        ...
                        >
                          ...
                          // cell name
                          <template v-slot:cell(CompanyName)="data" v-if="stage.viewType === 'tree'">
                            <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                            <div class="bg-tree-content bg-tree-td"
                                 :id="'table-item-'+data.item.CompanyID"
                                 :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                              <span style="padding-left: 20px" :title="data.item.Name">{{data.item.CompanyName}}</span>
                              <span
                          ...
                          <template v-else>
                            <span :title="data.item.CompanyName">{{data.item.CompanyName}} </span>
                          </template>

                          <template v-slot:cell(Email)="data">
                            <span :title="data.item.Email">{{data.item.Email}}</span>
                          </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.CompanyName}}</span>
                              </div>
                              <div class="kanban-record">
                                <span>Mã số </span>
                                <span class="kanban-no">{{item.CompanyNo}}</span>
                              </div>
                              <div class="kanban-record" v-if="item.Tel && item.Tel !== ''">
                                <span>ĐTCN</span>
                                <span class="kanban-text">{{item.Tel}}</span>
                              </div>
                              <div class="kanban-record" v-if="item.Fax && item.Fax !== ''">
                                <span>Số Fax </span>
                                <span class="kanban-text">{{item.Fax}}</span>
                              </div>
                              <div class="kanban-record" v-if="item.Email && item.Email !== ''">
                                <span>Email </span>
                                <span class="kanban-text">{{item.Email}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    import CompanyModalSearchLink from "./partials/CompanyModalSearchLink";
    ...
    export default {
        ...
        data() {
            return {
              filter: {
                ...
                CompanyLink: (this.$route.query && this.$route.query.CompanyLink) ? this.$route.query.CompanyLink : [],
              }
            }
        },
        components:{
          CompanyModalSearchLink,
        },
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'CompanyNo', label: 'Mã đơn vị', field: 'CompanyNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
                    {key: 'CompanyName', label: 'Tên đơn vị', thStyle: 'min-width: 300px; z-index:7',tdClass: 'bg-tree-tr', field: 'CompanyName',searchOnTopRow: {type: 'text'}},
                    {key: 'Tel', label: 'Số điện thoại', thStyle: 'width: 15%; min-width: 150px', field: 'Tel',searchOnTopRow: {type: 'text'}},
        ...
        created() {
          this.settings.FieldID = 'CompanyID';
          this.settings.Table = 'company';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/company';
          this.settings.DeleteApi = 'listing/api/company/delete';
          this.settings.CreateRouter = 'listing-company-create';
          this.settings.EditRouter = 'listing-company-edit';
          this.settings.ViewRouter = 'listing-company-view';

          this.modelSearch = {
            CompanyNo: (this.$route.query && this.$route.query.CompanyNo) ? this.$route.query.CompanyNo : '',
            CompanyName: (this.$route.query && this.$route.query.CompanyName) ? this.$route.query.CompanyName : '',
            Tel: (this.$route.query && this.$route.query.Tel) ? this.$route.query.Tel : '',
        ...
        methods: {
          ...
          fetchData(){
              ...
              if (this.modelSearch.CompanyName !== '') {
                  requestData.data.CompanyName = this.modelSearch.CompanyName;
              }
              ...
              if (this.filter.CompanyLink.length) {
                let haveLink = false;
                _.forEach(this.filter.CompanyLink, function (companyLink, key) {
                ...
                if (haveLink) {
                  requestData.data.CompanyLink = this.filter.CompanyLink;
                }
              }
          ...
          handleExportPrint() {
            ...
            let request = {};
            if (this.modelSearch.CompanyNo !== '') {
              request.CompanyNo = this.modelSearch.CompanyNo;
            }
            if (this.modelSearch.CompanyName !== '') {
              request.CompanyName = this.modelSearch.CompanyName;
            }
            if (this.modelSearch.Tel !== '') {
              request.Tel = this.modelSearch.Tel;
            }
            if (this.modelSearch.Fax !== '') {
            ...
            if (this.filter.CompanyLink.length) {
              ...
              if (haveLink) {
                request.CompanyLink = this.filter.CompanyLink;
              }
            }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCompany = this.getAllChildCompany(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.CompanyID);
              }
              _.forEach(allChildCompany, function (company, key) {
                let _index = _.findIndex(self.itemsArray, ['CompanyNo', company.CompanyNo]);
                if (_index >= 0) {
                  if(company.ParentID == item.CompanyID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(company.Detail == 0){
                      company.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
          ...
          getAllChildCompany(item, companyArr){
            let self = this, listChild = [];
            let allChild = _.filter(companyArr, ['ParentID', item.CompanyID]);
            if (allChild.length) {
              allChild = _.orderBy(allChild, ['CompanyID'], ['asc']);
              _.forEach(allChild, function (value, key) {
                listChild.push(value);
                if (_.filter(companyArr, ['ParentID', value.CompanyID]).length) {
                  let recursiveArr = self.getAllChildCompany(value, companyArr);
                  recursiveArr = _.orderBy(recursiveArr, ['CompanyID', 'asc']);
          ...
          haveChildren(item){
            let children = _.filter(this.itemsArray, ['ParentID', item.CompanyID]);
          ...
          getListChild(CompanyID){
            let self = this;
            let requestData = {
              method: 'post',
              url: 'listing/api/company/get-list-child',
              data: {
                per_page: this.perPage,
                page: this.currentPage,
                ParentID: CompanyID,
            ...
            ApiService.customRequest(requestData).then((response) => {
              ...
              if (responseData.status === 1) {
                ...
                let keyParent = _.findIndex(self.itemsArray, ['CompanyID', CompanyID]);
                ...
                ApiService.customRequest(requestData).then((response) => {
                  ...
                  if (dataResponse.status === 1) {
                    ...
                    _.forEach(self.idsSelected, function (id, key) {
                      let index = _.findIndex(self.itemsArray,['CompanyID', id]);
                      let keyParent = _.findIndex(self.itemsArray, ['CompanyID',self.itemsArray[index].ParentID]);
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('company');
            let request = {};
            if (this.modelSearch.CompanyNo !== '') {
              request.CompanyNo = this.modelSearch.CompanyNo;
            }
            ...
            let reportData = await this.$_lists_handleReportResponse('listing/api/company/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'Company');
          },
        },
        watch: {
          currentPage() {
