          ...
          <b-row class="mb-2">
            <b-col md="12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Vùng</span>
              </div>
            </b-col>
            <b-col md="12"></b-col>
          </b-row>
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(AreaName)="data">
                  <span :title="data.item.AreaName">{{data.item.AreaName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.AreaName}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      captions: function() {
        let fieldsTable = [
          ...
          {key: 'AreaName', label: 'Tên Vùng', field: 'AreaName', searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
        ];
    ...
    created() {
      this.settings.FieldID = 'AreaID';
      this.settings.Table = 'area';
      this.settings.FieldInactive = 'Inactive';
      this.settings.ListApi = 'listing/api/area';
      this.settings.ViewRouter = 'listing-area-view';
      this.settings.CreateRouter = 'listing-area-create';
      this.settings.DeleteApi = 'listing/api/area/delete';
      this.modelSearch = {
        AreaName: ''
      };
    ...
    methods: {
      ...
      fetchData(){
        ...
        if (this.modelSearch.AreaName !== '') {
          requestData.data.AreaName = this.modelSearch.AreaName;
        }
      ...
      handleExportPrint() {
        ...
        let request = {};
        if (this.modelSearch.AreaName !== '') {
          request.AreaName = this.modelSearch.AreaName;
        }
