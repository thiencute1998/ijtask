<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-eas-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-eas-per-view>
      <coa-eas-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaEasOption="CoaEasOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-eas-per-form>
...
import CoaEasPerView from "./CoaEasPerView";
import CoaEasPerForm from "./CoaEasPerForm";

export default {
  name: 'coa-eas-per-modal',
  components: {CoaEasPerForm, CoaEasPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-eas/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaEas', 'isDetail', 'per', 'EmployeeOption', 'CoaEasOption', 'GroupOption'],
}
</script>
<style>
.readonly {
