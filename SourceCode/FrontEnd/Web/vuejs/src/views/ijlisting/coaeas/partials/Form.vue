        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Hệ thống tài khoản hợp nhất<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Hệ thống tài khoản hợp nhất<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.AccountName" type="text" id="AccountName" class="form-control" placeholder="Tên hệ thống tài khoản hợp nhất" name="AccountName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-9">
                <CoaEasModalSearchInputCatelist
                  v-model="model.coaEasCate"
                  title-modal="Loại HTTKHN"
                  placeholder="Loại hệ thống tài khoản  hợp nhất"
                ></CoaEasModalSearchInputCatelist>
              </div>
...
import CoaEasModalSearchInputCatelist from "@/views/ijlisting/coaeas/partials/CoaEasModalSearchInputCatelist";
import IjcoreModalSystemParent from "../../../../components/IjcoreModalSystemParent";
import CoaEasModalSearchEmployee from './CoaEasModalSearchEmployee';

const ListRouter = 'listing-coa-con';
const EditRouter = 'listing-coa-con-edit';
const ViewRouter = 'listing-coa-con-view';
const CreateRouter = 'listing-coa-con-create';
const ViewApi = 'listing/api/coa-con/view';
const EditApi = 'listing/api/coa-con/edit';
const CreateApi = 'listing/api/coa-con/create';
const StoreApi = 'listing/api/coa-con/store';
    ...
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          if (_.isArray(responsesData.data.coaEas)){

            self.model.coaEasOption = [];
            _.forEach(responsesData.data.coaEas, function (value, key) {
              let tmpObj = {};
              tmpObj.id = value.AccountID;
              tmpObj.text = value.AccountName;
              self.model.coaEasOption.push(tmpObj);
            });
          }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        data: {
          ...
          coaEasCate: this.model.coaEasCate,
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          ...
          let item = {
            ...
            coaEasCate: self.model.coaEasCate,
          }
