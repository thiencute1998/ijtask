<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-eas-link" scrollable size="xl">
      <template slot="modal-title">
        <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaEasLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaEasLinkContent>
...
import CoaEasLinkContent from "./CoaEasLinkContent";

export default {
  name: 'CoaEasLinkModal',
  props: {
    value: [Array, Object],
    title: {},
    CoaEas: {}
  ...
  methods: {
    ...
    onToggleModal(){
      ...
      if(this.$parent.showCoaEasLink){
        console.log(this.$parent);
        this.$parent.showCoaEasLink = true;
      }
    },
    onEdit(){
      this.fetchData();
    ...
    onUpdate(){
      ...
      let requestData = {
        method: 'post',
        url: '/listing/api/coa-eas/coa-eas-data-list',
        data: {
          CoaEasLink : this.value,
          AccountID: this.CoaEas.AccountID
        }
