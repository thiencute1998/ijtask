          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Hệ thống tài khoản hợp nhất</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <coa-con-modal-search-link
                    v-model="filter.CoaEasLink"
...
import CoaEasModalSearchLink from "./partials/CoaEasModalSearchLink";
import CoaEasModalSearchEmployee from "./partials/CoaEasModalSearchEmployee";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        CoaEasLink: (this.$route.query && this.$route.query.CoaEasLink) ? this.$route.query.CoaEasLink : [],
      }
    }
  },
  components:{
    CoaEasModalSearchLink,
    CoaEasModalSearchEmployee
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'AccountName', label: 'Tên Hệ thống tài khoản hợp nhất', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'AccountName',searchOnTopRow: {type: 'text'}},
      ];
      return fieldsTable;
    }
  },
  created() {
    ...
    this.settings.Table = 'coa_eas';
    this.settings.FieldInactive = 'Inactive';
    this.settings.ListApi = 'listing/api/coa-con';
    this.settings.DeleteApi = 'listing/api/coa-con/delete';
    this.settings.GetListChildApi = 'listing/api/coa-con/get-list-child';
    this.settings.CreateRouter = 'listing-coa-con-create';
    this.settings.EditRouter = 'listing-coa-con-edit';
    this.settings.ViewRouter = 'listing-coa-con-view';
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.filter.CoaEasLink.length) {
        let haveLink = false;
        _.forEach(this.filter.CoaEasLink, function (coaEasLink, key) {
        ...
        if (haveLink) {
          requestData.data.CoaEasLink = this.filter.CoaEasLink;
        }
      }
    ...
    onToggleChildren($event,item){
      ...
      let allChildCoaEas= this.getAllChildCoaEas(item, this.itemsArray);
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          self.$store.commit('isLoading', true);
          self.getListChild(item.AccountID);
        }
        _.forEach(allChildCoaEas, function (coaEas, key) {

          let _index = _.findIndex(self.itemsArray, ['AccountNo', coaEas.AccountNo]);
          if (_index >= 0) {
            if(coaEas.ParentID == item.AccountID){
              self.$set(self.itemsArray[_index], 'hidden', false);
              if(coaEas.Detail == 0){
                coaEas.Class = 'fa fa-plus-square-o';
              }
            }
          }
        });
    ...
    getAllChildCoaEas(item, coaeasArr){
      let self = this, listChild = [];
      let allChild = _.filter(coaeasArr, ['ParentID', item.AccountID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(coaeasArr, ['ParentID', value.AccountID]).length) {
            let recursiveArr = self.getAllChildCoaEas(value, coaeasArr);
    ...
    openReport(){
      this.$router.push({
        name: 'listing-coa-con-report-list'
      });
    },
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('coa-con');
      let request = {};
      ...
      if (this.filter.CoaEasLink.length) {
        ...
        if (haveLink) {
          request.CoaEasLink = this.filter.CoaEasLink;
        }
      }
      request.exportData = true;

      let reportData = await this.$_lists_handleReportResponse('listing/api/coa-con/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CoaEas');
    },
  },
  watch: {
    currentPage() {
