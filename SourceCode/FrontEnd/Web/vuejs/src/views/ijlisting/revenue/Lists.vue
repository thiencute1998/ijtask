          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Khoản  thu</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <revenue-modal-search-link
                    v-model="filter.RevenueLink"
                ...
                <div class="col-6 mb-2">
                  <revenue-modal-search-input-catelist
                    v-model="filter.RevenueCateList"
                    :listApi="'listing/api/revenue/get-revenue-cate-list'"
                    ref="myModalSearchInputCateList"
                    placeholder="Loại khoản thu"
                    @onSubmitSearch="handleSubmitSearch"
                    @onClear="fetchData"
                    title-modal="Loại khoản thu" size-modal="lg"
                    id-modal="myModalSearchInputCateList">
                  ></revenue-modal-search-input-catelist>
                </div>
                <b-col>
          ...
          <div class="content-body">
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              ...
              >
                ...
                <template v-slot:cell(RevenueName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-'+data.item.RevenueID"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.RevenueName}}</span>
                    <span
                ...
                <template v-else>
                  ...
                  <span :title="data.item.RevenueName">{{data.item.RevenueName}} </span>

                </template>
                <template v-slot:cell(RevenueMap) = "data">
                  <revenue-map :revenue="data.item" ></revenue-map>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.RevenueName}}</span>
                      <span class="kanban-no">{{item.RevenueNo}}</span>
                    </div>
...
import RevenueModalSearchLink from "./partials/RevenueModalSearchLink";
import RevenueModalSearchInputCatelist  from "./partials/RevenueModalSearchInputCatelist";
import RevenueMap from "./partials/RevenueMap";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        RevenueLink: (this.$route.query && this.$route.query.RevenueLink) ? this.$route.query.RevenueLink : [],
        RevenueCateList: (this.$route.query && this.$route.query.RevenueLink) ? this.$route.query.CateList : [],
      }
    }
  },
  components:{
    RevenueModalSearchLink,
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'RevenueNo', label: 'Mã số', field: 'RevenueNo', thStyle: 'width: 8%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'RevenueName', label: 'Tên Khoản  thu', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'RevenueName',searchOnTopRow: {type: 'text'}},
        {key:'RevenueMap', label: '%', thStyle: 'width: 5%'}
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'RevenueID';
    this.settings.Table = 'revenue';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/revenue';
    this.settings.DeleteApi = 'listing/api/revenue/delete';
    this.settings.GetListChildApi = 'listing/api/revenue/get-list-child';
    this.settings.CreateRouter = 'listing-revenue-create';
    this.settings.EditRouter = 'listing-revenue-edit';
    this.settings.ViewRouter = 'listing-revenue-view';

    this.modelSearch = {
      RevenueNo: (this.$route.query && this.$route.query.RevenueNo) ? this.$route.query.RevenueNo : '',
      RevenueName: (this.$route.query && this.$route.query.RevenueName) ? this.$route.query.RevenueName : '',
  ...
  methods: {
    ...
    handleDebugger(data){
     ...
     fetchData(){
      ...
      if (this.modelSearch.RevenueNo.trim() !== '') {
        requestData.data.RevenueNo = this.modelSearch.RevenueNo;
      }
      if (this.modelSearch.RevenueName !== '') {
        requestData.data.RevenueName = this.modelSearch.RevenueName;
      }
      if (this.stage.filterInactive !== 2) {
        requestData.data.Inactive = this.stage.filterInactive;
      }
      if (this.filter.RevenueCateList !== '') {
        requestData.data.RevenueCateList = this.filter.RevenueCateList;
      }
      if (this.fullTextSearch !== '') {
        requestData.data.fullTextSearch = this.fullTextSearch;
      }

      if (this.filter.RevenueLink.length) {
        let haveLink = false;
        _.forEach(this.filter.RevenueLink, function (revenueLink, key) {
        ...
        if (haveLink) {
          requestData.data.RevenueLink = this.filter.RevenueLink;
        }
      }
    ...
    onToggleChildren($event,item){
      ...
      let allChildRevenue = this.getAllChildRevenue(item, this.itemsArray);
      if (item.Class == 'fa fa-plus-square-o') {
        if(!self.haveChildren(item)){
          self.$store.commit('isLoading', true);
          self.getListChild(item.RevenueID);
        }
        _.forEach(allChildRevenue, function (revenue, key) {
          let _index = _.findIndex(self.itemsArray, ['RevenueNo', revenue.RevenueNo]);
          if (_index >= 0) {
            if(revenue.ParentID == item.RevenueID){
              self.$set(self.itemsArray[_index], 'hidden', false);
              if(revenue.Detail == 0){
                revenue.Class = 'fa fa-plus-square-o';
              }
            }
          }
        });
    ...
    getAllChildRevenue(item, revenueArr){
      let self = this, listChild = [];
      let allChild = _.filter(revenueArr, ['ParentID', item.RevenueID]);
      if (allChild.length) {
        allChild = _.orderBy(allChild, ['RevenueNo'], ['asc']);
        _.forEach(allChild, function (value, key) {
          listChild.push(value);
          if (_.filter(revenueArr, ['ParentID', value.RevenueID]).length) {
            let recursiveArr = self.getAllChildRevenue(value, revenueArr);
            recursiveArr = _.orderBy(recursiveArr, ['RevenueNo', 'asc']);
    ...
    getListChild(RevenueID){
      ...
      ApiService.customRequest(requestData).then((response) => {
        ...
        if (responseData.status === 1) {
          ...
          let keyParent = _.findIndex(self.itemsArray, ['RevenueID', RevenueID]);
    ...
    haveChildren(item){
      let children = _.filter(this.itemsArray, ['ParentID', item.RevenueID]);
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.RevenueNo !== '') {
        request.RevenueNo = this.modelSearch.RevenueNo;
      }
      if (this.modelSearch.RevenueName !== '') {
        request.RevenueName = this.modelSearch.RevenueName;
      }
      ...
      if (this.filter.RevenueLink.length) {
        ...
        if (haveLink) {
          request.RevenueLink = this.filter.RevenueLink;
        }
      }
      if(this.filter.RevenueCateList.length){
        request.RevenueCateList = this.filter.RevenueCateList
      }
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                debugger
                let index = _.findIndex(self.itemsArray,['RevenueID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['RevenueID',self.itemsArray[index].ParentID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('revenue');
      let request = {};
      if (this.modelSearch.RevenueNo !== '') {
        request.RevenueNo = this.modelSearch.RevenueNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/revenue/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Revenue');
    },
  },
  watch: {
    currentPage() {
