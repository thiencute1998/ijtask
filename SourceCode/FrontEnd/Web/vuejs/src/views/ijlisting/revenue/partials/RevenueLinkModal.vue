<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-revenue-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <RevenueLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></RevenueLinkContent>
...
<script>
  ...
  import RevenueLinkContent from "./RevenueLinkContent";

  export default {
    name: 'RevenueLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Revenue: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showRevenueLink){
          console.log(this.$parent);
          this.$parent.showRevenueLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/revenue/revenue-data-list',
          data: {
            RevenueLink : this.value,
            RevenueID: this.Revenue.RevenueID
          }
