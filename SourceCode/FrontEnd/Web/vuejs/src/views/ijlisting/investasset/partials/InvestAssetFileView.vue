    ...
    <table class="not-border">
      ...
      <tr v-for="(item, key) in value">
        ...
        <td style="text-align: center;width: auto; position: absolute; right: 15px;">
          ...
          <invest-asset-file-edit :Invest="Invest" v-model="value[key]"></invest-asset-file-edit>
...
<script>
  ...
  import InvestAssetFileEdit from "./InvestAssetFileEdit";
  ...
  export default {
    ...
    methods: {
      ...
      downloadFile(FileID, name) {
        let self = this;
        let urlApi = '/listing/api/invest-asset/download-file/' + FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
      ...
      viewFile(key) {
        ...
        let ValidInvestTypes = ["pptx", "ppt", "xls", "xlsx", "doc", "docx"];
        let self = this;
        let urlApi = '/listing/api/invest-asset/download-file/' + this.value[key].FileID;
        let requestData = {
          method: 'get',
          url: urlApi,
          data: {},
        ...
        ApiService.customRequest(requestData).then((response) => {
          ...
          if (ValidInvestTypes.indexOf(type) >= 0) {
            self.FileType = 1;
      ...
      deleteLine(key) {
        let self = this;
        let urlApi = '/listing/api/invest-asset/delete-file/' + self.value[key].LineID;
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {},
