        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Tài sản đầu tư<span v-if="model.InvestAssetName">:</span> {{model.InvestAssetName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Tài sản đầu tư<span v-if="model.InvestAssetName">:</span> {{model.InvestAssetName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-lg-17 col-md-21 col-sm-20 mb-sm-2 mb-md-3 mb-lg-0 mb-3 app-object-name">
                <input v-model="model.InvestAssetName" type="text" class="form-control" placeholder="Tên tài sản đầu tư"/>
              </div>
              <div class="col-lg-4 app-object-code d-md-none d-sm-none d-none d-lg-flex align-items-center">
                <span>Mã số</span>
                <input type="text" v-model="model.InvestAssetNo" class="form-control" placeholder="Mã số"/>
              </div>
            </div>
            ...
            <div class="form-group row align-items-center">
              <div class="col-lg-3 col-md-3 col-sm-4 mb-lg-0 mb-md-3 mb-sm-3" title="Loại tài sản đầu tư">Loại TSĐT</div>
              <div class="col-lg-21 col-md-21 col-sm-20 mb-lg-0 mb-md-3 mb-sm-3 mb-3">
                <invest-asset-modal-search-input-vcatelist
                  v-model="model.InvestAssetCate"
...
<script>
  ...
  import InvestAssetModalSearchInputVcatelist from "./partials/InvestAssetModalSearchInputVcatelist";
  ...
  const ViewRouter = 'listing-invest-asset-view';
  const ViewApi = 'listing/api/invest-asset/view';
  const EditApi = 'listing/api/invest-asset/edit';
  const CreateApi = 'listing/api/invest-asset/create';
  const StoreApi = 'listing/api/invest-asset/store';
  const UpdateApi = 'listing/api/invest-asset/update';
  const ListApi = 'listing/api/invest-asset';

  export default {
    name: 'listing-invest-asset-view',
    data () {
      return {
    ...
    components: {
      ...
      InvestAssetModalSearchInputVcatelist
    },
    beforeCreate() {},
    mounted() {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data)) {
                self.model.InvestAssetNo = responsesData.data.data.InvestAssetNo;
                self.model.InvestAssetName = responsesData.data.data.InvestAssetName;
                ...
                self.model.InvestAssetCate = responsesData.data.data.InvestAssetCate;
              }
              if (!_.isEmpty(self.itemCopy)) {
                self.model.InvestAssetNo = responsesData.data.auto;
              }
            } else {
              self.model.InvestAssetNo = responsesData.data.auto;
            }
      ...
      handleSubmitForm(){
        ...
        const requestData = {
          ...
          url: StoreApi + '',
          data: {
            InvestAssetNo: this.model.InvestAssetNo,
            InvestAssetName: this.model.InvestAssetName,
        ...
        if (this.model.InvestAssetCate && this.model.InvestAssetCate.length) {
          requestData.data.InvestAssetCate = this.model.InvestAssetCate;
        }

        // edit user
        if (this.idParams) {
          requestData.data.InvestAssetID = this.idParams;
          requestData.url = UpdateApi + '/' + this.idParams;
        }

        if (!_.isEmpty(this.itemCopy)) {
          requestData.data.CopyID = this.itemCopy.data.data.InvestAssetID;
        }
        ...
        ApiService.customRequest(requestData).then((responses) => {
          ...
          if (responsesData.status === 1) {
            ...
            self.$router.push({
              ...
              params: {id: responsesData.data.InvestAssetID, req: self.$route.params.req, message: 'Bản ghi đã được cập nhật!'}
            });
          } else {
