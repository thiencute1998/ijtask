<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <invest-asset-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></invest-asset-per-view>
      <invest-asset-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :UserIdOption="UserIdOption" :EmployeeLogin="EmployeeLogin"></invest-asset-per-form>
...
<script>
  ...
  import InvestAssetPerView from "./InvestAssetPerView";
  import InvestAssetPerForm from "./InvestAssetPerForm";

  export default {
    name: 'invest-asset-per-modal',
    components: {InvestAssetPerForm, InvestAssetPerView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/invest-asset/update-per';
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'InvestAsset', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .readonly {
