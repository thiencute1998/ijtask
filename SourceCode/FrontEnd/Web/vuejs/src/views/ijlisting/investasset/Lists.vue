                  ...
                  <b-row class="mb-2">
                    <b-col class="col-md-12">
                        <div class="main-header-item main-header-name">
                            <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Tài sản đầu tư</span>
                        </div>
                    </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <invest-asset-modal-search-link
                            v-model="filter.InvestAssetLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list' || stage.viewType === 'tree'">
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(InvestAssetName)="data">
                                <span :title="data.item.InvestAssetName">{{data.item.InvestAssetName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record">
                                <span>Mã số: </span>
                                <span class="kanban-text">{{item.InvestAssetNo}}</span>
                              </div>
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.InvestAssetName}}</span>
                              </div>
                              <div class="kanban-record">
                                <span>Đơn vị tính: </span>
                                <span class="kanban-text">{{item.UomName}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    import InvestAssetModalSearchLink from "./partials/InvestAssetModalSearchLink";
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
          return {
            filter: {
              ...
              InvestAssetLink: (this.$route.query && this.$route.query.InvestAssetLink) ? this.$route.query.InvestAssetLink : [],
            }
          }
        },
        components:{
          InvestAssetModalSearchLink
        },
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'InvestAssetNo', label: 'Mã số', thStyle: 'width: 6%; min-width: 100px', field: 'InvestAssetNo',searchOnTopRow: {type: 'text'}},
                    {key: 'InvestAssetName', label: 'Tên tài sản đầu tư', thStyle: 'width: 40%; min-width: 300px', field: 'InvestAssetName',searchOnTopRow: {type: 'text'}},
                    {key: 'UomName', label: 'Đơn vị tính', thStyle: 'width: 15%; min-width: 150px', field: 'UomName',searchOnTopRow: {type: 'text'}},
                ];
        ...
        created() {
          this.settings.FieldID = 'InvestAssetID';
          this.settings.Table = 'invest-asset';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/invest-asset';
          this.settings.DeleteApi = 'listing/api/invest-asset/delete' + '';
          this.settings.CreateRouter = 'listing-invest-asset-create';
          this.settings.EditRouter = 'listing-invest-asset-edit';
          this.settings.ViewRouter = 'listing-invest-asset-view';
          this.modelSearch = {
            InvestAssetNo: (this.$route.query && this.$route.query.InvestAssetNo) ? this.$route.query.InvestAssetNo : '',
            InvestAssetName: (this.$route.query && this.$route.query.InvestAssetName) ? this.$route.query.InvestAssetName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.InvestAssetNo !== '') {
                requestData.data.InvestAssetNo = this.modelSearch.InvestAssetNo;
            }
            if (this.modelSearch.InvestAssetName !== '') {
                requestData.data.InvestAssetName = this.modelSearch.InvestAssetName;
            }
            ...
            if (this.filter.InvestAssetLink.length) {
              let haveLink = false;
              _.forEach(this.filter.InvestAssetLink, function (investAssetLink, key) {
              ...
              if (haveLink) {
                requestData.data.InvestAssetLink = this.filter.InvestAssetLink;
              }
            }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCapital = this.getAllChildCapital(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.CapitalID);
              }
              _.forEach(allChildCapital, function (capital, key) {
                let _index = _.findIndex(self.itemsArray, ['CapitalNo', capital.CapitalNo]);
                if (_index >= 0) {
                  if(capital.ParentID == item.CapitalID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(capital.Detail == 0){
                      capital.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
          ...
          getAllChildCapital(item, capitalArr){
            let self = this, listChild = [];
            let allChild = _.filter(capitalArr, ['ParentID', item.CapitalID]);
            if (allChild.length) {
              allChild = _.orderBy(allChild, ['CapitalID'], ['asc']);
              _.forEach(allChild, function (value, key) {
                listChild.push(value);
                if (_.filter(capitalArr, ['ParentID', value.CapitalID]).length) {
                  let recursiveArr = self.getAllChildCapital(value, capitalArr);
                  recursiveArr = _.orderBy(recursiveArr, ['CapitalID', 'asc']);
          ...
          haveChildren(item){
            let children = _.filter(this.itemsArray, ['ParentID', item.CapitalID]);
          ...
          getListChild(CapitalID){
            ...
            ApiService.customRequest(requestData).then((response) => {
              ...
              if (responseData.status === 1) {
                ...
                let keyParent = _.findIndex(self.itemsArray, ['CapitalID', CapitalID]);
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('invest-asset');
            let request = {};
            if (this.modelSearch.InvestAssetNo !== '') {
              request.InvestAssetNo = this.modelSearch.InvestAssetNo;
            }
            if (this.modelSearch.InvestAssetName !== '') {
              request.InvestAssetName = this.modelSearch.InvestAssetName;
            }
            if (this.modelSearch.officePhone !== '') {
            ...
            if (this.filter.InvestAssetLink.length) {
              ...
              if (haveLink) {
                request.InvestAssetLink = this.filter.InvestAssetLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/invest-asset/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'invest-asset');
          },
          handleExportPrint() {
            ...
            if (this.modelSearch.UomName !== '') {
              request.UomName = this.modelSearch.UomName;
            }
            if (this.stage.filterInactive !== 2) {
              request.Inactive = this.stage.filterInactive;
            }
            ...
            request.exportData = true;
            this.$router.push({
              name: 'listing-invest-asset-report',
              query: request
            });
          },
