  ...
  <div>
    <div class="form-group row mt-2">
      ...
      <div class="col-lg-20 col-md-20 col-sm-20 mb-sm-2 mb-md-3 mb-lg-0 mb-3">
        <Select2 v-model="CompanyID" @change="getListEmployee('Company')" :settings="{multiple: true, allowClear: true, placeholder: {id: null, text: 'Chọn đơn vị'}}" :options="CompanyOption"></Select2>
      </div>
    </div>
    ...
    <div class="table-responsive">
      <table class="not-border mb-2">
        ...
        <tr v-for="(field, key) in per">
          ...
          <td class="pr-3">
            <invest-asset-per-field-form :InvestAsset="value" :disable="!per[key].Access" :keyPer="key" :title="'Phân quyền'" field-model="AccessField" :table="'doc'" v-model="field.AccessField" @updateFromChild="updateParent"></invest-asset-per-field-form>
          </td>
          <td class="pr-3 text-center">
          ...
          <td class="pr-3">
            <invest-asset-per-field-form :InvestAsset="value" :disable="!per[key].Edit" :keyPer="key" :title="'Phân quyền'" field-model="EditField" :table="'doc'" v-model="field.EditField" @updateFromChild="updateParent"></invest-asset-per-field-form>
          </td>
          <td class="pr-3 text-center">
          ...
          <td @click="deleteLine(key)" v-if="UserIdOption[field.EmployeeID] != value.UserIDCreated " ><i class="fa fa-trash-o" style="cursor: pointer; font-size: 18px"></i></td>
        </tr>
        </tbody>
      </table>
...
<script>
  ...
  import InvestAssetPerFieldView from "./InvestAssetPerFieldView";
  import InvestAssetPerFieldForm from "./InvestAssetPerFieldForm";
  import Select2 from 'v-select2-component';
  import IjcoreModalMultiListing from "../../../../components/IjcoreModalMultiListing";

  export default {
    name: 'InvestAssetPerForm',
    components: {IjcoreModalMultiListing, InvestAssetPerFieldForm, InvestAssetPerFieldView, Select2},
    computed: {},
    data() {
      return {
        OptionAccessType: {
    ...
    methods: {
      getListEmployee(TypeSearch){
        ...
        if(TypeSearch == 'Company'){
          requestData.data.CompanyID = self.CompanyID.join()
        }
            ...
            _.forEach(dataResponse.data, function (val, key) {
              ...
              if(isExist == 0){
                self.per.push({
                  ...
                  InvestAssetID: self.value.InvestAssetID,
                  Access: true,
                  AccessField: 'all',
                  Edit: false,
                  EditField: '',
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'Task', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption', 'UserIdOption','EmployeeLogin'],
  }
</script>
<style>
  .mr-bottom-3 {
