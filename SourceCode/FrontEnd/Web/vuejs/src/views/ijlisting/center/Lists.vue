          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Cơ quan trung ương</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <center-modal-search-link
                    v-model="filter.SectorLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
              <b-table ref="selectableTable"
                ...
                <template v-slot:cell(SectorName)="data">
                  <span :title="data.item.SectorName">{{data.item.SectorName}}</span>
                </template>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.CenterName}}</span>
                    </div>
                    <div class="kanban-record">
                      <span>Mã số </span>
                      <span class="kanban-no">{{item.CenterNo}}</span>
                    </div>
                    <div class="kanban-record" v-if="item.CenterTel && item.CenterTel !== ''">
                      <span>Tel</span>
                      <span class="kanban-text">{{item.CenterTel}}</span>
                    </div>
                    <div class="kanban-record" v-if="item.CenterFax && item.CenterFax !== ''">
                      <span>Số Fax </span>
                      <span class="kanban-text">{{item.CenterFax}}</span>
                    </div>
                    <div class="kanban-record" v-if="item.CenterEmail && item.CenterEmail !== ''">
                      <span>Email </span>
                      <span class="kanban-text">{{item.CenterEmail}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        SectorLink: (this.$route.query && this.$route.query.SectorLink) ? this.$route.query.SectorLink : [],
        Employee: "",
      }
    }
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CenterNo', label: 'Mã trung ương', field: 'CenterNo', thStyle: 'width: 12%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'CenterName', label: 'Tên trung ương', thStyle: 'min-width: 300px', field: 'CenterName',searchOnTopRow: {type: 'text'}},
        {key: 'CenterTel', label: 'Số điện thoại', thStyle: 'width: 15%; min-width: 150px', field: 'CenterTel',searchOnTopRow: {type: 'text'}},
        {key: 'CenterFax', label: 'Fax', field: 'CenterFax', thStyle: 'width: 15%; min-width: 150px', searchOnTopRow: {type: 'text'}},
        {key: 'CenterEmail', label: 'Email', field: 'CenterEmail', thStyle: 'width: 15%; min-width: 150px',searchOnTopRow: {type: 'text'}, tdClass: 'pr-3'},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'CenterID';
    this.settings.Table = 'center';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/center';
    this.settings.DeleteApi = 'listing/api/center/delete';
    this.settings.CreateRouter = 'listing-center-create';
    this.settings.EditRouter = 'listing-center-edit';
    this.settings.ViewRouter = 'listing-center-view';
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.CenterNo.trim() !== '') {
        requestData.data.CenterNo = this.modelSearch.CenterNo;
      }
      if (this.modelSearch.CenterName !== '') {
        requestData.data.CenterName = this.modelSearch.CenterName;
      }

      if (this.modelSearch.CenterTel !== '') {
        requestData.data.CenterTel = this.modelSearch.CenterTel;
      }
      if (this.modelSearch.CenterFax !== '') {
        requestData.data.CenterFax = this.modelSearch.CenterFax;
      }
      if (this.modelSearch.CenterEmail !== '') {
        requestData.data.CenterEmail = this.modelSearch.CenterEmail;
      }
      ...
      if (this.filter.SectorLink.length) {
        let haveLink = false;
        _.forEach(this.filter.SectorLink, function (centerLink, key) {
          if (centerLink.LinkID) {
            haveLink = true;
            return false;
          }
        });
        if (haveLink) {
          requestData.data.SectorLink = this.filter.SectorLink;
        }
      }
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.CenterNo !== '') {
        request.CenterNo = this.modelSearch.CenterNo;
      }
      if (this.modelSearch.CenterName !== '') {
        request.CenterName = this.modelSearch.CenterName;
      }
      if (this.modelSearch.CenterTel !== '') {
        request.CenterTel = this.modelSearch.CenterTel;
      }
      if (this.modelSearch.CenterFax !== '') {
        request.CenterFax = this.modelSearch.CenterFax;
      }
      if (this.modelSearch.CenterEmail !== '') {
        request.CenterEmail = this.modelSearch.CenterEmail;
      }
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('center');
      let request = {};
      if (this.modelSearch.CenterNo !== '') {
        request.CenterNo = this.modelSearch.CenterNo;
      }
      ...
      if (this.filter.SectorLink.length) {
        ...
        if (haveLink) {
          request.SectorLink = this.filter.SectorLink;
        }
      }
      request.exportData = true;

      let reportData = await this.$_lists_handleReportResponse('listing/api/center/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Sector');
    },
  },
  watch: {
    currentPage() {
