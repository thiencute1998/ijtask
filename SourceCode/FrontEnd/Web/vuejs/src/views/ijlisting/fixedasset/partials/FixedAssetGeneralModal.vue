<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-fixed-asset-general" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{this.title}}
      </template>
      <fixed-asset-general-view v-model="value" v-if="!isForm" :isDetail="true"></fixed-asset-general-view>
      <fixed-asset-general-form v-model="value" v-if="isForm"></fixed-asset-general-form>
...
<script>
  ...
  import FixedAssetGeneralView from "./FixedAssetGeneralView";
  import FixedAssetGeneralForm from "./FixedAssetGeneralForm";

  export default {
    name: 'fixed-asset-general-modal',
    components: {FixedAssetGeneralForm, FixedAssetGeneralView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/fixed-asset/update';
        const requestData = {
          method: 'post',
          url: UpdateApi,
          data: {
            FixedAssetNo: this.value.FixedAssetNo,
            FixedAssetName: this.value.FixedAssetName,
        ...
        if (this.value.FixedAssetCate && this.value.FixedAssetCate.length) {
          requestData.data.FixedAssetCate = this.value.FixedAssetCate;
        }
