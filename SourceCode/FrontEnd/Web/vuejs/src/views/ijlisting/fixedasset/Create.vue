        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Tài sản cố định<span v-if="model.FixedAssetName">:</span> {{model.FixedAssetName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Tài sản cố định<span v-if="model.FixedAssetName">:</span> {{model.FixedAssetName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-lg-17 col-md-21 col-sm-20 mb-sm-2 mb-md-3 mb-lg-0 mb-3 app-object-name">
                <input v-model="model.FixedAssetName" type="text" class="form-control" placeholder="Tên tài sản cố định"/>
              </div>
              <div class="col-lg-4 app-object-code d-md-none d-sm-none d-none d-lg-flex align-items-center">
                <span>Mã số</span>
                <input type="text" v-model="model.FixedAssetNo" class="form-control" placeholder="Mã số"/>
              </div>
            </div>

            <div class="form-group row align-items-center">
              <div class="col-lg-3 col-md-3 col-sm-4 mb-lg-0 mb-md-3 mb-sm-3" title="Số hiệu sản phẩm">Số hiệu SP</div>
              <div class="col-lg-9 col-md-9 col-sm-20 mb-lg-0 mb-md-3 mb-sm-3 mb-3">
                <input v-model="model.Serialnumber" type="text" class="form-control" placeholder="Số hiệu sản phẩm"/>
              </div>
              <div class="col-lg-3 col-md-3 col-sm-4" title="Đơn vị tính">Đơn vị tính</div>
            ...
            <div class="form-group row align-items-center">
              <div class="col-lg-3 col-md-3 col-sm-4 mb-lg-0 mb-md-3 mb-sm-3" title="Loại tài sản cố định">Loại TSCĐ</div>
              <div class="col-lg-21 col-md-21 col-sm-20 mb-lg-0 mb-md-3 mb-sm-3 mb-3">
                <fixed-asset-modal-search-input-vcatelist
                  v-model="model.FixedAssetCate"
...
<script>
  ...
  import FixedAssetModalSearchInputVcatelist from "./partials/FixedAssetModalSearchInputVcatelist";
  ...
  const ViewRouter = 'listing-fixed-asset-view';
  const ViewApi = 'listing/api/fixed-asset/view';
  const EditApi = 'listing/api/fixed-asset/edit';
  const CreateApi = 'listing/api/fixed-asset/create';
  const StoreApi = 'listing/api/fixed-asset/store';
  const UpdateApi = 'listing/api/fixed-asset/update';
  const ListApi = 'listing/api/fixed-asset';

  export default {
    name: 'listing-fixed-asset-view',
    data () {
      return {
    ...
    components: {
      ...
      FixedAssetModalSearchInputVcatelist
    },
    beforeCreate() {},
    mounted() {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data)) {
                self.model.FixedAssetNo = responsesData.data.data.FixedAssetNo;
                self.model.FixedAssetName = responsesData.data.data.FixedAssetName;
                ...
                self.model.FixedAssetCate = responsesData.data.data.FixedAssetCate;
              }
              if (!_.isEmpty(self.itemCopy)) {
                self.model.FixedAssetNo = responsesData.data.auto;
              }
            } else {
              self.model.FixedAssetNo = responsesData.data.auto;
            }
      ...
      handleSubmitForm(){
        ...
        const requestData = {
          ...
          url: StoreApi,
          data: {
            FixedAssetNo: this.model.FixedAssetNo,
            FixedAssetName: this.model.FixedAssetName,
        ...
        if (this.model.FixedAssetCate && this.model.FixedAssetCate.length) {
          requestData.data.FixedAssetCate = this.model.FixedAssetCate;
        }

        // edit user
        if (this.idParams) {
          requestData.data.FixedAssetID = this.idParams;
          requestData.url = UpdateApi + '/' + this.idParams;
        }

        if (!_.isEmpty(this.itemCopy)) {
          requestData.data.CopyID = this.itemCopy.data.data.FixedAssetID;
        }
        ...
        ApiService.customRequest(requestData).then((responses) => {
          ...
          if (responsesData.status === 1) {
            ...
            self.$router.push({
              ...
              params: {id: responsesData.data.FixedAssetID, req: self.$route.params.req, message: 'Bản ghi đã được cập nhật!'}
            });
          } else {
