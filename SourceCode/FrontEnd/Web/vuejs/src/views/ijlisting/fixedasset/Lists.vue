                  ...
                  <b-row class="mb-2">
                    <b-col class="col-md-12">
                        <div class="main-header-item main-header-name">
                            <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Tài sản cố định</span>
                        </div>
                    </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <fixed-asset-modal-search-link
                            v-model="filter.FixedAssetLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(FixedAssetName)="data">
                                <span :title="data.item.FixedAssetName">{{data.item.FixedAssetName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record">
                                <span>Mã số: </span>
                                <span class="kanban-text">{{item.FixedAssetNo}}</span>
                              </div>
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.FixedAssetName}}</span>
                              </div>
                              <div class="kanban-record">
...
<script>
    ...
    import FixedAssetModalSearchLink from "./partials/FixedAssetModalSearchLink";
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
          return {
            filter: {
              ...
              FixedAssetLink: (this.$route.query && this.$route.query.FixedAssetLink) ? this.$route.query.FixedAssetLink : [],
            }
          }
        },
        components:{
          FixedAssetModalSearchLink
        },
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'FixedAssetNo', label: 'Mã số', thStyle: 'width: 6%; min-width: 100px', field: 'FixedAssetNo',searchOnTopRow: {type: 'text'}},
                    {key: 'FixedAssetName', label: 'Tên tài sản cố định', thStyle: 'width: 40%; min-width: 300px', field: 'FixedAssetName',searchOnTopRow: {type: 'text'}},
        ...
        created() {
          this.settings.FieldID = 'FixedAssetID';
          this.settings.Table = 'fixed-asset';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/fixed-asset';
          this.settings.DeleteApi = 'listing/api/fixed-asset/delete' + '';
          this.settings.CreateRouter = 'listing-fixed-asset-create';
          this.settings.EditRouter = 'listing-fixed-asset-edit';
          this.settings.ViewRouter = 'listing-fixed-asset-view';
          this.modelSearch = {
            FixedAssetNo: (this.$route.query && this.$route.query.FixedAssetNo) ? this.$route.query.FixedAssetNo : '',
            FixedAssetName: (this.$route.query && this.$route.query.FixedAssetName) ? this.$route.query.FixedAssetName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.FixedAssetNo !== '') {
                requestData.data.FixedAssetNo = this.modelSearch.FixedAssetNo;
            }
            if (this.modelSearch.FixedAssetName !== '') {
                requestData.data.FixedAssetName = this.modelSearch.FixedAssetName;
            }
            ...
            if (this.filter.FixedAssetLink.length) {
              let haveLink = false;
              _.forEach(this.filter.FixedAssetLink, function (fixedAssetLink, key) {
              ...
              if (haveLink) {
                requestData.data.FixedAssetLink = this.filter.FixedAssetLink;
              }
            }
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('fixed-asset');
            let request = {};
            if (this.modelSearch.FixedAssetNo !== '') {
              request.FixedAssetNo = this.modelSearch.FixedAssetNo;
            }
            if (this.modelSearch.FixedAssetName !== '') {
              request.FixedAssetName = this.modelSearch.FixedAssetName;
            }
            if (this.modelSearch.officePhone !== '') {
            ...
            if (this.filter.FixedAssetLink.length) {
              ...
              if (haveLink) {
                request.FixedAssetLink = this.filter.FixedAssetLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/fixed-asset/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'fixed-asset');
          },
          ...
          handleExportPrint() {
            ...
            request.exportData = true;
            this.$router.push({
              name: 'listing-fixed-asset-report',
              query: request
            });
          },
