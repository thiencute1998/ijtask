  ...
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-task-file-edit" size="lg" no-close-on-backdrop>
      <template slot="modal-title">
        <i class="fa fa-edit"></i> Tệp: {{SbiChapter.SbiChapterName}}
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      onUpdate() {
        ...
        formData.append('SbiChapterID', self.value.SbiChapterID);
        ...
        // send upload request
        axios.post('listing/api/sbi-chapter/sbi-chapter-upload-file/' + self.SbiChapter.SbiChapterID, formData, config)
