        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Chương<span v-if="model.SbiChapterName">:</span> {{model.SbiChapterName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Chương<span v-if="model.SbiChapterName">:</span> {{model.SbiChapterName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-lg-17 col-md-21 col-sm-20 mb-sm-2 mb-md-3 mb-lg-0 mb-3 app-object-name">
                <input v-model="model.SbiChapterName" type="text" class="form-control" placeholder="Tên chương"/>
              </div>
              <div class="col-lg-4 app-object-code d-md-none d-sm-none d-none d-lg-flex align-items-center">
                <span>Mã số</span>
                <input type="text" v-model="model.SbiChapterNo" class="form-control" placeholder="Mã số"/>
              </div>
            </div>

            <div class="form-group row align-items-center">
              <div class="col-lg-3 col-md-3 col-sm-4 mb-lg-0 mb-md-3 mb-sm-3" title="Loại chương">Loại chương</div>
              <div class="col-lg-21 col-md-21 col-sm-20 mb-lg-0 mb-md-3 mb-sm-3 mb-3">
                <sbi-chapter-modal-search-input-vcatelist
                  v-model="model.SbiChapterCate"
...
<script>
  ...
  import SbiChapterModalSearchInputVcatelist from "./partials/SbiChapterModalSearchInputVcatelist";
  ...
  const ViewRouter = 'listing-sbi-chapter-view';
  const ViewApi = 'listing/api/sbi-chapter/view';
  const EditApi = 'listing/api/sbi-chapter/edit';
  const CreateApi = 'listing/api/sbi-chapter/create';
  const StoreApi = 'listing/api/sbi-chapter/store';
  const UpdateApi = 'listing/api/sbi-chapter/update';
  const ListApi = 'listing/api/sbi-chapter';

  export default {
    name: 'listing-sbi-chapter-view',
    data () {
      return {
    ...
    components: {
      ...
      SbiChapterModalSearchInputVcatelist
    },
    beforeCreate() {},
    mounted() {
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data)) {
                ...
                self.model.SbiChapterCate = responsesData.data.data.SbiChapterCate;
              }
              if (!_.isEmpty(self.itemCopy)) {
                self.model.SbiChapterNo = responsesData.data.auto;
              }
            } else {
              self.model.SbiChapterNo = responsesData.data.auto;
            }
      ...
      handleSubmitForm(){
        ...
        if (this.model.SbiChapterCate && this.model.SbiChapterCate.length) {
          requestData.data.SbiChapterCate = this.model.SbiChapterCate;
        }

        // edit user
        if (this.idParams) {
          requestData.data.SbiChapterID = this.idParams;
          requestData.url = UpdateApi + '/' + this.idParams;
        }

        if (!_.isEmpty(this.itemCopy)) {
          requestData.data.CopyID = this.itemCopy.data.data.SbiChapterID;
        }
        ...
        ApiService.customRequest(requestData).then((responses) => {
          ...
          if (responsesData.status === 1) {
            ...
            self.$router.push({
              ...
              params: {id: responsesData.data.SbiChapterID, req: self.$route.params.req, message: 'Bản ghi đã được cập nhật!'}
            });
          } else {
