                  ...
                  <b-row class="mb-2">
                    <b-col class="col-md-12">
                        <div class="main-header-item main-header-name">
                            <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Chương</span>
                        </div>
                    </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <sbi-chapter-modal-search-link
                            v-model="filter.SbiChapterLink"
                    ...
                    <div class="content-body">
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(SbiChapterName)="data">
                                <span :title="data.item.SbiChapterName">{{data.item.SbiChapterName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record">
                                <span>Mã số: </span>
                                <span class="kanban-text">{{item.SbiChapterNo}}</span>
                              </div>
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.SbiChapterName}}</span>
                              </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    import SbiChapterModalSearchLink from "./partials/SbiChapterModalSearchLink";
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
          return {
            filter: {
              ...
              SbiChapterLink: (this.$route.query && this.$route.query.SbiChapterLink) ? this.$route.query.SbiChapterLink : [],
            }
          }
        },
        components:{
          SbiChapterModalSearchLink
        },
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                  ...
                  {key: 'SbiChapterNo', label: 'Mã số', field: 'SbiChapterNo', thStyle: 'width: 5%; min-width: 60px', searchOnTopRow: {type: 'text'}},
                  {key: 'SbiChapterName', label: 'Tên chương', thStyle: 'min-width: 300px', field: 'SbiChapterName',searchOnTopRow: {type: 'text'}},
                ];
        ...
        created() {
          this.settings.FieldID = 'SbiChapterID';
          this.settings.Table = 'sbi-chapter';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/sbi-chapter';
          this.settings.DeleteApi = 'listing/api/sbi-chapter/delete' + '?XDEBUG_SESSION_START=PHPSTORM';
          this.settings.CreateRouter = 'listing-sbi-chapter-create';
          this.settings.EditRouter = 'listing-sbi-chapter-edit';
          this.settings.ViewRouter = 'listing-sbi-chapter-view';
          this.modelSearch = {
            SbiChapterNo: (this.$route.query && this.$route.query.SbiChapterNo) ? this.$route.query.SbiChapterNo : '',
            SbiChapterName: (this.$route.query && this.$route.query.SbiChapterName) ? this.$route.query.SbiChapterName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.SbiChapterNo !== '') {
                requestData.data.SbiChapterNo = this.modelSearch.SbiChapterNo;
            }
            if (this.modelSearch.SbiChapterName !== '') {
                requestData.data.SbiChapterName = this.modelSearch.SbiChapterName;
            }
            ...
            if (this.filter.SbiChapterLink.length) {
              let haveLink = false;
              _.forEach(this.filter.SbiChapterLink, function (fixedAssetLink, key) {
              ...
              if (haveLink) {
                requestData.data.SbiChapterLink = this.filter.SbiChapterLink;
              }
            }
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('sbi-chapter');
            let request = {};
            if (this.modelSearch.SbiChapterNo !== '') {
              request.SbiChapterNo = this.modelSearch.SbiChapterNo;
            }
            if (this.modelSearch.SbiChapterName !== '') {
              request.SbiChapterName = this.modelSearch.SbiChapterName;
            }
            if (this.modelSearch.officePhone !== '') {
            ...
            if (this.filter.SbiChapterLink.length) {
              ...
              if (haveLink) {
                request.SbiChapterLink = this.filter.SbiChapterLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/sbi-chapter/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'sbi-chapter');
          },
          ...
          handleExportPrint() {
            ...
            request.exportData = true;
            this.$router.push({
              name: 'listing-sbi-chapter-report',
              query: request
            });
          },
