<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <sbi-chapter-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></sbi-chapter-per-view>
      <sbi-chapter-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :UserIdOption="UserIdOption" :EmployeeLogin="EmployeeLogin"></sbi-chapter-per-form>
...
<script>
  ...
  import SbiChapterPerView from "./SbiChapterPerView";
  import SbiChapterPerForm from "./SbiChapterPerForm";

  export default {
    name: 'sbi-chapter-per-modal',
    components: {SbiChapterPerForm, SbiChapterPerView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/sbi-chapter/update-per';
        ...
        const requestData = {
          ...
          data: {
            ...
            SbiChapterPer: self.per
          }
        };
        // edit user
        requestData.url = UpdateApi + '/' + self.value.SbiChapterID + '?XDEBUG_SESSION_START=PHPSTORM';
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'SbiChapter', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .readonly {
