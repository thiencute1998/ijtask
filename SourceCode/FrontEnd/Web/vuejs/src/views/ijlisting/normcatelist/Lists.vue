...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CateName', label: 'Tên loại tiêu chí phân bổ dự toán', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'norm_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/norm-cate-list';
    this.settings.DeleteApi = 'listing/api/norm-cate-list/delete';
    this.settings.CreateRouter = 'listing-normcatelist-create';
    this.settings.EditRouter = 'listing-normcatelist-edit';
    this.settings.ViewRouter = 'listing-normcatelist-view';
  ...
  methods: {
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/norm-cate-list/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: CateID,
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('norm-cate-list');
      let request = {};
      if (this.modelSearch.CateID !== '') {
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/norm-cate-list/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateNorm');
    },
  },
  watch: {
    currentPage() {
