          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên tiêu chí phân bổ dự toán" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại tiêu chí phân bổ dự toán'"
                  :api="'/listing/api/common/listCate?XDEBUG_SESSION_START=PHPSTORM'"
                  :table="'norm'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại tiêu chí phân bổ dự toán cha'"
                  :placeholderSearch="'Nhập loại tiêu chí phân bổ dự toán'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.normCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeNormTableItem">
                <tr class="draggable" v-for="(field, key) in model.normCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.normCateValue[key].Description"
                      autocomplete="norm cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.normCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.normCateValue[key].CateValue"
                      autocomplete="norm cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.normCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.normCateValue[key].CateValue"
...
const ListRouter = 'listing-normcatelist';
const EditRouter = 'listing-normcatelist-edit';
const CreateRouter = 'listing-normcatelist-edit';
const ViewRouter = 'listing-normcatelist-view';
const ViewApi = 'listing/api/norm-cate-list/view';
const EditApi = 'listing/api/norm-cate-list/edit';
const CreateApi = 'listing/api/norm-cate-list/create';
const StoreApi = 'listing/api/norm-cate-list/store';
const UpdateApi = 'listing/api/norm-cate-list/update';
const ListApi = 'listing/api/norm-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.normCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.normCateValue = responsesData.data.normCateValue;

              // set max lineID
              _.forEach(self.model.normCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.normCateValue[key].CateValue = Number(self.model.normCateValue[key].CateValue);
                if (field.DataType == 2) self.model.normCateValue[key].CateValue = String(self.model.normCateValue[key].CateValue);
                if (field.DataType == 3) self.model.normCateValue[key].CateValue = moment(self.model.normCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.normCateValue[key].CateValue = moment(self.model.normCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.normCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.normCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.normCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.normCateValue
        }
      };
