                  ...
                  <b-row class="mb-2">
                    <b-col class="col-md-12">
                        <div class="main-header-item main-header-name">
                            <span class="text-capitalize"><i class="fa fa-list mr-2"></i> Vật tư - hàng hóa - dịch vụ</span>
                        </div>
                    </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <item-modal-search-link
                            v-model="filter.ItemLink"
                      ...
                      <div class="content-table content-body-list" v-if="stage.viewType === 'list'">
                            <b-table ref="selectableTable"
                              ...
                              <template v-slot:cell(ItemName)="data">
                                <span :title="data.item.ItemName">{{data.item.ItemName}}</span>
                              </template>
                          ...
                          <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                            <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                              <div class="kanban-record">
                                <span>Mã số: </span>
                                <span class="kanban-text">{{item.ItemNo}}</span>
                              </div>
                              <div class="kanban-record d-flex justify-content-between">
                                <span class="kanban-title">{{item.ItemName}}</span>
                              </div>
                              <div class="kanban-record">
...
<script>
    ...
    import ItemModalSearchLink from "./partials/ItemModalSearchLink";
    ...
    export default {
      ...
      mixins: [mixinLists],
        data() {
          return {
            filter: {
              ...
              ItemLink: (this.$route.query && this.$route.query.ItemLink) ? this.$route.query.ItemLink : [],
            }
          }
        },
        components:{
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'ItemNo', label: 'Mã số', thStyle: 'width: 5%; min-width: 60px; z-index: 7', field: 'ItemNo',searchOnTopRow: {type: 'text'}},
                    {key: 'ItemName', label: 'Tên vật tư - hàng hóa - dịch vụ', thStyle: 'min-width: 300px', field: 'ItemName',searchOnTopRow: {type: 'text'}},
                ];
        ...
        created() {
          this.settings.FieldID = 'ItemID';
          this.settings.Table = 'item';
          this.settings.FieldInactive = 'Inactive';

          this.settings.ListApi = 'listing/api/item';
          this.settings.DeleteApi = 'listing/api/item/delete' + '';
          this.settings.CreateRouter = 'listing-item-create';
          this.settings.EditRouter = 'listing-item-edit';
          this.settings.ViewRouter = 'listing-item-view';
          this.modelSearch = {
            ItemNo: (this.$route.query && this.$route.query.ItemNo) ? this.$route.query.ItemNo : '',
            ItemName: (this.$route.query && this.$route.query.ItemName) ? this.$route.query.ItemName : '',
        ...
        methods: {
          ...
          fetchData(){
            ...
            if (this.modelSearch.ItemNo !== '') {
                requestData.data.ItemNo = this.modelSearch.ItemNo;
            }
            if (this.modelSearch.ItemName !== '') {
                requestData.data.ItemName = this.modelSearch.ItemName;
            }
            ...
            if (this.filter.ItemLink.length) {
              let haveLink = false;
              _.forEach(this.filter.ItemLink, function (itemLink, key) {
                if (itemLink.LinkID) {
                  haveLink = true;
                  return false;
                }
              ...
              if (haveLink) {
                requestData.data.ItemLink = this.filter.ItemLink;
              }
            }
          ...
          async handleExportExcel() {
            ...
            let reportJson = await this.$_lists_handleReportTemplate('item');
            let request = {};
            if (this.modelSearch.ItemNo !== '') {
              request.ItemNo = this.modelSearch.ItemNo;
            }
            if (this.modelSearch.ItemName !== '') {
              request.ItemName = this.modelSearch.ItemName;
            }
            if (this.modelSearch.officePhone !== '') {
            ...
            if (this.filter.ItemLink.length) {
              ...
              if (haveLink) {
                request.ItemLink = this.filter.ItemLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/item/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'item');
          },
          handleExportPrint() {
