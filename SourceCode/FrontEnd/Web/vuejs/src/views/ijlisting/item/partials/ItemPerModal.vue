<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <item-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></item-per-view>
      <item-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CompanyOption="CompanyOption" :GroupOption="GroupOption" :UserIdOption="UserIdOption" :EmployeeLogin="EmployeeLogin"></item-per-form>
...
<script>
  ...
  import ItemPerView from "./ItemPerView";
  import ItemPerForm from "./ItemPerForm";

  export default {
    name: 'item-per-modal',
    components: {ItemPerForm, ItemPerView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/item/update-per';
    ...
    filters: {},
    props: ['title', 'value', 'name', 'api', 'table', 'Item', 'isDetail', 'per', 'EmployeeOption', 'CompanyOption', 'GroupOption'],
  }
</script>
<style>
  .readonly {
