<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-item-general" size="xl" @hide="onHideModalDataflow($event)">
      <template slot="modal-title">
        <i class="icon-eye icon" v-if="!isForm"></i><i class="fa fa-edit" v-if="isForm"></i> {{this.title}}
      </template>
      <item-general-view v-model="value" v-if="!isForm" :isDetail="true"></item-general-view>
      <item-general-form v-model="value" v-if="isForm"></item-general-form>
...
<script>
  ...
  import ItemGeneralView from "./ItemGeneralView";
  import ItemGeneralForm from "./ItemGeneralForm";

  export default {
    name: 'item-general-modal',
    components: {ItemGeneralForm, ItemGeneralView},
    computed: {},
    data() {
      return {
        isForm: false,
    ...
    methods: {
      ...
      onUpdate() {
        ...
        let UpdateApi = 'listing/api/item/update';
        const requestData = {
          method: 'post',
          url: UpdateApi,
          data: {
            ItemNo: this.value.ItemNo,
            ItemName: this.value.ItemName,
            ItemType: this.value.ItemType,
        ...
        // edit user
        requestData.url = UpdateApi + '/' + self.value.ItemID;

        if (this.value.ItemCate && this.value.ItemCate.length) {
          requestData.data.ItemCate = this.value.ItemCate;
        }
