                    ...
                    <b-row class="mb-2">
                        <b-col class="col-md-12">
                            <div class="main-header-item main-header-name">
                                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Hệ thống tài khoản xã</span>
                            </div>
                        </b-col>
                    ...
                    <div class="main-header-filter pt-2">
                      <div class="row mb-2">
                        <div class="col-6 mb-2">
                          <coa-scb-modal-search-link
                            v-model="filter.CoaScbLink"
...
<script>
    ...
    import CoaScbModalSearchLink from "./partials/CoaScbModalSearchLink";
    import CoaScbModalSearchEmployee from "./partials/CoaScbModalSearchEmployee";
    ...
    export default {
        ...
        data() {
            return {
              filter: {
                ...
                CoaScbLink: (this.$route.query && this.$route.query.CoaScbLink) ? this.$route.query.CoaScbLink : [],
              }
            }
        },
        components:{
          CoaScbModalSearchLink,
        ...
        computed: {
            ...
            captions: function() {
                let fieldsTable = [
                    ...
                    {key: 'AccountName', label: 'Tên Hệ thống tài khoản xã', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'AccountName',searchOnTopRow: {type: 'text'}},
                ];
                return fieldsTable;
            }
        },
        created() {
          ...
          this.settings.Table = 'coa_scb';
          this.settings.FieldInactive = 'Inactive';
          this.settings.ListApi = 'listing/api/coa-scb';
          this.settings.DeleteApi = 'listing/api/coa-scb/delete';
          this.settings.GetListChildApi = 'listing/api/coa-scb/get-list-child';
          this.settings.CreateRouter = 'listing-coa-scb-create';
          this.settings.EditRouter = 'listing-coa-scb-edit';
          this.settings.ViewRouter = 'listing-coa-scb-view';
        ...
        methods: {
          ...
          handle (data){
            ...
            fetchData(){
              ...
              if (this.filter.CoaScbLink.length) {
                let haveLink = false;
                _.forEach(this.filter.CoaScbLink, function (coaScbLink, key) {
                ...
                if (haveLink) {
                  requestData.data.CoaScbLink = this.filter.CoaScbLink;
                }
              }
          ...
          onToggleChildren($event,item){
            ...
            let allChildCoaScb= this.getAllChildCoaScb(item, this.itemsArray);
            if (item.Class == 'fa fa-plus-square-o') {
              if(!self.haveChildren(item)){
                self.$store.commit('isLoading', true);
                self.getListChild(item.AccountID);
              }
              _.forEach(allChildCoaScb, function (coaScb, key) {

                let _index = _.findIndex(self.itemsArray, ['AccountNo', coaScb.AccountNo]);
                if (_index >= 0) {
                  if(coaScb.ParentID == item.AccountID){
                    self.$set(self.itemsArray[_index], 'hidden', false);
                    if(coaScb.Detail == 0){
                      coaScb.Class = 'fa fa-plus-square-o';
                    }
                  }
                }
              });
            ...
            getAllChildCoaScb(item, coascbArr){
              let self = this, listChild = [];
              let allChild = _.filter(coascbArr, ['ParentID', item.AccountID]);
              if (allChild.length) {
                allChild = _.orderBy(allChild, ['AccountID'], ['asc']);
                _.forEach(allChild, function (value, key) {
                  listChild.push(value);
                  if (_.filter(coascbArr, ['ParentID', value.AccountID]).length) {
                    let recursiveArr = self.getAllChildCoaScb(value, coascbArr);
          ...
          getListChild(AccountID){
            ...
            let requestData = {
              method: 'post',
              url: 'listing/api/coa-scb/get-list-child',
              data: {
                per_page: this.perPage,
                page: this.currentPage,
                ParentID: AccountID,
            ...
            openReport(){
              this.$router.push({
                name: 'listing-coa-scb-report-list'
              });
            },
            ...
            let reportJson = await this.$_lists_handleReportTemplate('coa-scb');
            let request = {};
            ...
            if (this.filter.CoaScbLink.length) {
              ...
              if (haveLink) {
                request.CoaScbLink = this.filter.CoaScbLink;
              }
            }
            request.exportData = true;

            let reportData = await this.$_lists_handleReportResponse('listing/api/coa-scb/get-report-data', request);
            this.$_lists_handleDowloadExcel(reportJson, reportData, 'CoaScb');
          },
        },
      watch: {
        currentPage() {
