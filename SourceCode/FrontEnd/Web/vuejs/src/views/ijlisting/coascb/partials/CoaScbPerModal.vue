<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-scb-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-scb-per-view>
      <coa-scb-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaScbOption="CoaScbOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-scb-per-form>
...
import CoaScbPerView from "./CoaScbPerView";
import CoaScbPerForm from "./CoaScbPerForm";

export default {
  name: 'coa-scb-per-modal',
  components: {CoaScbPerForm, CoaScbPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-scb/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaScb', 'isDetail', 'per', 'EmployeeOption', 'CoaScbOption', 'GroupOption'],
}
</script>
<style>
.readonly {
