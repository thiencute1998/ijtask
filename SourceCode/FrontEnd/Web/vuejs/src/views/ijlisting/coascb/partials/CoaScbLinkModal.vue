<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-scb-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaScbLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaScbLinkContent>
...
<script>
  ...
  import CoaScbLinkContent from "./CoaScbLinkContent";

  export default {
    name: 'CoaScbLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaScb: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaScbLink){
          console.log(this.$parent);
          this.$parent.showCoaScbLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-scb/coa-scb-data-list',
          data: {
            CoaScbLink : this.value,
            AccountID: this.CoaScb.AccountID
          }
