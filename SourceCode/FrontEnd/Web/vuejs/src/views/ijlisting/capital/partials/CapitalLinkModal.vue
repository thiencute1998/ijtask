<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-capital-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CapitalLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CapitalLinkContent>
...
<script>
  ...
  import CapitalLinkContent from "./CapitalLinkContent";

  export default {
    name: 'CapitalLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      Capital: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCapitalLink){
          console.log(this.$parent);
          this.$parent.showCapitalLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/capital/capital-data-list',
          data: {
            CapitalLink : this.value,
            CapitalID: this.Capital.CapitalID
          }
