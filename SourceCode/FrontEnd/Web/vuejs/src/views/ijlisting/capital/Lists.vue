          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-capitalize"><i class="fa fa-list mr-2"></i>Nguồn vốn</span>
              </div>
            </b-col>
            ...
            <div class="main-header-filter pt-2">
              <div class="row mb-2">
                <div class="col-6 mb-2">
                  <capital-modal-search-link
                    v-model="filter.CapitalLink"
          ...
          <div class="content-body">
            <div class="content-table content-body-list"  v-if="stage.viewType === 'list' || stage.viewType === 'tree'" >
              ...
              >
                ...
                <template v-slot:cell(CapitalName)="data" v-if="stage.viewType === 'tree'">
                  <span class="bg-tree-dot" :style="{'left': (level * 17) + 'px'}" v-for="level in data.item.Level" ></span>
                  <div class="bg-tree-content bg-tree-td"
                       :id="'table-item-'+data.item.CapitalID"
                       :style="{'margin-left': (data.item.Level * 17 - 17) + 'px'}">
                    <span style="padding-left: 20px" :title="data.item.Name">{{data.item.CapitalName}}</span>
                    <span
                ...
                <template v-else>
                  ...
                  <span :title="data.item.CapitalName">{{data.item.CapitalName}} </span>
                ...
                <div class="kanban-item col-lg-6 col-md-8 col-sm-12 col-24" v-for="(item, key) in itemsArray">
                  <div class="kanban-item-inner" @click="$_lists_onFieldClicked(item)">
                    <div class="kanban-record d-flex justify-content-between">
                      <span class="kanban-title">{{item.CapitalName}}</span>
                      <span class="kanban-no">{{item.CapitalNo}}</span>
                    </div>
...
import CapitalModalSearchLink from "./partials/CapitalModalSearchLink";
...
export default {
  ...
  data() {
    return {
      filter: {
        ...
        CapitalLink: (this.$route.query && this.$route.query.CapitalLink) ? this.$route.query.CapitalLink : [],
      }
    }
  },
  components:{
    CapitalModalSearchLink,
  },
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CapitalNo', label: 'Mã số', field: 'CapitalNo', thStyle: 'width: 8%; min-width: 100px', searchOnTopRow: {type: 'text'}},
        {key: 'CapitalName', label: 'Tên Nguồn vốn', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CapitalName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.FieldID = 'CapitalID';
    this.settings.Table = 'capital';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/capital';
    this.settings.DeleteApi = 'listing/api/capital/delete';
    this.settings.GetListChildApi = 'listing/api/capital/get-list-child';
    this.settings.CreateRouter = 'listing-capital-create';
    this.settings.EditRouter = 'listing-capital-edit';
    this.settings.ViewRouter = 'listing-capital-view';

    this.modelSearch = {
      CapitalNo: (this.$route.query && this.$route.query.CapitalNo) ? this.$route.query.CapitalNo : '',
      CapitalName: (this.$route.query && this.$route.query.CapitalName) ? this.$route.query.CapitalName : '',
  ...
  methods: {
    ...
    fetchData(){
      ...
      if (this.modelSearch.CapitalNo.trim() !== '') {
        requestData.data.CapitalNo = this.modelSearch.CapitalNo;
      }
      if (this.modelSearch.CapitalName !== '') {
        requestData.data.CapitalName = this.modelSearch.CapitalName;
      }
      ...
      if (this.filter.CapitalLink.length) {
        let haveLink = false;
        _.forEach(this.filter.CapitalLink, function (capitalLink, key) {
        ...
        if (haveLink) {
          requestData.data.CapitalLink = this.filter.CapitalLink;
        }
      }
    ...
    handleExportPrint() {
      ...
      let request = {};
      if (this.modelSearch.CapitalNo !== '') {
        request.CapitalNo = this.modelSearch.CapitalNo;
      }
      if (this.modelSearch.CapitalName !== '') {
        request.CapitalName = this.modelSearch.CapitalName;
      }
      ...
      if (this.filter.CapitalLink.length) {
        ...
        if (haveLink) {
          request.CapitalLink = this.filter.CapitalLink;
        }
      }
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                debugger
                let index = _.findIndex(self.itemsArray,['CapitalID', id]);
                let keyParent = _.findIndex(self.itemsArray, ['CapitalID',self.itemsArray[index].ParentID]);
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('capital');
      let request = {};
      if (this.modelSearch.CapitalNo !== '') {
        request.CapitalNo = this.modelSearch.CapitalNo;
      }
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/capital/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'Capital');
    },
