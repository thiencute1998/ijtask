        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại giao dịch báo giá<span
                v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại giao dịch báo giá<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
        ...
        <div class="container-fluid">
          <b-card>
            ...
            <div class="form-group row align-items-center">
              <label class="col-md-4 m-0" >Tên loại GDBG</label>
              <div class="col-md-20">
                <input v-model="model.CateName" type="text" id="CateName" class="form-control"
                       placeholder="Tên loại giao dịch báo giá" name="CateName"/>
              </div>
            </div>
...
<!--                <Select2 v-model="model.ParentID" :options="model.customerCateListOption"></Select2>-->
                <IjcoreModalParentListing v-model="model" :title="'loại giá trị báo giá'" :api="'/listing/api/common/list'"
                                    :table="'customer_quotation_trans_cate_list'"
            ...
            <label>Giá trị loại giao dịch báo giá:</label>
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <tbody>
              <tr v-for="(field, key) in model.customerQuotationTransCateValue">
                <td>
                  <b-form-input
                    type="text"
                    v-model="model.customerQuotationTransCateValue[key].CateValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                </td>
                <td>
                  <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                 v-model="model.customerQuotationTransCateValue[key].DataType"></b-form-select>
                </td>
                <td>
                  <b-form-input
                    v-if="field.DataType == 1"
                    type="number"
                    v-model="model.customerQuotationTransCateValue[key].ConvertedValue"
                    autocomplete="Customer cate list description">
                  </b-form-input>
                  <b-form-input
                  ...
                  <b-form-select v-if="field.DataType == 5" v-model="model.customerQuotationTransCateValue[key].ConvertedValue"
                                 :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                  <b-form-select v-if="field.DataType == 6" v-model="model.customerQuotationTransCateValue[key].ConvertedValue"
...
<script>
  ...
  const ListRouter = 'listing-cquotationtranscatelist';
  const EditRouter = 'listing-cquotationtranscatelist-edit';
  const CreateRouter = 'listing-cquotationtranscatelist-create';
  const ViewRouter = 'listing-cquotationtranscatelist-view';
  const ViewApi = 'listing/api/customer-quotation-trans-cate-list/view';
  const EditApi = 'listing/api/customer-quotation-trans-cate-list/edit';
  const CreateApi = 'listing/api/customer-quotation-trans-cate-list/create';
  const StoreApi = 'listing/api/customer-quotation-trans-cate-list/store';
  const UpdateApi = 'listing/api/customer-quotation-trans-cate-list/update';
  const ListApi = 'listing/api/customer-quotation-trans-cate-list';
  ...
  export default {
    name: 'listing-cquotationtranscatelist-view',
    data() {
      return {
        ...
        model: {
          CateName: '',
          ParentName: '',
          customerQuotationTransCateValue: [],
          ParentID: null,
    ...
    methods: {
          ...
          if (responsesData.status === 1) {
            ...
            if (self.idParams || !_.isEmpty(self.itemCopy)) {
              if (_.isObject(responsesData.data.data)) {
                ...
                self.model.customerQuotationTransCateValue = responsesData.data.customerQuotationTransCateValue;

                // set max ValueID
                _.forEach(self.model.customerQuotationTransCateValue, function (field, key) {
                  if (Number(field.ValueID) > self.model.maxValueID) self.model.maxValueID = Number(field.ValueID);
                  // set type of value
                  if (field.DataType == 1) self.model.customerQuotationTransCateValue[key].ConvertedValue = Number(self.model.customerQuotationTransCateValue[key].ConvertedValue);
                  if (field.DataType == 2) self.model.customerQuotationTransCateValue[key].ConvertedValue = String(self.model.customerQuotationTransCateValue[key].ConvertedValue);
                  if (field.DataType == 3) self.model.customerQuotationTransCateValue[key].ConvertedValue = moment(self.model.customerQuotationTransCateValue[key].ConvertedValue).format('DD/MM/YYYY');
                  if (field.DataType == 4) self.model.customerQuotationTransCateValue[key].ConvertedValue = moment(self.model.customerQuotationTransCateValue[key].ConvertedValue).format('DD/MM/YYYY hh:mm');
                });
            ...
            if (_.isArray(responsesData.data.customerQuotationTransCateList)) {

              self.model.customerCateListOption = [];
              _.forEach(responsesData.data.customerQuotationTransCateList, function (value, key) {
                let tmpObj = {};
                tmpObj.id = value.CateID;
                tmpObj.text = value.CateName;
                if (value.CateID == self.model.ParentID) self.model.ParentID = value.CateID;
      ...
      onAddFieldOnTable() {
        ...
        this.model.customerQuotationTransCateValue.push(fieldObj);
        this.$forceUpdate();
      },
      onDeleteFieldOnTable(field) {

        // remove field in fieldOnTableReq
        let fieldExist = _.find(this.model.customerQuotationTransCateValue, ['ValueID', field.ValueID]);
        if (_.isObject(fieldExist)) {
          _.remove(this.model.customerQuotationTransCateValue, ['ValueID', field.ValueID]);
        }
        this.$forceUpdate();
      },
      handleSubmitForm() {
        ...
        const requestData = {
          ...
          data: {
            ...
            detail: this.model.customerQuotationTransCateValue
          }
        };
