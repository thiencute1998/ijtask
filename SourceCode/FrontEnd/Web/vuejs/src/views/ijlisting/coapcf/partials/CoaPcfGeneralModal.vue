<template>
  <div>
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-coa-pcf-general" size="xl">
      <template v-slot:modal-title>
        <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i> {{title}}
      </template>
      <coa-pcf-general-form v-model="value" :coaPcfOptions="coaPcfOptions" :employeeOptions="employeeOptions" v-if="isForm"></coa-pcf-general-form>
      <coa-pcf-general-view :isDetail="true" v-model="value" v-else></coa-pcf-general-view>
...
import CoaPcfGeneralView from "./CoaPcfGeneralView";
import CoaPcfGeneralForm from "./CoaPcfGeneralForm";

export default {
  name: 'CoaPcfGeneralModal',
  props: ['value','title'],
  components: {
    CoaPcfGeneralView,
    CoaPcfGeneralForm
  ...
  methods: {
    ...
    onUpdate(){
      let updateAPI = 'listing/api/coa-pcf/update/';
      let requestData = {
        method: 'post',
        ...
        data: {
          ...
          CoaPcfCate: this.value.CoaPcfCate,
        }
      }
