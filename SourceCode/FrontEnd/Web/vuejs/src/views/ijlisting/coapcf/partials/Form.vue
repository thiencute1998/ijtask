        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Hệ thống tài khoản  quỹ tín dụng nhân dân<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Hệ thống tài khoản  quỹ tín dụng nhân dân<span v-if="model.AccountName">:</span> {{model.AccountName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.AccountName" type="text" id="AccountName" class="form-control" placeholder="Tên hệ thống tài khoản quỹ tín dụng nhân dân" name="AccountName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParent v-model="model" :title="'Hệ thống tài khoản  quỹ tín dụng nhân dân'" :api="'/listing/api/common/list'" :table="'coa_pcf'" :fieldID="'AccountID'" :fieldNo="'AccountNo'" :fieldName="'AccountName'" :placeholderInput="'Chọn hệ thống tài khoản  quỹ tín dụng nhân dân cha'" :placeholderSearch="'Nhập tên HTTK DN'" :columnID="'AccountID'" :columnNo="'AccountNo'" :columnName="'AccountName'">
                </IjcoreModalSystemParent>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-9">
                <CoaPcfModalSearchInputCatelist
                  v-model="model.CoaPcfCate"
                  title-modal="Loại HTTK DN"
                  placeholder="Loại hệ thống tài khoản  quỹ tín dụng nhân dân"
                ></CoaPcfModalSearchInputCatelist>
              </div>
...
import CoaPcfModalSearchInputCatelist from "@/views/ijlisting/coapcf/partials/CoaPcfModalSearchInputCatelist";
...
const ViewRouter = 'listing-coa-pcf-view';
const CreateRouter = 'listing-coa-pcf-create';
const ViewApi = 'listing/api/coa-pcf/view';
const EditApi = 'listing/api/coa-pcf/edit';
const CreateApi = 'listing/api/coa-pcf/create';
const StoreApi = 'listing/api/coa-pcf/store';
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          if (_.isArray(responsesData.data.coaPcf)) {

            self.model.CoaPcfOption = [];
            _.forEach(responsesData.data.coaPcf, function (value, key) {
              let tmpObj = {};
              tmpObj.id = value.AccountID;
              tmpObj.text = value.AccountName;
              self.model.CoaPcfOption.push(tmpObj);
            });
          }
    ...
    handleSubmitForm(){
      ...
      const requestData = {
        ...
        data: {
          ...
          CoaPcfCate: this.model.CoaPcfCate,
    ...
    changeUserContact() {
      ...
      if (employee) {
        this.model.PcftactName = employee.text;
      }
    }
