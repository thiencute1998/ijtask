<template>
  <a @click="onToggleModal" class="ij-a-icon" title="Chi tiết">
    ...
    <b-modal ref="modal" id="modal-form-input-coa-pcf-link" scrollable size="xl">
      <template slot="modal-title">
          <i class="fa fa-edit" v-if="isForm"></i><i class="icon-eye icon" v-else></i>{{title}}
      </template>
      <CoaPcfLinkContent v-model="value" :isForm="isForm" :SysTable="SysTable"></CoaPcfLinkContent>
...
<script>
  ...
  import CoaPcfLinkContent from "./CoaPcfLinkContent";

  export default {
    name: 'CoaPcfLinkModal',
    props: {
      value: [Array, Object],
      title: {},
      CoaPcf: {}
    ...
    methods: {
      ...
      onToggleModal(){
        ...
        if(this.$parent.showCoaPcfLink){
          console.log(this.$parent);
          this.$parent.showCoaPcfLink = true;
        }
      },
      onEdit(){
        this.fetchData();
      ...
      onUpdate(){
        ...
        let requestData = {
          method: 'post',
          url: '/listing/api/coa-pcf/coa-pcf-data-list',
          data: {
            CoaPcfLink : this.value,
            AccountID: this.CoaPcf.AccountID
          }
