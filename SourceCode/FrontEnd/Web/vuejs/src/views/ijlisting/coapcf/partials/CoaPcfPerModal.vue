<template>
  <a @click="onToggleModal()" class="ij-a-icon" title="Phân quyền">
    ...
    <b-modal scrollable ref="modal" id="modal-form-input-doc-general" size="lg" @hide="onHideModalDataflow($event)">
      ...
      <coa-pcf-per-view v-model="value" v-if="!isForm" :isDetail="true" :per="per" :EmployeeOption="EmployeeOptionArr"></coa-pcf-per-view>
      <coa-pcf-per-form v-model="value" v-if="isForm" :per="per" :EmployeeOption="EmployeeOptionArr" :CoaPcfOption="CoaPcfOption" :GroupOption="GroupOption" :EmployeeLogin=" EmployeeLogin"></coa-pcf-per-form>
...
import CoaPcfPerView from "./CoaPcfPerView";
import CoaPcfPerForm from "./CoaPcfPerForm";

export default {
  name: 'coa-pcf-per-modal',
  components: {CoaPcfPerForm, CoaPcfPerView},
  computed: {},
  data() {
    return {
      isForm: false,
  ...
  methods: {
    ...
    onUpdate() {
      ...
      let UpdateApi = 'listing/api/coa-pcf/update-per';
  ...
  filters: {},
  props: ['title', 'value', 'name', 'api', 'table', 'CoaPcf', 'isDetail', 'per', 'EmployeeOption', 'CoaPcfOption', 'GroupOption'],
}
</script>
<style>
.readonly {
