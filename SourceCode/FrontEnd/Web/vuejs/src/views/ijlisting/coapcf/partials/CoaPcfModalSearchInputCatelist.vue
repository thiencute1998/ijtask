<template>
  <div class="ijcore ijcore-modal component-modal-search-ccatelist" @mouseenter="onMouseenter" @mouseleave="onMouseleave">
    ...
    >
      ...
      <div class="ijcore-modal-coa-pcf-ccatelist ijcore-modal-search">
        <table class="table b-table table-sm table-bordered table-editable">
          ...
          <tr v-for="(item, key) in value" :key="key">
            <td>
              <Select2 v-model="value[key].CateID" :settings="{allowClear: true, placeholder: {id: null, text: 'Chọn loại HTTK QTDND'}}" :options="CateListOptions" @select="onSelectCateList($event, key)"></Select2>
            </td>
            <td>
...
export default {
  name: 'coa-pcf-modal-search-input-catelist',
  components: {
    Select2
  },
  props: {
  ...
  methods: {
    ...
    fetchData(){
      ...
      requestData.url= 'listing/api/coa-pcf/get-coa-pcf-cate-list';
      ...
      .then(response =>{
        ...
        self.CateList = responsesData.data.CoaPcfCateList;
        ...
        self.CateValue = responsesData.data.CoaPcfCateValue;
...
<style lang="css">
  ...
  .ijcore-modal-coa-pcf-ccatelist .select2.select2-container{
    width: 100%;
  }
