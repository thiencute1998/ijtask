          ...
          <b-row class="mb-2">
            <b-col class="col-md-12">
              <div class="main-header-item main-header-name">
                <span class="text-directionize"><i class="fa fa-list mr-2"></i>Loại chỉ thị</span>
              </div>
            </b-col>
...
export default {
  ...
  computed: {
    ...
    captions: function() {
      let fieldsTable = [
        ...
        {key: 'CateName', label: 'Tên loại chỉ thị', thStyle: 'min-width: 300px; z-index:7', tdClass: 'bg-tree-tr', field: 'CateName',searchOnTopRow: {type: 'text'}},
      ];
  ...
  created() {
    ...
    this.settings.Table = 'direction_cate_list';
    this.settings.FieldInactive = 'Inactive';

    this.settings.ListApi = 'listing/api/direction-cate-list';
    this.settings.DeleteApi = 'listing/api/direction-cate-list/delete';
    this.settings.CreateRouter = 'listing-directioncatelist-create';
    this.settings.EditRouter = 'listing-directioncatelist-edit';
    this.settings.ViewRouter = 'listing-directioncatelist-view';

    this.modelSearch = {
      CateID: (this.$route.query && this.$route.query.CateID) ? this.$route.query.CateID : '',
  ...
  methods: {
          ...
          ApiService.customRequest(requestData).then((response) => {
            ...
            if (dataResponse.status === 1) {
              ...
              _.forEach(self.idsSelected, function (id, key) {
                ...
                if(keyParent != null){
                  if(!self.haveChildren(self.itemsArray[keyParent])) self.itemsArray[keyParent].Detail = 1;
                }
              });
    ...
    fetchData(){
      ...
      if (this.modelSearch.CateID.trim() !== '') {
    ...
    onToggleChildren($event,item){
      ...
      let $icon = $('#table-item-' + item.CateID + ' .bg-tree-icon-action');
      if ($icon.hasClass('fa-plus-square-o')) {
        ...
        _.forEach(allChildCate, function (cate, key) {
          let _index = _.findIndex(self.itemsArray, ['CateID', cate.CateID]);
          if (_index >= 0) {
        ...
        $('#table-item-' + item.CateID + ' .bg-tree-icon-action').removeClass('fa-plus-square-o').addClass('fa-minus-square-o');
      }
      else {
        _.forEach(allChildCate, function (cate, key) {
          let _index = _.findIndex(self.itemsArray, ['CateID', cate.CateID]);
        ...
        $('#table-item-' + item.CateID + ' .bg-tree-icon-action').removeClass('fa-minus-square-o').addClass('fa-plus-square-o');
      }
    ...
    getListChild(CateID){
      ...
      let requestData = {
        method: 'post',
        url: 'listing/api/direction-cate-list/get-list-child',
        data: {
          per_page: this.perPage,
          page: this.currentPage,
          ParentID: CateID,
    ...
    async handleExportExcel() {
      ...
      let reportJson = await this.$_lists_handleReportTemplate('direction-cate-list');
      let request = {};
      ...
      let reportData = await this.$_lists_handleReportResponse('listing/api/direction-cate-list/get-report-data', request);
      this.$_lists_handleDowloadExcel(reportJson, reportData, 'CateDirection');
    },
  },
  watch: {
    currentPage() {
