        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại chỉ thị<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại chỉ thị<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="DirectionName" class="form-control" placeholder="Tên chỉ thị" name="DirectionName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              <label class="col-md-3 m-0">Chỉ thị cha</label>
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  ...
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại chỉ thị cha'"
                  :placeholderSearch="'Nhập loại chỉ thị'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.directionCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeRevenueTableItem">
                <tr class="draggable" v-for="(field, key) in model.directionCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.directionCateValue[key].Description"
                      autocomplete="direction cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.directionCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.directionCateValue[key].CateValue"
                      autocomplete="direction cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.directionCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.directionCateValue[key].CateValue"
...
const ListRouter = 'listing-directioncatelist';
const EditRouter = 'listing-directioncatelist-edit';
const CreateRouter = 'listing-directioncatelist-edit';
const ViewRouter = 'listing-directioncatelist-view';
const ViewApi = 'listing/api/direction-cate-list/view';
const EditApi = 'listing/api/direction-cate-list/edit';
const CreateApi = 'listing/api/direction-cate-list/create';
const StoreApi = 'listing/api/direction-cate-list/store';
const UpdateApi = 'listing/api/direction-cate-list/update';
const ListApi = 'listing/api/direction-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.directionCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.directionCateValue = responsesData.data.directionCateValue;

              // set max lineID
              _.forEach(self.model.directionCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.directionCateValue[key].CateValue = Number(self.model.directionCateValue[key].CateValue);
                if (field.DataType == 2) self.model.directionCateValue[key].CateValue = String(self.model.directionCateValue[key].CateValue);
                if (field.DataType == 3) self.model.directionCateValue[key].CateValue = moment(self.model.directionCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.directionCateValue[key].CateValue = moment(self.model.directionCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.directionCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.directionCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.directionCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.directionCateValue
        }
      };
