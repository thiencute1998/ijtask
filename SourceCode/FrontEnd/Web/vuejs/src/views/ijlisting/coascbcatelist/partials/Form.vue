        ...
        <b-row class="mb-2">
          <b-col class="col-md-18">
            <div class="main-header-item main-header-name">
              <span v-if="idParams"><i class="fa fa-edit mr-2"></i> Loại hệ thống tài khoản xã hội<span v-if="model.CateName">:</span> {{model.CateName}}</span>
              <span v-if="!idParams"><i class="fa fa-plus mr-2"></i> Loại hệ thống tài khoản xã hội<span v-if="model.CateName">:</span> {{model.CateName}}</span>
            </div>
          </b-col>
          ...
          <b-card>
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17 col-sm-14 mb-sm-0 mb-md-0 col-20 mb-3 app-object-name">
                <input v-model="model.CateName" type="text" id="RevenuName" class="form-control" placeholder="Tên hệ thống tài khoản xã hội" name="RevenuName"/>
              </div>
            ...
            <div class="form-group row align-items-center">
              ...
              <div class="col-md-17">
                <IjcoreModalSystemParentCate
                  v-model="model"
                  :title="'loại hệ thống tài khoản xã hội'"
                  :api="'/listing/api/common/listCate'"
                  :table="'coa_scb'"
                  :fieldName="'CateName'"
                  :fieldNo="'CateNo'"
                  :fieldID="'CateID'"
                  :placeholderInput="'Chọn loại hệ thống tài khoản xã hội cha'"
                  :placeholderSearch="'Nhập loại hệ thống tài khoản xã hội'"
                  :columnID="'CateID'"
                  :columnNo="'CateNo'"
                  :columnName="'CateName'"
                >
            ...
            <table class="table b-table table-sm table-bordered table-editable">
              ...
              <draggable v-model="model.coaScbCateValue" tag="tbody" draggable=".draggable" handle=".my-handle" @change="changeCoaScbTableItem">
                <tr class="draggable" v-for="(field, key) in model.coaScbCateValue">
                  ...
                  <td>
                    <b-form-input
                      type="text"
                      v-model="model.coaScbCateValue[key].Description"
                      autocomplete="coaScb cate list description">
                    </b-form-input>
                  </td>
                  <td>
                    <b-form-select :options="dataTypeOption" @change="onChangeDataType($event, field)"
                                   v-model="model.coaScbCateValue[key].DataType"></b-form-select>
                  </td>
                  <td>
                    <b-form-input
                      v-if="field.DataType === 1"
                      type="number"
                      v-model="model.coaScbCateValue[key].CateValue"
                      autocomplete="coaScb cate list description">
                    </b-form-input>
                    <b-form-input
                    ...
                    <b-form-select v-if="field.DataType === 5" v-model="model.coaScbCateValue[key].CateValue"
                                   :options="[{value: 1, text: 'Có'},{value: 2, text: 'Không'}]"></b-form-select>
                    <b-form-select v-if="field.DataType === 6" v-model="model.coaScbCateValue[key].CateValue"
...
const ListRouter = 'listing-coascbcatelist';
const EditRouter = 'listing-coascbcatelist-edit';
const CreateRouter = 'listing-coascbcatelist-edit';
const ViewRouter = 'listing-coascbcatelist-view';
const ViewApi = 'listing/api/coa-scb-cate-list/view';
const EditApi = 'listing/api/coa-scb-cate-list/edit';
const CreateApi = 'listing/api/coa-scb-cate-list/create';
const StoreApi = 'listing/api/coa-scb-cate-list/store';
const UpdateApi = 'listing/api/coa-scb-cate-list/update';
const ListApi = 'listing/api/coa-scb-cate-list';
...
export default {
  ...
  methods: {
          ...
          if (self.idParams || !_.isEmpty(self.itemCopy)) {
            if (_.isObject(responsesData.data.data)) {
              if( responsesData.data.data.ParentID){
                ...
                let Parent = _.filter(responsesData.data.coaScbCateList, ["CateID", responsesData.data.data.ParentID])
              ...
              self.model.coaScbCateValue = responsesData.data.coaScbCateValue;

              // set max lineID
              _.forEach(self.model.coaScbCateValue, function (field, key) {
                if (Number(field.LineID) > self.model.maxLineID) self.model.maxLineID = Number(field.LineID);
                // set type of value
                if (field.DataType == 1) self.model.coaScbCateValue[key].CateValue = Number(self.model.coaScbCateValue[key].CateValue);
                if (field.DataType == 2) self.model.coaScbCateValue[key].CateValue = String(self.model.coaScbCateValue[key].CateValue);
                if (field.DataType == 3) self.model.coaScbCateValue[key].CateValue = moment(self.model.coaScbCateValue[key].CateValue).format('DD/MM/YYYY');
                if (field.DataType == 4) self.model.coaScbCateValue[key].CateValue = moment(self.model.coaScbCateValue[key].CateValue).format('DD/MM/YYYY hh:mm');
              });
    ...
    onAddFieldOnTable() {
      ...
      this.model.coaScbCateValue.push(fieldObj);
      this.$forceUpdate();
    },
    onDeleteFieldOnTable(field) {

      // remove field in fieldOnTableReq
      let fieldExist = _.find(this.model.coaScbCateValue, ['LineID', field.LineID]);
      if (_.isObject(fieldExist)) {
        _.remove(this.model.coaScbCateValue, ['LineID', field.LineID]);
      }
      this.$forceUpdate();
    },
    handleSubmitForm() {
      ...
      const requestData = {
        ...
        data: {
          ...
          detail: this.model.coaScbCateValue
        }
      };
