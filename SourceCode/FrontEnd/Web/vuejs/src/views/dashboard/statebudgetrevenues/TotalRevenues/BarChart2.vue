<template>
  <div>
    ...
    <b-modal id="modal-bar-chart-2-fullwidth" size="xl" ref="modal-bar-chart-2-fullwidth" title="Tổng thu" ok-only ok-title="Đóng">
...
export default {
  name: 'dashboard-module-state-budget-revenues-bar-chart-2',
  props: {
    filter: [Array, Object],
  ...
  methods: {
      ...
      if(self.optionDataBar2.labels.length){
        ...
        for (let i = self.optionDataBar2.labels.length - 1; i >= 0; i--) {
          ...
          if(keyDataset == 1){
            ...
            if(i != 0){
              randomNumber = _.random(Number(totalRevenue)/Number(self.optionDataBar2.labels.length * 2), Number(totalRevenue)/Number(self.optionDataBar2.labels.length));
              total += randomNumber;
            }
            else{
    ...
    changeChartRevenue(){
      ...
      this.optionDataBar2.labels = labels;
      this.optionDataBar2.norder = norder;
          ...
          if(self.optionDataBar2.labels.length){
            ...
            for (let i = self.optionDataBar2.labels.length - 1; i >= 0; i--) {
              ...
              if(keyDataset == 1){
                ...
                if(i != 0){
                  randomNumber = Math.round(_.random(Number(self.overviewNumber.totalRevenue)/Number(self.optionDataBar2.labels.length * 2), Number(self.overviewNumber.totalRevenue)/Number(self.optionDataBar2.labels.length)));
                  total += randomNumber;
                }
                else{
