    ...
    changeChart(){
      if(this.model1.ProvinceID === this.filter.ProvinceID && this.model1.DistrictID === this.filter.DistrictID){
        ...
        _.forEach(self.optionDataBar.datasets, function (dataset, keyDataset) {
          ...
          if(self.filter.ProvinceID == 26 && self.filter.ExpenseCateID == null && self.filter.DistrictID == null && self.filter.CommuneID == null && ((self.filter.PeriodID == 1 && self.filter.PeriodValue == 2020) || self.filter.PeriodID == null)){
            let valueTotal = [];
            if(keyDataset == 0){
