  ...
  data: function () {
    return {
      optionDataBar3: {
        ...
        datasets: [
          {
            label: "Dự toán "+ this.labelsType ,
            backgroundColor: "#2eadd3",
            fill: true,
            data: []
          },
          {
            label: "Ước thực hiện "+ this.labelsType,
    ...
    setData(values){
      ...
      if(self.value){
        _.forEach(values, function (val, key){
          arrName.push(val.CateName);
          arrDT.push(val.DT);
          arrUTT.push(val.UTT);
        });
        this.optionDataBar3.labels = arrName;
        this.optionDataBar3.datasets[0].data = arrDT;
        this.optionDataBar3.datasets[1].data = arrUTT;
      }
        ...
        if(this.filter.ProvinceID == 36 && this.filter.DistrictID == null && this.filter.CommuneID == null && ((this.filter.PeriodID == 1 && this.filter.PeriodValue == 2020) || this.filter.PeriodID == null)){
         ...
         if(this.filter.RevenueCateID == 22){
           ...
           arrayLabels = ['Thuế giá trị gia tăng','Thuế tiêu thụ đặc biệt','Thuế thu nhập DN',
             'Thuế tài nguyên','Thuế bảo vệ MT','Thuế thu nhập CN','Thuế SD đất phi nông nghiệp',
