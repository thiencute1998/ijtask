  ...
  <div>
    <div class="sort-area" style="position: absolute;">
      <select class="dropdown" style="background-color: rgb(255 255 255)" id="sort-cocau-2" v-model="model1.SortValue">
        ...
        <option value="3">Địa bàn thu</option>
      </select>
    </div>
...
export default {
  ...
  data: function () {
    return {
      optionDataBar: {
        ...
        AdressID: [356,358,359,360,361,362,363,364,365,366],
        AdressNo: ['07100','07200','07250','07300','07400','07500','07600','07700','07800','07900'],
  ...
  mounted () {
    ...
    BarChart = new Chart(ctxBar, {
      ...
      data: {
        ...
        districtID: self.optionDataBar.AdressID,
        districtNo: self.optionDataBar.AdressNo,
      ...
      options: {
        ...
        title: {
          display: true,
          text: 'Dự toán thu NSNN theo địa bàn (tỷ đồng)',
          position: "top",
          fontSize: 16
        },
        legend: {
  ...
  methods: {
    ...
    changeChart(){
      ...
      _.forEach(self.value.Adress , function (val, key){
          ...
          _.forEach(self.value.data, function (v, k){
            if(v.ID === val.ID){
              if(v.DT){
                self.optionDataBar.datasets[0].data[key] = v.DT;
              }
              if(v.UTT){
                self.optionDataBar.datasets[1].data[key] = v.UTT;
              }
            }
          });
      });
    ...
    sortChart(){
      ...
      let arrayDistrictID = self.optionDataBar.AdressID;
      let arrayDistrictNo = self.optionDataBar.AdressNo;
      ...
      if(this.model1.SortValue != ''){
        ...
        self.optionDataBar.AdressID = newArrayDistrictID;
        self.optionDataBar.AdressNo = newArrayDistrictNo;
        ...
        if(this.model1.showFullWidth){
          this.updateData(ChartFullwidth, self.optionDataBar.labels, self.optionDataBar.AdressID, self.optionDataBar.AdressNo, self.optionDataBar.norder, self.optionDataBar.datasets, '');
        }
      }
      ...
      ApiService.customRequest(requestData).then((responses) => {
        ...
        if (responsesData.status === 1) {
          ...
          _.forEach(responsesData.data, function (district, key) {
            ...
            self.optionDataBar.AdressID.push(district.DistrictID);
            self.optionDataBar.AdressNo.push(district.DistrictNo);
          });
