<template>
  <div>
    ...
    <b-modal id="modal-bar-chart-22-fullwidth" size="xl" ref="modal-bar-chart-22-fullwidth" title="Tổng chi" ok-only ok-title="Đóng">
...
export default {
  name: 'dashboard-module-state-budget-expenses-bar-chart-22',
  props: {
    filter: [Array, Object],
  ...
  methods: {
    ...
    sortChart(){
      ...
      if(this.model1.SortValue != ''){
        ...
        this.updateData(BarChart22, self.optionDataBar2.labels, self.optionDataBar2.norder, self.optionDataBar2.datasets, '');
      }
    ...
    changeChartExpense(){
      let self = this;
      if(self.filter.ProvinceID == 36 && self.filter.ExpenseCateID == null && this.filter.DistrictID == null && this.filter.CommuneID == null && ((this.filter.PeriodID == 1 && this.filter.PeriodValue == 2020) || this.filter.PeriodID == null)){
        let arrayDuToan = [1339,131,224,4393,172,1049,1091,2057,526,7507];
        let arrayThucHien = [1333,658,327,1936,166,821,854,1609,518,7045];
        self.optionDataBar2.datasets[0].data = arrayThucHien;
        self.optionDataBar2.datasets[1].data = arrayDuToan;
      }
      else{
        _.forEach(self.optionDataBar2.datasets, function (dataset, keyDataset) {
          self.optionDataBar2.datasets[keyDataset].data = [];
          if(self.optionDataBar2.labels.length){
            let total = 0;
            for (let i = self.optionDataBar2.labels.length - 1; i >= 0; i--) {
              let randomNumber = 0;
              if(keyDataset == 1){
                if(i != 0){
                  randomNumber = Math.round(_.random(Number(self.overviewNumber.totalEstimate)/Number(self.optionDataBar2.labels.length * 2), Number(self.overviewNumber.totalEstimate)/Number(self.optionDataBar2.labels.length)));
                  total += randomNumber;
                }
                else{
                ...
                if(i != 0){
                  randomNumber = Math.round(_.random(Number(self.overviewNumber.totalExpense)/Number(self.optionDataBar2.labels.length * 2), Number(self.overviewNumber.totalExpense)/Number(self.optionDataBar2.labels.length)));
                  total += randomNumber;
                }
                else{
                  randomNumber = Number(self.overviewNumber.totalExpense) - total;
                }
              }
              randomNumber = Math.round(randomNumber);
              self.optionDataBar2.datasets[keyDataset].data.push(randomNumber);
