...
<!--    </div>-->
    ...
    <canvas id="bar-chart-revenue-1" height="180px"></canvas>
    <!--    <span class="icon-bar-chart-revenue-1-full-width"><i class="fa fa-expand" @click="chartFullWidth"></i></span>-->
<!--    <b-modal id="modal-bar-chart-revenue-1-fullwidth" size="xl" ref="modal-bar-chart-revenue-1-fullwidth" title="Tổng thu" ok-only ok-title="Đóng">-->
<!--      <canvas id="bar-chart-revenue-1-fullwidth" height="150px"></canvas>-->
<!--    </b-modal>-->
...
let BarChartRevenue1 = null;
let ChartFullwidth = null;
export default {
  name: 'dashboard-module-state-budget-revenues-bar-chart-revenue-1',
  props: {
    filter: [Array, Object],
  ...
  data: function () {
    return {
        ...
        datasets: [
          {
            ...
            fill: true,
            data: [9254,3659,647,1352,926,844,1790,569,368,3000],
          },
          {
            label: "Thực hiện",
      ...
      model1: {
        ...
        TotalEstimateDistrict: [9254,3659,647,1352,926,844,1790,569,368,3000],
        TotalRevenueDistrict: [6865,2714,480,1220,687,626,1327,422,273,2008],
        TotalSettlementDistrict: [6865,2714,480,1220,687,626,1327,422,273,2008],
      }
    }
  },
  components: {},
  mounted () {
    let self = this;
    let ctxBar = document.getElementById('bar-chart-revenue-1');
    ctxBar.height = 120;
    BarChartRevenue1 = new Chart(ctxBar, {
      type: 'bar',
      data: {
      ...
      options: {
        ...
        title: {
          display: true,
          text: 'Quyết toán thu NSNN theo địa bàn (tỷ đồng)',
          position: "top",
          fontSize: 16
        },
        legend: {
        ...
        'onClick' : function (evt) {
          ...
          // })
          let activePoints = BarChartRevenue1.getElementsAtEventForMode(evt, 'point', BarChartRevenue1.options);
          let firstPoint = activePoints[0];
          if(firstPoint && self.filter.DistrictID === null){
            let districtID = BarChartRevenue1.data.districtID[firstPoint._index];
            let districtNo = BarChartRevenue1.data.districtNo[firstPoint._index];
            let districtName = BarChartRevenue1.data.labels[firstPoint._index];
  ...
  methods: {
    ...
    changeChart(){
      if(this.model1.ProvinceID === this.filter.ProvinceID && this.model1.DistrictID === this.filter.DistrictID){
        ...
        for(let keyDataset = 0; keyDataset < 2;keyDataset++){
          ...
          if(self.filter.ProvinceID == 26 && self.filter.RevenueCateID == null && self.filter.DistrictID == null && self.filter.CommuneID == null && ((self.filter.PeriodID == 1 && self.filter.PeriodValue == 2020) || self.filter.PeriodID == null)){
            let valueTotal = [];
            if(keyDataset == 1){
              valueTotal = [6865,2714,480,1220,687,626,1327,422,273,2008];
            }
            else if(keyDataset == 0){
              valueTotal = [9254,3659,647,1352,926,844,1790,569,368,3000];
            }
            else{
              valueTotal = [6865,2714,480,1220,687,626,1327,422,273,2008];
            }
            ...
            self.getRandomRevenue(self.overviewNumber.totalEstimateRevenue, self.overviewNumber.totalRevenue, self.overviewNumber.totalSettlementRevenue, keyDataset)
          }
        }
        // _.forEach(self.optionDataBar.datasets, function (dataset, keyDataset) {
        //
        // });
        this.updateData(BarChartRevenue1, self.optionDataBar.labels, self.optionDataBar.districtID, self.optionDataBar.districtNo, self.optionDataBar.norder, self.optionDataBar.datasets, '');
        this.sortChart();
      }
    ...
    getCommuneSelected(){
      ...
      this.updateData(BarChartRevenue1, this.optionDataBar.labels, this.optionDataBar.districtID, this.optionDataBar.districtNo, this.optionDataBar.norder, this.optionDataBar.datasets, '');

    },
    chartFullWidth() {
      this.model1.showFullWidth = true;
      this.$bvModal.show('modal-bar-chart-revenue-1-fullwidth');
      let self = this;
      this.$nextTick(() => {
        let ctxChartFullwidth = document.getElementById('bar-chart-revenue-1-fullwidth');
        ChartFullwidth = new Chart(ctxChartFullwidth, {
          ...
          options: {
            ...
            title: {
              display: true,
              text: 'Quyết toán thu NSNN theo địa bàn (nghìn tỷ)',
              position: "top",
              fontSize: 16
            },
            legend: {
    ...
    getRandomRevenue(totalEstimateRevenue, totalRevenue, totalSettlementRevenue, keyDataset){
      let self = this;
      if(self.optionDataBar.labels.length){
        ...
        for (let i = self.optionDataBar.labels.length - 1; i >= 0; i--) {
          ...
          if(keyDataset == 0){
            if(i != 0){
              randomNumber = _.random(Number(totalEstimateRevenue)/Number(self.optionDataBar.labels.length * 2), Number(totalEstimateRevenue)/Number(self.optionDataBar.labels.length));
              total += randomNumber;
            }
            else{
              randomNumber = Number(totalEstimateRevenue) - total;
            }
      ...
      if(this.model1.DistrictSelected){
        ...
        ApiService.customRequest(requestData).then((responses) => {
          ...
          if (responsesData.status === 1) {
            ...
            for(let keyDataset = 0; keyDataset<2 ; keyDataset++){
              self.optionDataBar.datasets[keyDataset].data = [];
              self.getRandomRevenue(self.model1.TotalEstimateDistrict[positionDistrict - 1], self.model1.TotalRevenueDistrict[positionDistrict - 1], self.model1.TotalSettlementDistrict[positionDistrict - 1], keyDataset)
            }
