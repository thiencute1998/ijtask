x<template>
  <div>
    <b-input-group>
      <b-form-input :placeholder="'Nhập '+this.title" :title="value[FieldNameDataParent]" :readonly="true" v-model="value[FieldNameDataParent]"
                    class="readonly form-control" @click="onToggleModal()"></b-form-input>
      <input type="hidden" :readonly="true" v-model="value[FieldIDDataParent]" class="readonly form-control"></input>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.FieldID) {
        ...
        this.FieldIDDataParent = this.FieldID;
      } else {
        this.FieldIDData = this.tableName + 'ID';
        this.FieldIDDataParent = this.tableName + 'ID';
      }
      if (this.FieldNo) {
        this.FieldNoData = this.FieldNo;
        this.FieldNoDataParent = this.FieldNo;
      } else {
        this.FieldNoData = this.tableName + 'No';
        this.FieldNoDataParent = this.tableName + 'No';
      }
      if (this.FieldName) {
        this.FieldNameData = this.FieldName;
        this.FieldNameDataParent = this.FieldName;
      } else {
        this.FieldNameData = this.tableName + 'Name';
        this.FieldNameDataParent = this.tableName + 'Name';
      }


      if (this.FieldIDParent) {
        this.FieldIDDataParent = this.FieldIDParent;
      }
      if (this.FieldNoParent) {
        this.FieldNoDataParent = this.FieldNoParent;
      }
      if (this.FieldNameParent) {
        this.FieldNameDataParent = this.FieldNameParent;
      }
    ...
    methods: {
      ...
      updateForParent: function (ParentID, ParentName, ParentNo, Key) {
        let self = this;
        self.value[this.FieldIDDataParent] = ParentID;
        self.value[this.FieldNameDataParent] = ParentName;
        self.value[this.FieldNoDataParent] = ParentNo;
        if(self.FieldUpdate){
          _.forEach(this.FieldUpdate, function (field, k) {
            self.value[field] = self.listtable[Key][k];
          });
        }
