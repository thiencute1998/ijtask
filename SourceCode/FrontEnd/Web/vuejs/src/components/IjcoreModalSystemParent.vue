    ...
    <b-modal ref="modal" id="modal">
      ...
      <div class="table-responsive">
        <table class="table b-table table-hover table-sm b-table-selectable b-table-select-multi">
          <tbody>
          <tr v-for="item in listtable" @click="updateForParent(item[columnID], item[columnName], item[columnNo])">
            <td class="pr-3 tr-no" style="min-width: 80px;">{{item[columnNo]}}</td>
            <!--            <td class="pr-3"><span :title="item[tableName + 'Name']">{{item[tableName + 'Name']}}</span></td>-->
            <td class="pr-3" style="min-width: 280px;"><span :title="item[columnName]">{{item[columnName]}}</span></td>
          </tr>
          </tbody>
        </table>
      </div>
