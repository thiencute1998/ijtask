<template>
  <b-button type="submit" variant="primary" class="main-header-action mr-2" title="Chọn bảng chỉ tiêu ĐGCV"
            @click="onToggleModal" style="background-color: #00a2e8 !important;">
    <i class='fa fa-external-link'></i> Bảng
    <input type="hidden" :readonly="true" v-model="value['TableID']" class="readonly form-control"></input>
    <b-modal ref="modal" id="modal">
      <template slot="modal-title">
        Bảng chỉ tiêu đánh giá công việc
      </template>
      <b-input-group class="pt-10">
        <b-form-input v-model="search" :placeholder="'Nhập tên bảng chỉ tiêu ĐGCV'" class="readonly form-control"
      ...
      <div class="table-responsive pb-10">
        <table class="table b-table table-hover table-sm b-table-selectable b-table-select-multi">
          <tbody>
          <tr v-for="item in listtable" @click="updateForParent(item['TableID'], item['TableName'], item['TableNo'])">
            <td class="pr-3">{{item['TableName']}}</td>
          </tr>
          </tbody>
        </table>
      </div>
...
<script>
  ...
  export default {
    name: 'IjcoreModalIndicatorTable',
    mixins: [mixinLists],
    components: {},
    computed: {
      rows() {
    ...
    methods: {
      fetchData() {
        let self = this;
        let urlApi = '/task/api/evaluation-trans/get-list';
        let requestData = {
          method: 'post',
          url: urlApi,
          data: {
      ...
      updateForParent: function (ParentID, ParentName, ParentNo) {
        this.value['TableID'] = ParentID;
        // this.value['TableName'] = ParentName;
        // this.value['TableNo'] = ParentNo;
