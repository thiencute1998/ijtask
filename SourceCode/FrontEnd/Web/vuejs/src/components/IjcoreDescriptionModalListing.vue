<template>
    <div>
        <b-input-group>
            <b-form-input :placeholder="'Nhập '+this.title" v-model="value['Description']" class="readonly form-control"></b-form-input>
            <b-form-input style="display: none;" :placeholder="'Nhập '+this.title" :readonly="true" v-model="value[tableName+'Name']" class="readonly form-control"  @click="onToggleModal()"></b-form-input>
            <input type="hidden" :readonly="true" v-model="value[tableName+'ID']" class="readonly form-control"></input>
        ...
        <b-modal ref="modal" id="modal">
            ...
            <b-input-group>
                <b-form-input v-model="search" :placeholder="'Nhập tên '+this.title" class="readonly form-control" @change="fetchData"></b-form-input>
            ...
            <div class="table-responsive">
                <table class="table b-table table-hover table-sm b-table-selectable b-table-select-multi">
                    <tbody>
                        <tr v-for="(item, key) in listtable" @click="updateForParent(item[tableName+'ID'], item[tableName+'Name'], item[tableName+'No'], key)">
                          <td style="width: 20%">{{item[FieldNo]}}</td>
...
<script>
    ...
    export default {
        name: 'IjcoreDescriptionModalListing',
        mixins: [mixinLists],
        components: {
        },
        computed: {
        ...
        methods: {
            ...
            updateForParent: function (ParentID, ParentName, ParentNo, key) {
              let self = this;
              this.value[this.tableName+'ID'] = ParentID;
              this.value[this.tableName+'Name'] = ParentName;
              this.value[this.tableName+'No'] = ParentNo;
              if(!this.value['Description']){
                this.value['Description'] = ParentName;
              }
              if(this.FieldUpdate){
