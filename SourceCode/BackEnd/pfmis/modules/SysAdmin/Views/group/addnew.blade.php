            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title" id="horz-layout-basic"><span class="ft ft-plus-square icon-header"></span> <span class="text-header">Nhóm người dùng</span></h4>
                    ...
                    <div class="card-content collpase show">
                        <div class="card-body">
                            ...
                            @endif
                            <form class="form form-horizontal" id="form-group" method="post" action="{{route('group.store')}}">
                                @csrf
                                <div class="form-body">
                                    <div class="form-group row">
                                        <div class="col-md-12">
                                            <button type="submit" id="submit-button" class="btn btn-primary mr-1 btn-sbc">
                                    ...
                                    <div class="tab-content px-1 pt-1">
                                        <div class="tab-pane active" id="tab1" role="tabpanel" aria-labelledby="base-tab1">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-4">
                                                    <input type="text" id="UserGroupName" class="form-control" placeholder="Nhập tên nhóm người dùng" name="UserGroupName" value="{{old('UserGroupName')}}">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label class="col-md-2" for="UserGroupType">Loại nhóm</label>
                                                <div class="col-md-4">
                                                    <select id="UserGroupType" name="UserGroupType" class="form-control">
                                                        <option value="">Chọn loại nhóm người dùng</option>
                                                        <option value="1" @if(old('UserGroupType') == 1) selected @endif>Người quản trị</option>
                                                        <option value="2 " @if(old('UserGroupType') == 2) selected @endif>Người tác nghiệp </option>
                                                        <option value="3" @if(old('UserGroupType') == 3) selected @endif>Người khai thác</option>
                                                        <option value="4" @if(old('UserGroupType') == 4) selected @endif>Người kiểm tra</option>
                                                        <option value="5" @if(old('UserGroupType') == 5) selected @endif>Khách</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-4">
                                                    <textarea class="form-control" id="Note" rows="3" placeholder="Nhập ghi chú" name="Note">{{old('Note')}}</textarea>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label class="col-md-2" for="NOrder">Số thứ tự</label>
                                                <div class="col-md-2">
                                                    <input type="number" id="NOrder" class="form-control" placeholder="Nhập STT" name="NOrder" value="{{old('NOrder')}}">
                                                </div>
                                            </div>
                                        </div>
                                                            ...
                                                            <table class="table table-striped table-striped table-bordered zero-configuration dataTable" id="data-permission" role="grid" aria-describedby="DataTables_Table_0_info">
                                                                ...
                                                                @foreach($data as $item)
                                                                    ...
                                                                    <tr role="row" UserGroupType="{{$item->UserGroupType}}" class="tr-feature bg-tree-tr @if($item->ParentID == 0) feature-parent bold @else feature-child group-feature-{{$item->ParentID}} hide @endif">
                                                                        ...
                                                                        <td class="center">
                                                                            <input name="AccessInput[{{$item->FeatureID}}]" type="hidden" parent_id="{{$item->ParentID}}" data="{{$item->FeatureID}}" class="collumn-1 collumn-1-{{$item->FeatureID}} feature-child-group-{{$item->FeatureID}} feature-{{$tr}}-group-1-{{$parent}} feature-{{$tr}}-group-{{$parent}}" value="{{$item->DefaultAccess}}" />
                                                                        ...
                                                                        <td class="center">
                                                                            <input name="AddInput[{{$item->FeatureID}}]" type="hidden" parent_id="{{$item->ParentID}}" data="{{$item->FeatureID}}"  class="collumn-2 collumn-2-{{$item->FeatureID}} feature-child-group-{{$item->FeatureID}} feature-{{$tr}}-group-2-{{$parent}} feature-{{$tr}}-group-{{$parent}}" value="{{$item->DefaultAddnew}}"/>
                                                                        ...
                                                                        <td class="center">
                                                                            <input name="EditInput[{{$item->FeatureID}}]" type="hidden" parent_id="{{$item->ParentID}}" data="{{$item->FeatureID}}"  class="collumn-3 collumn-3-{{$item->FeatureID}} feature-child-group-{{$item->FeatureID}} feature-{{$tr}}-group-3-{{$parent}} feature-{{$tr}}-group-{{$parent}}" value="{{$item->DefaultEdit}}"/>
                                                                        ...
                                                                        <td class="center">
                                                                            <input name="DeleteInput[{{$item->FeatureID}}]" type="hidden" parent_id="{{$item->ParentID}}" data="{{$item->FeatureID}}" class="collumn-4 collumn-4-{{$item->FeatureID}} feature-child-group-{{$item->FeatureID}} feature-{{$tr}}-group-4-{{$parent}} feature-{{$tr}}-group-{{$parent}}" value="{{$item->DefaultDelete}}"/>
