...
class CustomerBusiness extends Model{
    ...
    public static function search(Request $request, $id){
        ...
        $query->select(DB::raw('customer_business.*, employee.EmployeeName'))
            ->leftJoin('employee', 'customer_business.EmployeeID', '=', 'employee.EmployeeID')
            ->where(['CustomerID' => $id]);
        ...
        if($request->post('EmployeeNameSearch')){
            $query->where('customer_business.EmployeeName', 'like', '%'.$request->EmployeeNameSearch.'%');
        }
        ...
        if($request->post('TransDateSearch')){
            ...
            if(isset($dateArr[2])) {
                ...
                $query->where(DB::raw('Date(customer_business.TransDate)'), str_replace('/', '-', $todate));
                $search = 1;
            }
        }
        ...
        if($request->post('fromdate')){
            ...
            if(isset($dateArr[2])){
                ...
                $query->where('customer_business.TransDate', '>=', str_replace('/', '-', $fromdate) .' 00:00');
                $search = 1;
            }
        }
        if($request->post('todate')){
            ...
            if(isset($dateArr[2])) {
                $todate = $dateArr[2] . '-' . $dateArr[1] . '-' . $dateArr[0];
                $query->where('customer_business.TransDate', '<=', str_replace('/', '-', $todate) . ' 23:59');
                $search = 1;
            }
        }
