...
use Module\Sbpassignplan\Models\Sbpassignplan;
...
class SbpestimateplanController extends Controller{
    ...
    public function list(Request $request){
        $data = Sbpestimateplan::search($request);
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data'] = $data;
    ...
    public function export(Request $request){
        return Excel::download(new ModelExport('act_gvouc_trans', ['select' => ['TransNo', 'TransDate'], 'conditions' => []]), 'Sbpestimateplan.xlsx');
    }
    ...
    public function store(Request $request){
        ...
        foreach ($detail as $item) {
            $item = (object)$item;
            if (!empty($item->AccountNo) || !empty($item->NormTableItemNo)) {
                $arrInsert[] = [
                    'TransID' => $TransID,
                    ...
                    'FCExecAmount' => isset($item->FCExecAmount) ? $item->FCExecAmount : '0',
                    'LCExecAmount' => isset($item->LCExecAmount) ? $item->LCExecAmount : '0',
    ...
    public function getDetailReportData(Request $request){
        ...
        $id = $request->id;
        $master = Sbpestimateplan::find($id);
    ...
    public function update(Request $request, $id){
        ...
        $master = (object)$request->master;
        $detail = $request->detail;
        $Sbpmakeplan = Sbpestimateplan::find($id);
        if(!$Sbpmakeplan){
            $json['status'] = 2;
        ...
        $Sbpmakeplan->InputCompanyID = Auth::user()->CompanyID;
    ...
    public function actbook(Request $request){
        ...
        $detail = $request->detail;
        $arrInsert = []; $arrInsert1 = [];
        if($Posted==1){
            SbpmakeplanBook::where('TransID', $TransID)->delete();
            Sbpestimateplan::where(['TransID' => $TransID])->update(['Posted' => 0]);
        }else {
            $PostedDate = date('Y-m-d H:i', time());
            Sbpestimateplan::where(['TransID' => $TransID])->update(['Posted' => 1,'PostedDate' => $PostedDate, 'PostedUserID' => Auth::user()->UserID]);
            foreach ($detail as $item) {
                $item = (object)$item;
...
//                        'DirectionName' => $DirectionName,
//                        'AccountID' => isset($item->AccountID) ? $item->AccountID : '',
//                        'AccountNo' => isset($item->AccountNo) ? IjCoa::stripCoa($item->AccountNo) : '',
//                        'CoAccountID' => isset($item->CoAccountID) ? $item->CoAccountID : '',
//                        'CoAccountNo' => isset($item->CoAccountNo) ? IjCoa::stripCoa($item->CoAccountNo) : '',
//                        'Description' => isset($item->Description) ? $item->Description : '',
//                        'CcyID' => isset($item->CcyID) ? $item->CcyID : '',
//                        'CcyNo' => isset($item->CcyNo) ? $item->CcyNo : '',
//                        'CcyName' => isset($item->CcyName) ? $item->CcyName : '',
...
//                        'NormAllotLevelNo' => isset($item->NormAllotLevelNo) ? $item->NormAllotLevelNo : '',
//                        'NormAllotLevelName' => isset($item->NormAllotLevelName) ? $item->NormAllotLevelName : '',
//                        'PathCompanyID' => Auth::user()->PathCompanyID,
//                    ];
                }
            }
...
//        }
    ...
    public function getList(Request $request){ //Log::debug('Test');
        $data = null;
        $query = Sbpassignplan::query();
        $where = [];
        //$query->where('InputCompanyID', '=', Auth::user()->CompanyID);
    ...
    public function loadTemp($id)
    {
        ...
        $data = Sbpassignplan::find($id);
        //Detail
        $query = Sbpassignplan::query();
        //$query->where('InputCompanyID', '=', Auth::user()->CompanyID);
        $query->where('TransTypeID', '=', 6);
        $query->where('TransDate', '>=', $Year.'-01-01');
        $query->where('TransDate', '<=', $Year.'-12-31');//Log::debug($query->toSql());
        $data_detail = $query->orderBy('TransDate', 'desc')->get();
        $arrTransID = [];
        foreach ($data_detail as $item) {
            $arrTransID[] = $item->TransID;
        }
        //$SbpmakeplanTrans = SbpmakeplanTrans::with('actautoact')->where('TransID',$id)->get();
        $SbpmakeplanTrans = SbpmakeplanTrans::with('actautoact')->whereIn('TransID',$arrTransID)->get();
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data']['data'] = $data;
        $json['data']['SbpmakeplanTrans'] = $SbpmakeplanTrans;
    ...
    public function getlistregu(Request $request){
        ...
        $PostDate = date('Y-m-d', strtotime(str_replace('/', '-', $request->post('PostDate'))));
        $RevenueRegu = DB::table('revenue_regu')->where('RevenueNo', '=', $RevenueNo)->where('EffectiveDate', '<=', $PostDate)->where('ExpirationDate', '>=', $PostDate)->where('RevenueReguActive', '=', 1)->get();
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
