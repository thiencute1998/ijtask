...
use Module\Sbpapprovalplan\Models\Sbpapprovalplan;
...
class SbpapprovalplanController extends Controller{

    public function list(Request $request){
        $data = Sbpapprovalplan::search($request);
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data'] = $data;
    ...
    public function export(Request $request){
        return Excel::download(new ModelExport('act_gvouc_trans', ['select' => ['TransNo', 'TransDate'], 'conditions' => []]), 'Sbpapprovalplan.xlsx');
    }
        ...
        foreach ($detail as $item) {
            ...
            if (!empty($item->FiscalYear)) {
                $arrInsert[] = [
                    ...
                    'TransTypeID' => $master->TransTypeID,
                    'FCReviewAmount' => isset($item->FCReviewAmount) ? $item->FCReviewAmount : '0',
                    'LCReviewAmount' => isset($item->LCReviewAmount) ? $item->LCReviewAmount : '0',
    ...
    public function getDetailReportData(Request $request){
        ...
        $id = $request->id;
        $master = Sbpapprovalplan::find($id);
    ...
    public function update(Request $request, $id){
        ...
        $master = (object)$request->master;
        $detail = $request->detail;
        $Sbpmakeplan = Sbpapprovalplan::find($id);
        if(!$Sbpmakeplan){
            $json['status'] = 2;
    ...
    public function actbook(Request $request){
        ...
        if($Posted==1){
            ...
            Sbpapprovalplan::where(['TransID' => $TransID])->update(['Posted' => 0]);
        }else {
            $PostedDate = date('Y-m-d H:i', time());
            Sbpapprovalplan::where(['TransID' => $TransID])->update(['Posted' => 1,'PostedDate' => $PostedDate, 'PostedUserID' => Auth::user()->UserID]);
            foreach ($detail as $item) {
                $item = (object)$item;
    ...
    //Lấy dữ liệu từ Xem xét dự toán
