...
class SbpassignplanController extends Controller{

    public function list(Request $request){
        $data = Sbpassignplan::search($request);
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data'] = $data;
    ...
    public function export(Request $request){
        return Excel::download(new ModelExport('act_gvouc_trans', ['select' => ['TransNo', 'TransDate'], 'conditions' => []]), 'Sbpassignplan.xlsx');
    }
        ...
        foreach ($detail as $item) {
            ...
            if (!empty($item->FiscalYear)) {
                $arrInsert[] = [
                    ...
                    'TransTypeID' => $master->TransTypeID,
                    'FCApprovedAmount' => isset($item->FCApprovedAmount) ? $item->FCApprovedAmount : '0',
                    'LCApprovedAmount' => isset($item->LCApprovedAmount) ? $item->LCApprovedAmount : '0',
    ...
    public function update(Request $request, $id){
        ...
        $master = (object)$request->master;
        $detail = $request->detail;
        $Sbpmakeplan = Sbpassignplan::find($id);
        if(!$Sbpmakeplan){
            $json['status'] = 2;
    ...
    public function actbook(Request $request){
        ...
        if($Posted==1){
            ...
            Sbpassignplan::where(['TransID' => $TransID])->update(['Posted' => 0]);
        }else {
            $PostedDate = date('Y-m-d H:i', time());
            Sbpassignplan::where(['TransID' => $TransID])->update(['Posted' => 1,'PostedDate' => $PostedDate, 'PostedUserID' => Auth::user()->UserID]);
            foreach ($detail as $item) {
                $item = (object)$item;
    ...
    //Lấy dữ liệu từ phê duyệt dự toán
