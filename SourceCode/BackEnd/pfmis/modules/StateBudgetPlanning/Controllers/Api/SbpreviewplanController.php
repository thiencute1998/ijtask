...
class SbpreviewplanController extends Controller{

    public function list(Request $request){
        $data = Sbpreviewplan::search($request);
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data'] = $data;
    ...
    public function export(Request $request){
        return Excel::download(new ModelExport('act_gvouc_trans', ['select' => ['TransNo', 'TransDate'], 'conditions' => []]), 'Sbpreviewplan.xlsx');
    }
...
//        $Revenue = Revenue::where('Inactive','!=',1)->get();
        ...
        $Revenue = cache()->remember('revenues', 60*60*8, function(){
            return Revenue::where('Inactive','!=',1)->get();
        });

        $Expense = cache()->remember('expenses', 60*60*8, function(){
            return Expense::where('Inactive','!=',1)->get();
        });

        $SbiChapter = cache()->remember('sbi_chapters', 60*60*8, function(){
            return SbiChapter::where('Inactive','!=',1)->get();
        });

        $SbiCategory = cache()->remember('sbi_categorys', 60*60*8, function(){
            return SbiCategory::where('Inactive','!=',1)->get();
        });

        $SbiItem = cache()->remember('sbi_items', 60*60*8, function(){
            return SbiItem::where('Inactive','!=',1)->get();
        });
        //$SbpPlan = config('dataoptions.SbpPlan');
        ...
        $json['data']['SbiChapter'] = $SbiChapter;
        $json['data']['SbiCategory'] = $SbiCategory;
        $json['data']['SbiItem'] = $SbiItem;
        //$json['data']['SbpPlan'] = $SbpPlan;
        ...
        foreach ($detail as $item) {
            ...
            if (!empty($item->FiscalYear)) {
                $arrInsert[] = [
                    ...
                    'TransTypeID' => $master->TransTypeID,
                    'FCRequestAmount' => isset($item->FCRequestAmount) ? $item->FCRequestAmount : '0',
                    'LCRequestAmount' => isset($item->LCRequestAmount) ? $item->LCRequestAmount : '0',
    ...
    public function update(Request $request, $id){
        ...
        $master = (object)$request->master;
        $detail = $request->detail;
        $Sbpmakeplan = Sbpreviewplan::find($id);
        if(!$Sbpmakeplan){
            $json['status'] = 2;
        ...
        $Sbpmakeplan->PeriodID = isset($master->PeriodID) ? $master->PeriodID : '';
    ...
    public function actbook(Request $request){
        ...
        if($Posted==1){
            ...
            Sbpreviewplan::where(['TransID' => $TransID])->update(['Posted' => 0]);
        }else {
            $PostedDate = date('Y-m-d H:i', time());
            Sbpreviewplan::where(['TransID' => $TransID])->update(['Posted' => 1,'PostedDate' => $PostedDate, 'PostedUserID' => Auth::user()->UserID]);
            foreach ($detail as $item) {
                $item = (object)$item;
    ...
    //Lấy dữ liệu từ Lập dự toán
    public function getList(Request $request){
        $data = null;
        $query = Sbpmakeplan::query();
        $where = [];
    ...
    public function loadTemp($id)
    {
        ...
        Sbpmakeplan::where(['TransID' => $id])->update(['RequestRepID' => 1]);
        $data = Sbpmakeplan::find($id);
        //$SbpmakeplanTrans = SbpmakeplanTrans::where(['TransID' => $id])->get();
        $SbpmakeplanTrans = SbpmakeplanTrans::with('actautoact')->where('TransID',$id)->get();
        //Log::debug(SbpmakeplanTrans::with('actautoact')->where('TransID',$id)->toSql());
        $json['status'] = 1;
        $json['msg'] = 'Thành công!';
        $json['data']['data'] = $data;
        $json['data']['SbpmakeplanTrans'] = $SbpmakeplanTrans;
