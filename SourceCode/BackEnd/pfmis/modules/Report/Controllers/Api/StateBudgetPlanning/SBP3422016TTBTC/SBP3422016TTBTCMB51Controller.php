...
class SBP3422016TTBTCMB51Controller extends Controller {
    public function SBP3422016TTBTC_MB51(Request $request){
        $json = [
            'status' => 1,
            'msg' => '',
            'data' => null
        ...
        $templateTable = DB::table('report_sbp_3422016ttbtc_mb51')->get();
        ...
        $this->I1235_I123($whereCondition, $month, $year);
        $this->I1235_I45689101112($whereCondition, $month, $year);
        $this->I1235_I7($whereCondition, $month, $year);
        $this->I1235_II($whereCondition, $month, $year);
        $this->I1235_III1($whereCondition, $month, $year);
        $this->I1235_III2($whereCondition, $month, $year);
        $this->I1235_IV($whereCondition, $month, $year);
...
//        $this->I4_I6();
    ...
    // ===================================== A =====================================
    public function I1235_I123($whereCondition, $month, $year){
        $preYear = $year - 1;
    ...
    public function  I1235_I45689101112($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
    ...
    public function  I1235_I7($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
                        ...
                        FROM (
                                 SELECT SUM(A.FCDebitAmount) AS I,A.AccountNo,A.PeriodFromDate,
                                        (Case when (RC.CateNo ='201110501' AND RC.CateValue='1') THEN 'A#01#07#01'
                                             ...
                                             when (RC.CateNo ='201110504' AND ( RC.CateValue='1' OR RC.CateValue='2')) THEN 'A#01#07#04'
                                             ELSE  'A#01#07#05'
                                         END) as ItemID
                                 FROM act_gl_books as A
                                            INNER JOIN revenue_cate as RC on RC.RevenueID = A.RevenueID
                                 WHERE A.AccountNo IN ('0341','0711','0361')
                                      AND ((RC.CateNo = '201110501' AND RC.CateValue='1') OR
                                          (RC.CateNo = '201110502' AND RC.CateValue='1') OR
                                          (RC.CateNo = '201110503' AND RC.CateValue='1') OR
                                          (RC.CateNo = '201110504' AND ( RC.CateValue='1' OR RC.CateValue='2')) OR
    ...
    public function  I1235_II($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
    ...
    public function  I1235_III1($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
        ...
        $sqlUpdate = " UPDATE table_tmp AS TT,(SELECT ItemID,
                        ...
                        FROM (
                                 SELECT SUM(A.FCDebitAmount) AS I,A.AccountNo,A.PeriodFromDate,
                                        (Case WHEN (RC.CateNo ='20113' AND RC.CateValue='5') THEN 'B#01#01'
                                             WHEN (RC.CateNo ='20113' AND RC.CateValue='1') THEN 'B#01#02'
                                             WHEN (RC.CateNo ='20113' AND ( RC.CateValue='2' OR RC.CateValue='6')) THEN 'B#01#03'
                                             WHEN (RC.CateNo ='20113' AND RC.CateValue='3') THEN 'B#01#04'
                                             WHEN (RC.CateNo ='20113' AND RC.CateValue='4')THEN 'B#01#05'
                                             ELSE 'B#01#06'
                                        END) as ItemID
                                 FROM act_gl_books as A
                                            INNER JOIN revenue_cate as RC on RC.RevenueID = A.RevenueID
                                 WHERE A.AccountNo IN ('0341','0711','0361')
                                      AND ((RC.CateNo = '20113' AND RC.CateValue='5') OR
                                          (RC.CateNo = '20113' AND RC.CateValue='1') OR
                                          (RC.CateNo = '20113' AND ( RC.CateValue='2' OR RC.CateValue='6')) OR
    ...
    // PS Nợ TK0441, 0721, 0461 , Khoản chi 5106 (Tất cả khoản chi con )
    public function  I1235_III2($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
    ...
    // PS Nợ TK0341 , 0711, 0361 Loại khoản thu 20114 (Tất cả GT) //đã test
    public function  I1235_IV($whereCondition, $month, $year){
        $preYear = $year - 1;
        $fromDateArr = $year . '/' . $month . '/01';
        $fromDate = Date::StringToDate($fromDateArr);
        $toDateArr = $year . '/' . $month . '/'.$this->setDate($month, $year );
