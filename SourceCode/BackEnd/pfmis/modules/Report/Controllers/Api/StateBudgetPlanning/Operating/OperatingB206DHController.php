...
class OperatingB206DHController extends Controller {
    use Common;
    ...
    public function SBPB206DH(Request $request)
    {
        $json = [
            'status' => 1,
            'msg' => '',
        ...
        $this->getParentDataTreeCommon(['program','project'],'B');
        ...
        $this->setIndexB206($maxLevel, 'A#01');
        $this->setIndexB206($maxLevel, 'A#02');
        $this->setIndexB206($maxLevel, 'A#03');
        $this->setIndexB206($maxLevel, 'B#01');
        $this->setIndexB206($maxLevel, 'B#02');
        $this->setIndexB206($maxLevel, 'B#03');
        ...
        $insertArr1 = [
            ...
            ['STT'=>'I','ItemID'=> 'B#01','ParentID'=>'B','Level'=> 1, 'ItemName'=> 'Chương trình','FontWeight'=> '0'],
            ['STT'=>'II','ItemID'=> 'B#02','ParentID'=>'B','Level'=> 1,'ItemName'=> 'Dự án','FontWeight'=> '0'],
            ['STT'=>'III','ItemID'=> 'B#03','ParentID'=>'B','Level'=> 1,'ItemName'=> 'Phi dự án','FontWeight'=> '0']
        ];
        DB::table('table_tmp')->insert($insertArr1);

        $maxLevel = DB::table('table_tmp')->select(DB::raw('MAX(table_tmp.Level) AS maxLevel'))->first();
        $this->sumUpSBPB206DH(0,6, $maxLevel);
    ...
    public function insertRevenueII($whereCondition, $year){
        $sql = "INSERT INTO table_tmp (ItemID, ParentID, ItemName, StartFinishDate, SponsorName,Detail, I1, I2, I3, I4)
        SELECT CONCAT(ParentID,'#',Project), ParentID, ItemName,  StartFinishDate, SponsorName, 1,
        SUM(CASE WHEN AccountNo = '0361' AND YEARD <=($year - 2 ) THEN S ELSE 0 END) AS I1,
		MAX(CASE WHEN AccountNo = '0341' AND YEARD = $year - 1 THEN I ELSE 0 END) AS I2,
		MAX(CASE WHEN AccountNo = '0711' AND YEARD = $year - 1 THEN I ELSE 0 END) AS I3,
		MAX(CASE WHEN AccountNo = '0311' AND YEARD = $year THEN I ELSE 0 END) AS I4
        ...
        CONCAT(DATE_FORMAT(StartedDate, '%d/%m/%Y'), ' - ', DATE_FORMAT(FinishedDate, '%d/%m/%Y')) as StartFinishDate,YEAR(A.PeriodFromDate) AS YEARD,
        SponsorName, SUM(A.FCDebitAmount) AS I,SUM(LCDebitAmount - LCCreditAmount) as S
        FROM act_gl_books as A
        INNER JOIN capital_cate as CC on A.CapitalID = CC.CapitalID
        INNER JOIN project as P on A.ProjectID = P.ProjectID
        INNER JOIN project_cate as PC on PC.ProjectID = A.ProjectID
    ...
    public function insertExpenseI($whereCondition, $year){
        $sql = "INSERT INTO table_tmp (ItemID, ParentID, ItemName,Program, StartFinishDate, SponsorName, ProjectTemporary,Detail, I1, I2, I3, I4)
        SELECT CONCAT(ParentID,'#',Project), ParentID, ItemName, Program, StartFinishDate, SponsorName,'1', 1,
        SUM(CASE WHEN AccountNo = '0461' AND YEARD <=($year - 2 ) THEN S ELSE 0 END) AS I1,
        ...
        FROM
        (SELECT CONCAT('B#','01#',A.ProgramNo) AS ParentID, A.ProjectName as ItemName, A.ProjectNo as Project, A.AccountNo,
        CONCAT(DATE_FORMAT(StartedDate, '%d/%m/%Y'), ' - ', DATE_FORMAT(FinishedDate, '%d/%m/%Y')) as StartFinishDate,YEAR(A.PeriodFromDate) AS YEARD,
        ...
        AND PC.CateNo = '018' AND PC.CateValue = 1
        $whereCondition
        GROUP BY A.AccountNo,P.ProjectNo, YEAR(A.PeriodFromDate),A.ProgramNo
        ) as CH GROUP BY Project, Program";
    ...
    public function insertExpenseII($whereCondition, $year){
		...
		MAX(CASE WHEN AccountNo = '0411' AND YEARD = $year THEN I ELSE 0 END) AS I4
        FROM
        (SELECT CONCAT('B#','02') AS ParentID, A.ProjectName as ItemName,A.ProjectNo as Project, A.AccountNo,
        CONCAT(DATE_FORMAT(StartedDate, '%d/%m/%Y'), ' - ', DATE_FORMAT(FinishedDate, '%d/%m/%Y')) as StartFinishDate,YEAR(A.PeriodFromDate) AS YEARD,
        SponsorName, SUM(A.FCDebitAmount) AS I,SUM(LCDebitAmount - LCCreditAmount) as S
        FROM act_gl_books as A
        INNER JOIN capital_cate as CC on A.CapitalID = CC.CapitalID
    ...
    public function insertExpenseIII($whereCondition, $year){
		...
		MAX(CASE WHEN AccountNo = '0411' AND YEARD = $year THEN I ELSE 0 END) AS I4
        FROM
        (SELECT CONCAT('B#','03') AS ParentID, A.ProjectName as ItemName,A.ProjectNo as Project, A.AccountNo,
        CONCAT(DATE_FORMAT(StartedDate, '%d/%m/%Y'), ' - ', DATE_FORMAT(FinishedDate, '%d/%m/%Y')) as StartFinishDate,YEAR(A.PeriodFromDate) AS YEARD,
        SponsorName, SUM(A.FCDebitAmount) AS I,SUM(LCDebitAmount - LCCreditAmount) as S
        FROM act_gl_books as A
        INNER JOIN capital_cate as CC on A.CapitalID = CC.CapitalID
    ...
    // SUM
    public function sumUpSBPB206DH($start, $end, $maxLevel){
        if ($maxLevel) {
    ...
    public function setIndexB206($maxLevel, $id = null){
        $insertArr = [];
