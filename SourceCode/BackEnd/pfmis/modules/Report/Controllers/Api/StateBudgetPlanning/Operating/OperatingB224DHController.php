...
class OperatingB224DHController extends Controller {
    use Common;
    ...
    public function SBPB224DH(Request $request)
    {
        $json = [
            'status' => 1,
            'msg' => '',
        ...
        $insertArr1 = [
            ['STT'=>'', 'ItemID'=> 'A', 'ParentID'=> '', 'Level'=> 0, 'ItemName'=> 'TỔNG THU', 'FontWeight'=> '1'],
        ];
        DB::table('table_tmp')->insert($insertArr1); // Query Builder approach
        $maxLevel = DB::table('table_tmp')->select(DB::raw('MAX(table_tmp.Level) AS maxLevel'))->first();
        $this->sumUpSBPB224DH(0,6, $maxLevel);
    ...
    // SUM
    public function sumUpSBPB224DH($start, $end, $maxLevel){
        if ($maxLevel) {
