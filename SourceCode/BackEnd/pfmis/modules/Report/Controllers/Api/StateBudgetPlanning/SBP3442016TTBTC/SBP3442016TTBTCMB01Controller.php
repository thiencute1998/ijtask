<?php
namespace Module\Report\Controllers\StateBudgetPlanning\SBP3442016TTBTC;

use IjCore\IjNumber;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use Module\Listing\Models\Company;
use Module\Report\Controllers\Controller;
use Illuminate\Support\Facades\Auth;
use PHPUnit\Framework\Constraint\Count;

class SBP3442016TTBTCMB01Controller extends Controller {
    public function SBP3442016TTBTC_MB01(Request $request){

        $json = [
            'status' => 1,
            'msg' => '',
            'data' => null
        ];
        $filter = $request->post('filter');
        $year = (isset($filter['Year'])) ? $filter['Year'] : 2021;
        $companyID = (isset($filter['CompanyID'])) ? $filter['CompanyID'] : null;
        $directionID = (isset($filter['DirectionID'])) ? $filter['DirectionID'] : null;
        $note = 'Ghi chú: ';
        if (isset($filter['SectorName'])) {
            $note .= 'Ngành ' . $filter['SectorName'] . ', ';
        }
        if (isset($filter['ProvinceName'])) {
            $note .= 'tỉnh ' . $filter['ProvinceName'] . ', ';
        }
        if (isset($filter['DistrictName'])) {
            $note .= 'huyện ' . $filter['DistrictName'] . ', ';
        }
        if (isset($filter['CommuneName'])) {
            $note .= 'xã ' . $filter['CommuneName'] . ', ';
        }
        if (isset($filter['CompanyName'])) {
            $note .= 'đơn vị ' . $filter['CompanyName'];
        }

        $whereCompany = '';
        if ($companyID) {
            $whereCompany .= " AND A.CompanyID = " . $companyID . ' ';
        }
        $whereDirection = '';
        if ($directionID) {
            $whereDirection = 'AND A.DirectionID = ' . $directionID . ' ';
        }

        $yearPre = (int)$year - 1;
        $yearNext = (int)$year + 1;

        $whereCondition = "AND (
        (YEAR(A.PeriodFromDate) = $year AND YEAR(A.PeriodToDate) = $year)
        OR (YEAR(A.PeriodFromDate) = $yearPre AND YEAR(A.PeriodToDate) = $yearPre)
        OR (YEAR(A.PeriodFromDate) = $yearNext AND YEAR(A.PeriodToDate) = $yearNext)
        ) $whereCompany";

        $createTable = "CREATE TEMPORARY TABLE IF NOT EXISTS `table_tmp` (
          `STT` varchar(10) DEFAULT '',
          `ItemID` varchar(100) NOT NULL DEFAULT '',
          `ParentID` varchar(100) DEFAULT '',
          `Level` int(2) DEFAULT 1,
          `ItemName1` varchar(255) DEFAULT NULL,
          `ItemName2` varchar(255) DEFAULT NULL,
          `FontWeight` tinyint(1) DEFAULT 0,
          `Italic` tinyint(1) DEFAULT 0,
          `Center` tinyint(1) DEFAULT 0,
          `SbiCategoryNo` varchar(20) DEFAULT '',
          `ConAccountNo` varchar(20) DEFAULT '',
          `ConCoAccountNo` varchar(20) DEFAULT '',
          `SbiCategoryValue` varchar(20) DEFAULT '',
          `ExpenseCateValue` varchar (20) DEFAULT '',
          `NormCateValue` varchar(20) DEFAULT '',
          `I1` double(20,4) DEFAULT NULL,
          `I2` double(20,4) DEFAULT NULL,
          PRIMARY KEY (`ItemID`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        DB::statement($createTable);
        $templateTable = DB::table('report_sbp_3442016ttbtc_mb01')->get();

        $insertArr = [];
        foreach ($templateTable as $template) {
            $insertArr[] = [
                'STT' => $template->STT,
                'ItemID' => $template->ItemID,
                'ParentID' => $template->ParentID,
                'Level' => $template->Level,
                'ItemName1' => $template->ItemName1,
                'ItemName2' => $template->ItemName2,
                'FontWeight' => $template->FontWeight,
                'Italic' => $template->Italic,
                'Center' => $template->Center,
                'SbiCategoryNo' => $template->SbiCategoryNo,
                'ConAccountNo' => $template->ConAccountNo,
                'ConCoAccountNo' => $template->ConCoAccountNo,
                'SbiCategoryValue' => $template->SbiCategoryValue,
                'ExpenseCateValue' => $template->ExpenseCateValue,
                'NormCateValue' => $template->NormCateValue,
                'I1' => $template->I1,
                'I2' => $template->I2,
            ];
        }
        DB::table('table_tmp')->insert($insertArr);
        $this->I1_124($whereCondition, $year);
        $this->I2_123($whereCondition, $year);
        $this->I1_3($whereCondition, $year);
        // sum up
        $this->sumUpTableTmp(1, 2);

        $date = date('d');
        $month = date('m');
        $yearC = date('Y');
        $data = DB::table("table_tmp")->get();

        foreach ($data as $key => $value) {
            $data[$key]->I1 = ($data[$key]->I1) ? $data[$key]->I1 : '';
            $data[$key]->I2 = ($data[$key]->I2) ? $data[$key]->I2 : '';
        }

        $company = Company::find($companyID);
        $dateTime = "Vĩnh Phú, ngày $date tháng $month năm $yearC";
        if ($company) {
            $dateTime = $company->Address . ", ngày $date tháng $month năm $yearC";
        }

        $response = [
            'master' => [
                "Year" => "",
                "CreateName" => "Trần Thế Thái",
                "TransNo" => "00001",
                "ReportName" => "BIỂU CÂN ĐỐI TỔNG HỢP DỰ TOÁN NGÂN SÁCH XÃ NĂM " . $year,
                "DateTime" => $dateTime,
                "CompanyName" => 'Tỉnh Vĩnh Phúc
                                  Huyện Yên Lạc
                                  Xã Trung Hà',
                "Note" => $note
            ],
            'detail' => $data
        ];

        $json['data'] = $response;
        return json_encode($json);
    }
    // ===================================== A =====================================
    public function I1_124($whereCondition, $year){
        $sqlUpdate = "UPDATE table_tmp as TT, (
                            SELECT SUM(A.FCDebitAmount) AS I1,
                               (CASE
                                    WHEN LEFT(RC.CateNo, 9) = '202110301' THEN 'A'
                                    WHEN LEFT(RC.CateNo, 9) = '202110302' THEN 'B'
                                    WHEN (RC.CateNo = '202110304' AND RC.CateValue = '1') THEN 'D'
                                   END) as ItemID
                            FROM act_gl_books AS A
                                     INNER JOIN revenue_cate AS RC ON RC.RevenueID = A.RevenueID
                            WHERE ( LEFT(RC.CateNo, 9) = '202110301' OR
                                    LEFT(RC.CateNo, 9) = '202110302' OR
                                    (RC.CateNo = '202110304' AND RC.CateValue = '1'))
                                   AND A.AccountNo = '0341'
                                   AND A.BudgetLevel = '4'
                                   AND YEAR(A.PeriodFromDate) = '$year'
                                        $whereCondition
                            GROUP BY ItemID, RC.CateNo
                            )AS HH
                        SET TT.I1 = HH.I1
                        WHERE  TT.ItemID = HH.ItemID";
        DB::statement($sqlUpdate);
    }
    public function I1_3($whereCondition, $year){
        $sqlUpdate = "UPDATE table_tmp as TT, (
                            SELECT SUM(A.FCDebitAmount) AS I1,
                               (CASE
                                    WHEN (RC.CateNo = '202110306' AND RC.CateValue = '1') THEN 'C#01'
                                    WHEN (RC.CateNo = '202110306' AND RC.CateValue = '2') THEN 'C#02'
                                END) as ItemID
                            FROM act_gl_books AS A
                                     INNER JOIN revenue_cate AS RC ON RC.RevenueID = A.RevenueID
                            WHERE  RC.CateNo = '202110306' AND RC.CateValue IN('1','2')
                                   AND A.AccountNo = '0341'
                                   AND A.BudgetLevel = '4'
                                   AND YEAR(A.PeriodFromDate) = '$year'
                                        $whereCondition
                            GROUP BY ItemID, RC.CateValue
                            )AS HH
                        SET TT.I1 = HH.I1
                        WHERE  TT.ItemID = HH.ItemID";
        DB::statement($sqlUpdate);
    }
    public function I2_123($whereCondition, $year){
        $sqlUpdate = "UPDATE table_tmp as TT, (
                            SELECT SUM(A.FCDebitAmount) AS I2,
                               (CASE
                                    WHEN LEFT(EC.CateNo, 5) = '20152' THEN 'A'
                                    WHEN LEFT(EC.CateNo, 5) = '20151' THEN 'B'
                                    WHEN (EC.CateNo = '20159' AND EC.CateValue = '1') THEN 'C'
                                   END) as ItemID
                            FROM act_gl_books AS A
                                     INNER JOIN expense_cate AS EC ON EC.ExpenseID = A.ExpenseID
                            WHERE ( LEFT(EC.CateNo, 5) = '20152' OR
                                    LEFT(EC.CateNo, 5) = '20151' OR
                                    (EC.CateNo = '20159' AND EC.CateValue = '1'))
                                   AND A.AccountNo = '0441'
                                   AND A.BudgetLevel = '4'
                                   AND YEAR(A.PeriodFromDate) = '$year'
                                        $whereCondition
                            GROUP BY ItemID, EC.CateNo
                            )AS HH
                        SET TT.I2 = HH.I2
                        WHERE  TT.ItemID = HH.ItemID";
        DB::statement($sqlUpdate);
    }

    // sum up
    public function sumUpTableTmp($start, $end = null){
        $maxLevel = DB::table('table_tmp')->select(DB::raw('MAX(table_tmp.Level) AS maxLevel'))->first();
        if ($maxLevel) {
            $end = $maxLevel->maxLevel;
        }
        if ($end) {
            for ($i = $end; $i > $start; $i--) {
                $j = $i - 1;
                $sql = "SELECT ItemID, ParentID, SUM(M.I1) AS I1, SUM(M.I2) AS I2
                FROM table_tmp M
                WHERE M.Level = " . $i . " GROUP BY ParentID";
                $result = DB::select($sql);
                $result = (array)$result;
                if ($result) {
                    foreach ($result as $value) {
                        $value = (array)$value;
                        $I1 = ($value['I1']) ? $value['I1'] : 0;
                        $I2 = ($value['I2']) ? $value['I2'] : 0;
                        $update = "UPDATE table_tmp
                    SET I1 = " . $I1 . ", I2 = " . $I2 . "
                    WHERE ItemID = '" . $value['ParentID'] . "'
                    AND table_tmp.Level = " . $j;
                        DB::statement($update);
                    }
                }
            }
        }
    }

}
