...
class OperatingB202DHController extends Controller {
    use Common;
    ...
    public function insertRevenue($whereCondition, $year){
		...
		(SELECT CONCAT('A#',RCL.Path) AS ParentID, A.RevenueName as ItemName, A.RevenueNo as Revenue, RCL.Path,
        A.AccountNo, A.BudgetLevel , YEAR(A.PeriodFromDate) AS YEARD, SUM(A.FCDebitAmount) AS I FROM act_gl_books as A
        INNER JOIN revenue_cate as RC on A.RevenueID = RC.RevenueID
        INNER JOIN revenue_cate_list as RCL on RC.CateID = RCL.CateID
		WHERE A.AccountNo IN ('0341','0311')
        AND A.BudgetLevel IN ('2','3','4') AND left(RC.CateNo,5)='20011'
        $whereCondition
    ...
    public function insertExpense($whereCondition, $year){
		...
		(SELECT CONCAT('A#',ECL.Path) AS ParentID, A.ExpenseName as ItemName, A.ExpenseNo as Expense, ECL.Path as Path,EC.CateNo as CateNo,
		...
		WHERE left(EC.CateNo,5) BETWEEN  '20051' AND  '20069'  AND A.AccountNo IN ('0441','0411')
		  $whereCondition
