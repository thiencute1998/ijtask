...
class OperatingB205DHController extends Controller {
    use Common;
    ...
    public function SBPB205DH(Request $request)
    {
...
//          Thu
        ...
        $this->getParentDataTreeForB200([['company_cate_value','0001'],'company','revenue_cate_list','revenue'],'A');
...
//        $this->updateIndexReport('B');
        ...
        $this->sumUpSBPB205DH(0,6, $maxLevel);
    ...
    // SUM
    public function sumUpSBPB205DH($start, $end, $maxLevel){
        if ($maxLevel) {
    ...
    public function insertRevenue($whereCondition, $year){
        $sql = "INSERT INTO table_tmp (ItemID, ParentID, ItemName, Revenue, Path, CompanyCateValue, Company, Detail, I1, I2, I3, I4)
        SELECT CONCAT(ParentID, '#', Revenue), ParentID, ItemName, Revenue, Path, CompanyCateValue, Company, 1,
		...
		(SELECT CONCAT('A#', CC.CateValue, '#', C.CompanyNo,'#',RCL.Path) AS ParentID, A.RevenueName as ItemName, A.RevenueNo as Revenue, RCL.Path,
        A.AccountNo, CC.CateValue as CompanyCateValue, C.CompanyNo as Company, YEAR(A.PeriodFromDate) AS YEARD,
        SUM(A.LCDebitAmount) AS I FROM act_gl_books as A
        ...
        AND CC.CateNo = '0001' AND CC.CateValue IS NOT NULL
        AND LEFT(RC.CateNo, 5) = '20011'
        $whereCondition
        GROUP BY A.AccountNo, A.RevenueNo, A.InputCompanyID, YEAR(A.PeriodFromDate)) as CH GROUP BY Revenue, Company";
    ...
    public function getTotalRevenue(){
        $list = DB::table('table_tmp')->where('Level',1)
            ->where(DB::raw('LEFT(ItemID,1)'),'=','A')->get();

        foreach($list as $item){
            $detailItemID = explode('#',$item->ItemID);
            $itemID = join('#',array($detailItemID[0], sprintf("%04d",($detailItemID[1]))));
